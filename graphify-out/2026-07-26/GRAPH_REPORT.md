# Graph Report - Voyager  (2026-07-25)

## Corpus Check
- 358 files · ~348,812 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7876 nodes · 11151 edges · 442 communities (249 shown, 193 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 47 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `368ac28e`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Database Data
- Calendar Features
- Calendar Day
- Journal Features
- Features Shell
- Calendar Features
- Sync Remote
- Todo Features
- Repositories Domain
- Calendar Grid
- App Dev
- Todo Features
- Models Domain
- Calendar Markers
- Dev Remote
- Repositories Data
- Sync Weather
- Todo Task
- App Providers
- Analytics Features
- Merger Services
- Search Features
- Todo Features
- Text Widgets
- Windows Runner
- Widgets Color
- Widgets Rounded
- Widgets Contextual
- Windows Ephemeral
- Time Widgets
- Calendar Features
- Widgets Datetime
- Domain Services
- Windows Cpp
- Shell Features
- Import Export
- Todo List
- Sync Firestore
- Windows Ephemeral
- Sync Engine
- Calendar Event
- Sync Conflict
- Calendar Features
- Features Dev
- Models Domain
- Todo Features
- Sync Conflict
- Time Widgets
- Domain Services
- Dev Journal
- Windows Ephemeral
- Widgets Text
- Weather Features
- Widgets Field
- Weather Forecast
- Windows Ephemeral
- Windows Ephemeral
- Widgets Notched
- Data Remote
- Crdt Frac
- Weather Features
- Journal Features
- Sync Activity
- Models Domain
- Platform Keyboard
- Database Users
- Features Shell
- Windows Ephemeral
- Firestore Data
- Windows Ephemeral
- Widgets Geometric
- Text Sync
- Calendar Features
- Domain Services
- Widgets Keep
- Domain Services
- Main App
- Widgets Date
- Features Shell
- Calendar Features
- Sync Journal
- Domain Services
- Shell Shortcuts
- Models Domain
- Cloud Data
- Journal Features
- Widgets Desktop
- Domain Calendar
- Windows Ephemeral
- Client Data
- Dev Cache
- Features Shell
- Functions Forecast
- Features Page
- Dev Features
- Theme Menu
- Registrar Windows
- Functions Src
- Dev Settings
- Dev Todo
- Binding Utils
- Widgets Resizable
- Auth Data
- Character Domain
- Features Calendar
- Features Calendar
- Widgets Clamp
- Hotkey Features
- Palette Color
- Dev Sync
- Domain Repositories
- Calendar Features
- Windows Ephemeral
- Widgets Desktop
- Widgets Dropdown
- App Providers
- Windows Ephemeral
- Sync Debouncer
- Widgets Menu
- Widgets Search
- Models Domain
- Settings Palette
- Windows Ephemeral
- Platform Widgets
- Shell Features
- Windows Ephemeral
- Database Data
- Menu Widgets
- Sync Outbox
- Functions Scripts
- Dev Warmup
- Todo Task
- Gradient Widgets
- Models Analytics
- Models Domain
- Windows Ephemeral
- Platform Domain
- Domain Services
- Functions Tsconfig
- Auth Firebase
- Color Widgets
- FlutterView
- Windows Ephemeral
- Windows Ephemeral
- Widgets Selector
- Constants Google
- Features Analytics
- Features Settings
- Shell App
- Widgets Features
- Constants Journal
- Widgets Confirm
- Weather Fakes
- Features Settings
- warmup_tracker.dart
- Data Remote
- Delta Scramble
- Dev Sync
- Windows Runner
- Domain Services
- Shell Features
- Windows Ephemeral
- Sync Text
- Sync Pending
- App Providers
- trackerRepositoryProvider
- Theme Spacing
- quick_popups.dart
- Windows Ephemeral
- @immutable
- ByteBufferStreamReader
- Widgets Enter
- Gpusurfacetexture Windows
- @immutable
- Constants Todo
- Shell Features
- Messenger Windows
- dart:math
- Constants Hotkey
- Sync Firestore
- @visibleForTesting
- MethodCall
- Anagram List1
- dart:math
- Calendar Features
- debouncer.dart
- Widgets Rounded
- Chart Weather
- Android Gradlew
- Deinterleave Evens
- WeatherApiClient
- Dev Flags
- Utils Ids
- Utils Journal
- Google Desktop
- Constants Todo
- PLAN.md
- Mainactivity Android
- Calendar Customclipper
- High-Level Design: Voyager Weekly Calendar View
- IMPORT_EXPORT.md
- Constants App
- todoSortDebugLoggerProvider
- Color
- Constants Default
- desktop_google_oauth_test.dart
- Domain Services
- Calendar Features
- SingleChildLayoutDelegate
- ADR 001: Local-First Data Model
- ADR 002: Sync Protocol Boundaries
- ADR 003: Module Structure
- List
- _VoyagerAppState
- Voyager
- _AddListMenuItem
- weatherForecastProvider
- TIME_SELECTOR.md
- MessageHandler
- weather_chart_curve_test.dart
- CalendarRepository
- graphify.md
- graphify.md
- FEEDBACK.md
- MorphDayEventStack
- package:voyager/domain/services/weather_forecast_chart.dart
- _CalendarMorphWarmupState
- _HeatmapDayCell
- _MorphAnimationLayer
- MethodResultFunctions
- package:voyager/core/sync/text_delta_injector.dart
- addedAt
- sync_conflict.dart
- FlutterViewController::HandleTopLevelWindowProc
- alertTimeHour
- todoSortDebugLoggerProvider
- MethodResult
- voyager_spacing.dart
- cadence
- geometric_texture_settings.dart
- GpuSurfaceTexture
- calendarNavigateRightKey
- collection
- _HeatmapWeekBlock
- dart:math
- _AddListMenuItem
- completed
- _MonthGridBox
- package:voyager/core/sync/text_delta_injector.dart
- _TrackerDialog
- VoyagerColors
- _YearGridBox
- @visibleForTesting
- dart:async
- package:voyager/core/theme/voyager_theme.dart
- _MorphAnimationLayer
- _QuickDelayedDragStartListener
- _CalendarWeekTimelineState
- geometric_texture_settings.dart
- accentColor
- addedAt
- alertOnPeriodicPrompts
- alertTimeHour
- allocatedAt
- amountCents
- anchorDueDate
- asOf
- assetId
- birthYear
- body
- boolValue
- cadence
- calendarId
- calendarNavigateLeftKey
- calendarNavigateRightKey
- collection
- collectionName
- colorPaletteJson
- colorValue
- completed
- createdAt
- customQuote
- customStartupPage
- defaultBool
- defaultEnumOption
- defaultInt
- deletedAt
- detectedAt
- devForceConflictUi
- deviceId
- devJournalDebugLog
- devOpenWeatherApiKey
- devShowCacheStatus
- devShowCalendarInstantViewSwitch
- devShowCalendarZoomPrewarm
- devShowConflictDocumentIds
- devShowFpsCounter
- devShowJournalRemotePullButton
- devShowSyncDownloads
- devShowSyncLocalSaves
- devShowSyncUploads
- devSlowCalendarAnimations
- devTodoSortDebugLog
- devUseDirectOpenWeather
- documentId
- dueDate
- dueDateSetAt
- end
- entryDate
- enumOptionsJson
- enumValue
- externalId
- geometricTextureFocalPointX
- geometricTextureFocalPointY
- geometricTextureFocalSpread
- geometricTextureIntensity
- geometricTextureScale
- geometricTextureVariationFloor
- geometricWaveDirectionDegrees
- geometricWaveEnabled
- geometricWaveMaskClusterScale
- geometricWaveMaskDensity
- geometricWaveMassLagSeconds
- None Rect
- geometricWaveMassSpring
- None Size
- None String
- geometricWavePeriod
- geometricWavePopBrightness
- geometricWavePopBrightnessVariance
- geometricWavePopHoldSeconds
- geometricWavePopScale
- geometricWaveScatterLitAmount
- geometricWaveScatterMode
- geometricWaveShadowLightDegrees
- geometricWaveShadowOffset
- geometricWaveShadowSoftness
- geometricWaveShadowStrength
- geometricWaveShape
- geometricWaveSpeed
- geometricWaveTiltAmount
- geometricWaveTiltShading
- geometricWaveTwinkleSparsity
- geometricWaveWidth
- goalId
- guidedJournaling
- guidedPrompt
- hideCompletedTasks
- id
- package:voyager/domain/services/weather_forecast_chart.dart
- integerCap
- intValue
- isFullDay
- dart:ui
- journalEntryListWidth
- journalHotkey
- journalId
- lastSeenNavPage
- lastViewedJournalId
- lastViewedTodoListId
- limitCents
- listId
- localPayloadJson
- localText
- localTitle
- mood
- name
- navPageOrderJson
- note
- notes
- occurredAt
- parentTaskId
- period
- periodStart
- petalColor
- petalFallSpeed
- petalMaxCount
- petalWindFrequency
- petalWindStrength
- preStarSortOrder
- promptCycleDays
- quoteId
- recurrence
- remotePayloadJson
- remoteText
- remoteTitle
- richBodyJson
- showAnnualizedSubscriptionCost
- showDefaultTrackersInCalendar
- showDefaultTrackersInGrid
- showOnCalendar
- showQuotes
- sortOrder
- source
- starred
- start
- startupPageMode
- tag
- tagsJson
- targetCents
- targetDate
- themeMode
- timelineModeYearZero
- timestamp
- title
- todoCompletedSectionExpanded
- todoHotkey
- trackerId
- trackingStyle
- type
- updatedAt
- valueCents
- version
- weatherChartCurveTension
- weatherChartRainColor
- weatherChartTempColor
- weatherConditionCode
- weatherFetchedAt
- weatherForecastJson
- weatherIcon
- weatherLat
- weatherLocationLabel
- weatherLocationUpdatedAt
- weatherLon
- weatherTempC
- weekStartsOnMonday
- Map
- Set
- T
- GLASS_BUTTON.md
- weather_chart_curve_test.dart
- Iterable
- @visibleForTesting

## God Nodes (most connected - your core abstractions)
1. `remoteSyncServiceProvider` - 64 edges
2. `journalRepositoryProvider` - 36 edges
3. `settingsProvider` - 35 edges
4. `todoRepositoryProvider` - 29 edges
5. `FlutterEngine` - 26 edges
6. `DartProject` - 25 edges
7. `settingsRepositoryProvider` - 22 edges
8. `Win32Window` - 22 edges
9. `DataClass` - 21 edges
10. `MethodCodec` - 19 edges

## Surprising Connections (you probably didn't know these)
- `build` --references--> `currentWeatherProvider`  [EXTRACTED]
  test/shell_weather_widget_test.dart → lib/app/providers.dart
- `_FlakySyncRepository` --inherits--> `InMemorySyncRepository`  [EXTRACTED]
  test/sync_integration_test.dart → lib/data/remote/in_memory_sync.dart
- `ThrowingJournalRepository` --inherits--> `DriftJournalRepository`  [EXTRACTED]
  test/import_export_test.dart → lib/data/repositories/drift_repositories.dart
- `FakeAuthRepository` --implements--> `AuthRepository`  [EXTRACTED]
  test/import_export_test.dart → lib/domain/repositories/repositories.dart
- `_confirmDeleteAllEntries` --references--> `journalRepositoryProvider`  [EXTRACTED]
  lib/features/dev/dev_page.dart → lib/app/providers.dart

## Import Cycles
- None detected.

## Communities (442 total, 193 thin omitted)

### Community 0 - "Database Data"
Cohesion: 0.01
Nodes (386): BoolColumn get, _, _accentColorMeta, actualTableName, _addedAtMeta, _alertOnPeriodicPromptsMeta, _alertTimeHourMeta, _alias (+378 more)

### Community 1 - "Calendar Features"
Cohesion: 0.01
Nodes (257): AnimationStatusListener?, _, _abortMorphAnimation, accentColor, adjacentColor, _advanceWarmup, anchor, _applyYearTransform (+249 more)

### Community 2 - "Calendar Day"
Cohesion: 0.01
Nodes (221): accentColor, adjacentBorderT, adjacentTextT, alpha, _animatingEventId, available, barHeight, barStride (+213 more)

### Community 3 - "Journal Features"
Cohesion: 0.01
Nodes (138): accentColor, _appliedSavedPreferences, _applyingListShortcut, _applyJournalDeletedUiState, _applyListContinuation, _applySavedPreferencesIfReady, _bodyFocusNode, bodyPreview (+130 more)

### Community 4 - "Features Shell"
Cohesion: 0.02
Nodes (121): allowOverflow, allowVerticalOverflow, axisMaxY, axisMinY, belowPath, bottom, bottomAxisHeight, bucket (+113 more)

### Community 5 - "Calendar Features"
Cohesion: 0.02
Nodes (94): accentColor, allDayAreaTop, allDayShelfHeight, borderedClipRects, borderedDayColumnRect, borderedRects, borderRadius, build (+86 more)

### Community 6 - "Sync Remote"
Cohesion: 0.02
Nodes (92): CharacterOpRegistry get, _activeDebouncers, _activelyEditedDocuments, addPendingTextMergeListener, applyPendingJournalEntryTextMerge, applyPendingTodoTaskNotesMerge, cancelDocument, cancelPending (+84 more)

### Community 7 - "Todo Features"
Cohesion: 0.02
Nodes (87): _applyPendingNotesMerge, _beginEditingSession, build, _buildHeader, _clearDueDate, _close, color, _controller (+79 more)

### Community 8 - "Repositories Domain"
Cohesion: 0.02
Nodes (105): appendOperation, appendOperationsBatch, authStateChanges, claimCalendarLock, claimWeatherFetchLock, countEntriesByJournal, currentUserId, deleteAllEntries (+97 more)

### Community 9 - "Calendar Grid"
Cohesion: 0.03
Nodes (73): calendar_day_entries.dart, calendar_day_grid.dart, calendar_todo_markers.dart, calendar_week_timeline.dart, CalendarEntryMenuBuilder, accentColor, allDay, base (+65 more)

### Community 10 - "App Dev"
Cohesion: 0.05
Nodes (42): journalDebugLoggerProvider, pendingStatEntriesProvider, subscriptionsProvider, GeometricWaveShape, Subscription, build, StatisticsActionFab, _CacheStatusItemList (+34 more)

### Community 11 - "Todo Features"
Cohesion: 0.02
Nodes (105): AsyncValue, CurvedAnimation, _activeInList, animateIn, _animController, _applyListMoveOptimistic, _applyPersistedSortBatchToUi, _applySortBatchOptimistic (+97 more)

### Community 12 - "Models Domain"
Cohesion: 0.02
Nodes (102): accentColor, alertOnPeriodicPrompts, alertTimeHour, birthYear, calendarNavigateLeftKey, calendarNavigateRightKey, colorPalette, copyWith (+94 more)

### Community 13 - "Calendar Markers"
Cohesion: 0.03
Nodes (69): build, buildCalendarTodoMarkers, CalendarDayTodoBar, CalendarDayTodoIcons, CalendarDayTodoTile, calendarMaxTodoIconsPerDay, calendarMorphTodoIconProgress, calendarPackWeekAllDayShelf (+61 more)

### Community 14 - "Dev Remote"
Cohesion: 0.03
Nodes (63): _addInstantDiff, allMatched, _charMerger, compareAllJournalEntries, compareAllTodoLists, comparedAt, _compareJournalEntry, compareTodoList (+55 more)

### Community 15 - "Repositories Data"
Cohesion: 0.02
Nodes (96): countEntriesByJournal, _db, deleteAllEntries, deleteAllEvents, deleteAllJournals, deleteConflict, deleteConflictsForDocument, getAllEntries (+88 more)

### Community 16 - "Sync Weather"
Cohesion: 0.04
Nodes (63): _, @DriftDatabase, fakes/fake_weather_api_client.dart, SyncEngine, AppDatabase, InMemorySyncRepository, DriftSettingsRepository, SettingsRepository (+55 more)

### Community 17 - "Todo Task"
Cohesion: 0.03
Nodes (59): activeTasks, activeTopLevelTasks, applyDueDateChange, applyNewUndatedTask, applyNormalizeUnstarredIfNeeded, applyReorder, _applyStar, applyStarToggle (+51 more)

### Community 18 - "App Providers"
Cohesion: 0.03
Nodes (82): ChangeNotifier, AuthNotifier, all, allJournalEntriesScope, buildCalendarTodoMarkers, calendarEventsProvider, calendarTodoMarkersProvider, controller (+74 more)

### Community 19 - "Analytics Features"
Cohesion: 0.01
Nodes (188): await, DateTime get, Key? dateKey,
  double, accent, _addOption, all, analytics, anchor (+180 more)

### Community 20 - "Merger Services"
Cohesion: 0.05
Nodes (37): build, createState, dispose, initialColor, initState, _nameController, palette, _selectedColor (+29 more)

### Community 21 - "Search Features"
Cohesion: 0.06
Nodes (33): _accentColor, _bodyController, _bodyFocusNode, _coordinator, createState, _deletedIds, dispose, entry (+25 more)

### Community 22 - "Todo Features"
Cohesion: 0.07
Nodes (44): dataImportServiceProvider, journalAllEntryIdsProvider, journalEntryCacheInvalidatorProvider, journalEntryCountsProvider, journalListEntriesProvider, journalRepositoryProvider, journalsProvider, journalWriteCoordinatorProvider (+36 more)

### Community 23 - "Text Widgets"
Cohesion: 0.04
Nodes (53): accentColor, _applyHighlightText, build, contentPadding, controller, createState, cursorColor, decoration (+45 more)

### Community 24 - "Windows Runner"
Cohesion: 0.16
Nodes (15): Point, Size, wchar_t, wstring, Scale(), Create, Destroy, GetHandle (+7 more)

### Community 25 - "Widgets Color"
Cohesion: 0.04
Nodes (47): boxH, boxW, cappedMaxHeight, cappedMaxWidth, cell, clipPartialNextRow, ColorPaletteGrid, _ColorSwatch (+39 more)

### Community 26 - "Widgets Rounded"
Cohesion: 0.04
Nodes (46): AddListDropdownValue, addListLabel, _addListSentinel, build, closedTrailing, createState, didChangeDependencies, displayLabel (+38 more)

### Community 27 - "Widgets Contextual"
Cohesion: 0.05
Nodes (42): CapturedThemes, Color get, accentColor, barrierColor, barrierDismissible, barrierLabel, _borderWidth, build (+34 more)

### Community 28 - "Windows Ephemeral"
Cohesion: 0.02
Nodes (86): _advance, _advancePetal, approachLogDt, baseOpacity, build, burstAt, center, _clock (+78 more)

### Community 29 - "Time Widgets"
Cohesion: 0.05
Nodes (44): FixedExtentScrollController, FixedExtentScrollPhysics, _amPmController, applyTo, build, _buildWheel, createBallisticSimulation, createState (+36 more)

### Community 30 - "Calendar Features"
Cohesion: 0.06
Nodes (31): assigned, _buildClusters, CalendarOverlapSlot, clusterColumns, clusters, column, columnEnds, dayEnd (+23 more)

### Community 31 - "Widgets Datetime"
Cohesion: 0.05
Nodes (42): ColorScheme, build, _ClockDial, _ClockTimeMode, _ClockTimePicker, _ClockTimePickerState, colorScheme, createState (+34 more)

### Community 32 - "Domain Services"
Cohesion: 0.05
Nodes (42): ForecastPeriod?, 0, bucketHours, buckets, buildDayForecastChartSeries, chartBucketHour, chartBucketRangeCenteredOn, currentTimeChartHour (+34 more)

### Community 33 - "Windows Cpp"
Cohesion: 0.06
Nodes (43): EncodedType, ByteBufferStreamReader, bytes_, location_, size_, ByteBufferStreamWriter, bytes_, ByteStreamReader (+35 more)

### Community 34 - "Shell Features"
Cohesion: 0.05
Nodes (39): cachedCurrentWeatherProvider, isExpired, purgeEligibleAfter, SoftDeletePolicy, accent, branchIndex, build, child (+31 more)

### Community 35 - "Import Export"
Cohesion: 0.08
Nodes (24): class MockDocumentReference extends, Exception, package:fake_cloud_firestore/fake_cloud_firestore.dart, package:voyager/data/remote/firestore_sync_repository.dart, package:voyager/features/settings/services/data_export_service.dart, package:voyager/features/settings/services/data_import_service.dart, ../test/fakes/fake_weather_api_client.dart, authStateChanges (+16 more)

### Community 36 - "Todo List"
Cohesion: 0.11
Nodes (18): _asOf, _AssetModal, _AssetModalState, _canSave, _colorValue, createState, _datePopoverOpen, dispose (+10 more)

### Community 37 - "Sync Firestore"
Cohesion: 0.05
Nodes (39): body, CrdtTextFields, _dateToFirestore, _dateToFirestoreRequired, firestoreDocumentIdForLocal, fromJournalPayload, fromTodoPayload, id (+31 more)

### Community 38 - "Windows Ephemeral"
Cohesion: 0.10
Nodes (20): allCalendars, assigner, choice, color, controller, created, defaultColor, deleteCalendarList (+12 more)

### Community 39 - "Sync Engine"
Cohesion: 0.05
Nodes (43): BackgroundSyncOrchestrator, backoffMultiplier, _calendarRepository, cancelScheduledDocumentSync, _crdtResolver, _debouncer, _debouncerFor, _delayForAttempt (+35 more)

### Community 40 - "Calendar Event"
Cohesion: 0.05
Nodes (37): build, _buildPayload, _calendarFlagColor, _calendarId, calendars, _closingAfterSave, _colorValue, createState (+29 more)

### Community 41 - "Sync Conflict"
Cohesion: 0.09
Nodes (25): syncConflictsProvider, build, _confirmPermanentRemoteDelete, conflict, _ConflictDocumentIds, createState, _DiffColumn, dispose (+17 more)

### Community 42 - "Calendar Features"
Cohesion: 0.05
Nodes (43): _AnalyticsToolbar, _EmptyTrackersCard, _GridView, _HeatmapGroupDivider, _HeatmapMonthTile, _MacroStatsRow, _SectionLabel, _StatChip (+35 more)

### Community 43 - "Features Dev"
Cohesion: 0.07
Nodes (38): authNotifierProvider, backgroundSyncOrchestratorProvider, databaseProvider, lazyLoadProvider, liveSyncProvider, quoteBankProvider, quotesLoadedProvider, shellDataWarmupProvider (+30 more)

### Community 44 - "Models Domain"
Cohesion: 0.05
Nodes (36): buildDailyForecastSummaries, byDay, conditionCode, dailySummaries, date, day, days, description (+28 more)

### Community 45 - "Todo Features"
Cohesion: 0.18
Nodes (11): build, _captured, createState, current, dispose, _handleKeyEvent, initState, _KeyBindingDialog (+3 more)

### Community 46 - "Sync Conflict"
Cohesion: 0.05
Nodes (41): build, burst, burstFromContext, color, colors, ConfettiOverlay, _controller, copyWith (+33 more)

### Community 47 - "Time Widgets"
Cohesion: 0.05
Nodes (45): _activeIsStart, _applyDuration, _applyEndDt, _applyStartDt, _applyTimeDt, build, _canPop, createState (+37 more)

### Community 48 - "Domain Services"
Cohesion: 0.06
Nodes (35): archiveLat, archiveLon, bucketHour, bucketMap, d, day, forecastArchiveLocationMatches, forecastBucketKey (+27 more)

### Community 49 - "Dev Journal"
Cohesion: 0.06
Nodes (34): _append, applySettings, _appStartMarker, bodyDraftEntryIds, bodyFocused, bodyText, clearLog, enabled (+26 more)

### Community 50 - "Windows Ephemeral"
Cohesion: 0.06
Nodes (28): FlutterDesktopViewControllerRef, FlutterDesktopViewRef, WindowProcDelegate, FlutterViewController, controller_, ForceRedraw, HandleTopLevelWindowProc, view_id (+20 more)

### Community 51 - "Widgets Text"
Cohesion: 0.06
Nodes (33): FocusNode get, InputCounterWidgetBuilder?, accentColor, autofocus, build, buildCounter, controller, createState (+25 more)

### Community 52 - "Weather Features"
Cohesion: 0.09
Nodes (21): AppSettings, _apiKeyController, build, createState, didUpdateWidget, dispose, initState, _saveApiKey (+13 more)

### Community 53 - "Widgets Field"
Cohesion: 0.06
Nodes (34): bool?, accentColor, alignLabelToTop, autofocus, borderRadius, build, contentPadding, controller (+26 more)

### Community 54 - "Weather Forecast"
Cohesion: 0.06
Nodes (33): DayForecastChartSeries, _animation, _beginTransition, build, _controller, createState, _currentTimeHour, didUpdateWidget (+25 more)

### Community 55 - "Windows Ephemeral"
Cohesion: 0.08
Nodes (32): ConsumerWidget, double?, cacheStatusSnapshotProvider, devSettingsProvider, fpsMonitorProvider, _HeatmapGrid, _scaledMorphDuration, build (+24 more)

### Community 56 - "Windows Ephemeral"
Cohesion: 0.07
Nodes (21): GeneratedPluginRegistrant, Keep, FlutterEngine, engine_, GetRegistrarForPlugin, next_frame_callback_, owns_engine_, ProcessExternalWindowMessage (+13 more)

### Community 57 - "Widgets Notched"
Cohesion: 0.07
Nodes (28): accentColor, alignLabelToTop, borderRadius, borderWidth, build, child, color, contentPadding (+20 more)

### Community 58 - "Data Remote"
Cohesion: 0.06
Nodes (35): appendOperation, appendOperationsBatch, _calendarLock, claimCalendarLock, claimWeatherFetchLock, _collectionWatchers, _currentWeather, deleteDocument (+27 more)

### Community 59 - "Crdt Frac"
Cohesion: 0.10
Nodes (18): EditableText, build, child, EnterToSubmitScope, onSubmit, _textFieldHasFocus, display, divisions (+10 more)

### Community 60 - "Weather Features"
Cohesion: 0.07
Nodes (29): DateFormat, weatherForecastLastDayProvider, DailyForecastSummary, createState, _DailyCard, day, dayFormat, forecast (+21 more)

### Community 61 - "Journal Features"
Cohesion: 0.08
Nodes (24): allJournals, assigner, choice, color, controller, created, defaultColor, deleteJournalList (+16 more)

### Community 62 - "Sync Activity"
Cohesion: 0.06
Nodes (30): applySettings, clearTimer, collection, direction, _displayDuration, dispose, event, eventFor (+22 more)

### Community 63 - "Models Domain"
Cohesion: 0.06
Nodes (30): body, byCreated, byDate, colorValue, compareJournalEntriesNewestFirst, copyWith, customQuote, entryDate (+22 more)

### Community 64 - "Platform Keyboard"
Cohesion: 0.11
Nodes (20): authRepositoryProvider, build, createState, dispose, _emailController, _error, _formatAuthError, _googleSignIn (+12 more)

### Community 65 - "Database Users"
Cohesion: 0.09
Nodes (43): AssetsTableCompanion, AssetsTableData, AssetValuationsTableCompanion, AssetValuationsTableData, BudgetsTableCompanion, BudgetsTableData, CalendarEventsTableCompanion, CalendarEventsTableData (+35 more)

### Community 66 - "Features Shell"
Cohesion: 0.07
Nodes (28): cacheGeneration, clear, degreeGridArgb, _entries, from, generation, gradientStartHour, hashCode (+20 more)

### Community 67 - "Windows Ephemeral"
Cohesion: 0.13
Nodes (12): calendar, debug, journal, manage, search, VoyagerIcons, weatherIconData, package:flutter/widgets.dart (+4 more)

### Community 68 - "Firestore Data"
Cohesion: 0.07
Nodes (27): appendOperation, appendOperationsBatch, claimCalendarLock, claimWeatherFetchLock, _collection, deleteDocument, deleteOperationsForDocument, _doc (+19 more)

### Community 69 - "Windows Ephemeral"
Cohesion: 0.04
Nodes (56): baseR, buildLeafSprite, canvas, center, cx, cy, endRecording, gradient (+48 more)

### Community 70 - "Widgets Geometric"
Cohesion: 0.04
Nodes (57): accentColor, animates, _animating, baseColor, build, _clock, copyWith, createState (+49 more)

### Community 71 - "Text Sync"
Cohesion: 0.08
Nodes (23): addListener, advance, applyToLocalText, bufferWhileEditing, clearDocument, collection, documentId, documentKey (+15 more)

### Community 72 - "Calendar Features"
Cohesion: 0.03
Nodes (60): dart:io, Future, clearLog, _enqueue, log, _logFileName, logFilePath, _maxLogBytes (+52 more)

### Community 73 - "Domain Services"
Cohesion: 0.07
Nodes (26): allOps, CharacterOpRegistry, CharacterOpSession, clear, clientId, ensureSession, _insertOp, key (+18 more)

### Community 74 - "Widgets Keep"
Cohesion: 0.10
Nodes (25): AutomaticKeepAliveClientMixin, EdgeInsetsGeometry, IndexedWidgetBuilder, build, cacheExtent, child, children, controller (+17 more)

### Community 75 - "Domain Services"
Cohesion: 0.08
Nodes (19): EncodableValueVariant, any, decodeColorPaletteJson, encodeColorPaletteJson, formatColorHex, jsonEncode, normalizeColorValue, normalized (+11 more)

### Community 76 - "Main App"
Cohesion: 0.17
Nodes (11): CalendarDayEntry, build, CalendarDayEntryBar, date, entry, fontSize, height, onTap (+3 more)

### Community 77 - "Widgets Date"
Cohesion: 0.07
Nodes (28): accentColor, build, _canPop, createState, DateSelectorPopover, _DateSelectorPopoverState, didUpdateWidget, dispose (+20 more)

### Community 78 - "Features Shell"
Cohesion: 0.06
Nodes (32): build, _closingAfterSave, _completed, createState, didUpdateWidget, _discard, dispose, _dueDate (+24 more)

### Community 79 - "Calendar Features"
Cohesion: 0.08
Nodes (24): _, allDay, allDayEvent, available, calendarDayEntriesForDay, CalendarDayEntryKind, calendarVisibleEntryCount, clamp (+16 more)

### Community 80 - "Sync Journal"
Cohesion: 0.12
Nodes (18): remoteSyncCompareServiceProvider, syncCompareLoggerProvider, build, _compareAllTodoLists, _compareJournalEntries, _compareTodoList, _comparing, createState (+10 more)

### Community 81 - "Domain Services"
Cohesion: 0.08
Nodes (23): _deviceId, fetchForecastIfNeeded, isCacheStale, isForecastCacheStale, _lockDuration, mergeForecastLocally, _persistCache, _persistForecast (+15 more)

### Community 82 - "Shell Shortcuts"
Cohesion: 0.08
Nodes (28): Intent, _SubmitIntent, _blockingShortcuts, _BlockShellTabIntent, BlockShellTabShortcuts, build, child, createState (+20 more)

### Community 83 - "Models Domain"
Cohesion: 0.05
Nodes (41): boolValue, buildJournalEntriesTracker, buildStreakTracker, buildWordCountTracker, cadence, colorValue, copyWith, _dailySpan (+33 more)

### Community 84 - "Cloud Data"
Cohesion: 0.08
Nodes (23): A. Journal — create and propagate, Automated tests (CI / local), B. Journal — concurrent edit (CRDT stress), C. Journal — metadata, Check import/export works, especially for todo lists, and deleting journals keeps journal count consistent, CRDT / journal body checks, D. Todo — create and propagate, E. Todo — reorder and star (+15 more)

### Community 85 - "Journal Features"
Cohesion: 0.07
Nodes (29): FocusNode, accentColor, build, _canPop, createState, _currentDateTime, DateTimeSelectorPopover, _DateTimeSelectorPopoverState (+21 more)

### Community 86 - "Widgets Desktop"
Cohesion: 0.09
Nodes (22): build, buttonHeight, buttonRadius, buttonWidth, createState, DesktopWindowTitleBar, _DesktopWindowTitleBarState, dispose (+14 more)

### Community 87 - "Domain Calendar"
Cohesion: 0.08
Nodes (25): a, _areConsecutiveCalendarDays, b, calendarEventBarEndsOnDay, calendarEventBarsBridge, calendarEventBarStartsOnDay, calendarEventOccursOnDay, calendarEventOccursOnDayNormalized (+17 more)

### Community 88 - "Windows Ephemeral"
Cohesion: 0.10
Nodes (20): _buildEditor, createState, date, didUpdateWidget, dispose, _formatDate, initState, _intController (+12 more)

### Community 89 - "Client Data"
Cohesion: 0.03
Nodes (67): anchor, _anim, borderRadius, build, _buildRootMenu, _buildSubmenu, capturedThemes, child (+59 more)

### Community 90 - "Dev Cache"
Cohesion: 0.09
Nodes (21): double get, attempted, attemptedFraction, attemptedPercent, CacheItemState, cacheStateColor, cacheStateLabel, cacheStatusFromAsync (+13 more)

### Community 91 - "Features Shell"
Cohesion: 0.09
Nodes (21): EdgeInsets, belowCurvePath, curvedLinePath, hourAtPixelX, matchesScale, maxX, maxY, minX (+13 more)

### Community 92 - "Functions Forecast"
Cohesion: 0.13
Nodes (15): chartBucketHour(), firestore_1, forecastBucketKey(), formatDateKey(), localPartsFromUtc(), localTodayKey(), mergeForecastArchive(), parseBucketDateKey() (+7 more)

### Community 93 - "Features Page"
Cohesion: 0.09
Nodes (24): weatherForecastProvider, _advance, build, createState, dispose, _done, _frame, initState (+16 more)

### Community 94 - "Dev Features"
Cohesion: 0.11
Nodes (24): deviceIdProvider, syncActivityProvider, build, _confirmDeleteAllEntries, _confirmResetJournals, DevPage, devShowTimeSelectorHitboxesProvider, devSlowHeatmapPopoverAnimationProvider (+16 more)

### Community 95 - "Theme Menu"
Cohesion: 0.11
Nodes (18): dropdownMenuTheme, _itemHighlightRadius, itemPadding, menuBorderRadius, menuColor, menuColorOf, menuStyle, menuTheme (+10 more)

### Community 96 - "Registrar Windows"
Cohesion: 0.04
Nodes (50): _addMonths, allocatedAt, amountCents, anchorDueDate, annualCents, annualCentsFor, asOf, assetId (+42 more)

### Community 97 - "Functions Src"
Cohesion: 0.16
Nodes (17): ArchiveDoc, archiveLocationMatches(), chartBucketHour(), clearForecastArchive(), forecastBucketKey(), ForecastPeriod, formatDateKey(), localPartsFromUtc() (+9 more)

### Community 98 - "Dev Settings"
Cohesion: 0.10
Nodes (20): applySettings, devForceConflictUi, loadFromSettings, _persist, setDevForceConflictUi, setShowCacheStatus, setShowCalendarInstantViewSwitch, setShowCalendarZoomPrewarm (+12 more)

### Community 99 - "Dev Todo"
Cohesion: 0.05
Nodes (39): Brightness, accentLineAmount, accentLineTarget, appBar, blurScale, brightness, _build, card (+31 more)

### Community 100 - "Binding Utils"
Cohesion: 0.10
Nodes (20): alt, binding, control, _displayToken, formatKeyBinding, join, KeyBinding, keyBindingFromKeyEvent (+12 more)

### Community 101 - "Widgets Resizable"
Cohesion: 0.10
Nodes (20): build, clampListWidth, createState, defaultListWidth, dividerWidth, _dragging, _dragStartGlobalX, editorPadding (+12 more)

### Community 102 - "Auth Data"
Cohesion: 0.10
Nodes (21): _auth, _authErrorMessage, authStateChanges, _controller, currentUserId, dispose, fallback, FirebaseAuthRepository (+13 more)

### Community 103 - "Character Domain"
Cohesion: 0.10
Nodes (19): formatTime12Hour, formatTimeOfDay12Hour, jm, local, now, accent, chars, formattedDate (+11 more)

### Community 104 - "Features Calendar"
Cohesion: 0.10
Nodes (19): CacheItemStatus get, int get, checks, _checksEqual, _checksEqualLists, detail, idle, isFullyPrewarmed (+11 more)

### Community 105 - "Features Calendar"
Cohesion: 0.07
Nodes (39): devGeometricTexturePanelOpenProvider, devGeometricWavePanelOpenProvider, devLeafGalleryPanelOpenProvider, geometricDebugRowFadeProvider, geometricTextureParamsProvider, geometricWaveParamsProvider, paperShaderProvider, petalFieldParamsProvider (+31 more)

### Community 106 - "Widgets Clamp"
Cohesion: 0.11
Nodes (18): GlobalKey, GlobalKey get, attach, ClampToTargetBounds, createRenderObject, detach, _handleScroll, _hasAttemptedScrollLookup (+10 more)

### Community 107 - "Hotkey Features"
Cohesion: 0.10
Nodes (19): Client, FirebaseAuth, _apiKey, _http, _periodFromForecastItem, refreshForecast, refreshWeather, _auth (+11 more)

### Community 108 - "Palette Color"
Cohesion: 0.09
Nodes (19): BinaryMessengerImpl, handlers_, messenger_, Send, SetMessageHandler, BinaryMessageHandler, FlutterDesktopMessengerRef, map (+11 more)

### Community 109 - "Dev Sync"
Cohesion: 0.07
Nodes (28): BaseSliderTrackShape, build, color, ColorCornerFlag, colorValue, _CornerFlagPainter, JournalBookmarkFlag, JournalTitleCornerFlag (+20 more)

### Community 110 - "Domain Repositories"
Cohesion: 0.12
Nodes (14): int? current,
  Set, assign, nextColor, palette, PaletteAssigner, paletteFromItems, pickColorFromPalette, pickPaletteColorWithRef (+6 more)

### Community 111 - "Calendar Features"
Cohesion: 0.09
Nodes (26): @visibleForTesting, build, CalendarKeyboardShortcuts, _CalendarKeyboardShortcutsState, calendarNavDeltaForEvent, calendarNavShortcutsEnabled, calendarNavShortcutsEnabledForState, child (+18 more)

### Community 112 - "Windows Ephemeral"
Cohesion: 0.17
Nodes (11): int?, budget, _BudgetRow, color, _EmptyBudgets, pace, _PacingBar, spentCents (+3 more)

### Community 113 - "Widgets Desktop"
Cohesion: 0.11
Nodes (18): Animation, AnimationController, _animDuration, _barVisible, build, child, _controller, createState (+10 more)

### Community 114 - "Widgets Dropdown"
Cohesion: 0.10
Nodes (20): FormField, FormFieldState, InputDecoration?, accentColor, build, createState, decoration, enabled (+12 more)

### Community 115 - "App Providers"
Cohesion: 0.09
Nodes (22): FragmentProgram?, FragmentShader?, baseColor, build, createState, defaults, didUpdateWidget, dispose (+14 more)

### Community 116 - "Windows Ephemeral"
Cohesion: 0.04
Nodes (46): colorPaletteProvider, build, _changeEventColor, build, created, _createJournal, createState, _deleteJournal (+38 more)

### Community 117 - "Sync Debouncer"
Cohesion: 0.10
Nodes (20): character, CharacterOperation, charOps, charOpsKey, CharOpsPayload, clientId, copyWith, deleted (+12 more)

### Community 118 - "Widgets Menu"
Cohesion: 0.12
Nodes (16): IconData? get, buildCatalogMenu, defaultCatalogMenuChild, defaultEntityManageMenuEntries, entityManageMenuEntries, forWeatherIcon, from, icon (+8 more)

### Community 119 - "Widgets Search"
Cohesion: 0.09
Nodes (22): createState, dispose, _entryIdController, _purgeEntry, _purging, build, createdId, _createdListId (+14 more)

### Community 120 - "Models Domain"
Cohesion: 0.12
Nodes (16): colorValue, completed, copyWith, dueDate, dueDateSetAt, fromJson, isSubtask, listId (+8 more)

### Community 121 - "Settings Palette"
Cohesion: 0.08
Nodes (40): _ConfettiBurst, _ConfettiBurstState, _PalettePickDialog, _PalettePickDialogState, _ContextMenuOverlay, _ContextMenuOverlayState, _CreateNameColorDialog, _CreateNameColorDialogState (+32 more)

### Community 122 - "Windows Ephemeral"
Cohesion: 0.17
Nodes (11): googleOAuthClientId, _googleOAuthClientIdRaw, googleOAuthClientSecret, _googleOAuthClientSecretRaw, googleOAuthRedirectPort, id, isGoogleOAuthClientSecretConfigured, isGoogleOAuthConfigured (+3 more)

### Community 123 - "Platform Widgets"
Cohesion: 0.07
Nodes (27): dart:collection, _current, currentFps, dispose, _fps, _history, historyLength, isRunning (+19 more)

### Community 124 - "Shell Features"
Cohesion: 0.08
Nodes (24): build, QuickJournalPopup, QuickTodoPopup, icon, index, label, order, page (+16 more)

### Community 125 - "Windows Ephemeral"
Cohesion: 0.12
Nodes (16): dart:async, _callbacks, flushAll, instance, PendingFlushRegistry, register, unregister, _activeUntil (+8 more)

### Community 126 - "Database Data"
Cohesion: 0.09
Nodes (22): @DataClassName, AssetsTable, AssetValuationsTable, BudgetsTable, CalendarEventsTable, CalendarsTable, FinanceCategoriesTable, GoalAllocationsTable (+14 more)

### Community 127 - "Menu Widgets"
Cohesion: 0.13
Nodes (12): main, ops, session, dart:math, indices, main, r, sorted (+4 more)

### Community 128 - "Sync Outbox"
Cohesion: 0.10
Nodes (23): StreamHandlerCancel, StreamHandlerListen, EventSink, EndOfStreamInternal, ErrorInternal, SuccessInternal, string, T (+15 more)

### Community 129 - "Functions Scripts"
Cohesion: 0.13
Nodes (14): dependencies, firebase-admin, firebase-functions, devDependencies, typescript, engines, node, main (+6 more)

### Community 130 - "Dev Warmup"
Cohesion: 0.05
Nodes (42): Alignment, BorderRadius, alignment, blurSigma, borderColor, borderOpacity, borderRadius, borderWidth (+34 more)

### Community 131 - "Todo Task"
Cohesion: 0.12
Nodes (17): trackerRepositoryProvider, _createTracker, _delete, _deleteTracker, _editTracker, _handleTap, _HeatmapBucket, _HeatmapBucketState (+9 more)

### Community 132 - "Gradient Widgets"
Cohesion: 0.09
Nodes (20): nanoseconds, FlutterDesktopEngineRef, FlutterDesktopPluginRegistrarRef, function, HWND, LPARAM, LRESULT, optional (+12 more)

### Community 133 - "Models Analytics"
Cohesion: 0.18
Nodes (10): WeatherForecast, WeatherSnapshot, forecastCalls, forecastResult, geocodeCalls, geocodeResult, refreshCalls, refreshForecast (+2 more)

### Community 134 - "Models Domain"
Cohesion: 0.17
Nodes (11): collection, detectedAt, documentId, id, localPayloadJson, localText, localTitle, remotePayloadJson (+3 more)

### Community 135 - "Windows Ephemeral"
Cohesion: 0.09
Nodes (24): FlutterDesktopMessage, TextureVariant, BinaryMessengerImpl::BinaryMessengerImpl(), BinaryMessengerImpl::Send(), BinaryMessengerImpl::SetMessageHandler(), BinaryMessageHandler, BinaryReply, FlutterDesktopMessengerRef (+16 more)

### Community 136 - "Platform Domain"
Cohesion: 0.25
Nodes (11): OnCreate, RECT, HWND, Win32Window, child_content_, GetClientArea, OnCreate, quit_on_close_ (+3 more)

### Community 137 - "Domain Services"
Cohesion: 0.17
Nodes (11): _, GoRouter, auth, child, _preloadedShellPaths, settingsRepo, package:voyager/core/widgets/desktop_window_frame.dart, package:voyager/features/auth/login_page.dart (+3 more)

### Community 138 - "Functions Tsconfig"
Cohesion: 0.15
Nodes (12): compileOnSave, compilerOptions, esModuleInterop, module, noImplicitReturns, noUnusedLocals, outDir, skipLibCheck (+4 more)

### Community 139 - "Auth Firebase"
Cohesion: 0.15
Nodes (12): contains, embedded, firebaseAuthErrorMessage, invalidCredentialHints, _isGenericInternalMessage, lower, _messageFromEmbeddedDetails, normalizedCode (+4 more)

### Community 140 - "Color Widgets"
Cohesion: 0.05
Nodes (43): DateTime? now,
  bool, amountCents, assetCents, best, BreakdownSlice, byAsset, cashCents, cashFlowGranularityLabel (+35 more)

### Community 141 - "FlutterView"
Cohesion: 0.22
Nodes (8): MethodCallHandler, string, T, unique_ptr, MethodChannel, codec_, messenger_, name_

### Community 142 - "Windows Ephemeral"
Cohesion: 0.10
Nodes (18): bool get, DateTime, _auth, dispose, _isAuthenticated, _subscription, copyWithDeleted, createdAt (+10 more)

### Community 143 - "Windows Ephemeral"
Cohesion: 0.10
Nodes (19): MessageHandler, BasicMessageChannel, codec_, messenger_, name_, BinaryReply, string, T (+11 more)

### Community 144 - "Widgets Selector"
Cohesion: 0.33
Nodes (5): _entry, entryDate, id, main, now

### Community 145 - "Constants Google"
Cohesion: 0.25
Nodes (7): build, children, currentIndex, ShellBranchContainer, shellBranchContainerBuilder, package:go_router/go_router.dart, ShellNavigationContainerBuilder

### Community 146 - "Features Analytics"
Cohesion: 0.14
Nodes (13): Files touched by this migration, How it works, Intentional exception: the inline todo-item rename field, Known limitations / non-goals, `LabeledTextField` (`lib/core/widgets/labeled_text_field.dart`), `TagHighlightedTextField` (`lib/core/widgets/tag_highlighted_text_field.dart`), Text box widget: `NotchedFieldBorder`, The notch math (+5 more)

### Community 147 - "Features Settings"
Cohesion: 0.20
Nodes (12): calendarRepositoryProvider, calendarsProvider, changeCalendarListColor, createCalendarList, renameCalendarList, _createCalendarFromDropdown, _defaultColorForCalendar, _deleteEvent (+4 more)

### Community 148 - "Shell App"
Cohesion: 0.08
Nodes (21): CopyBufferCallback, FlutterDesktopGpuSurfaceDescriptor, FlutterDesktopGpuSurfaceType, FlutterDesktopPixelBuffer, ObtainDescriptorCallback, function, TextureRegistrarImpl::UnregisterTexture(), GpuSurfaceTexture (+13 more)

### Community 149 - "Widgets Features"
Cohesion: 0.12
Nodes (17): CustomPainter, _ConfettiPainter, _ClockDialPainter, GeometricTexturePainter, _GlassBorderPainter, _NotchedBorderPainter, PaperTexturePainter, _PetalPainter (+9 more)

### Community 150 - "Constants Journal"
Cohesion: 0.18
Nodes (10): firestoreId, firestoreJournalId, journalDocumentIdForFirestore, journalDocumentIdFromFirestore, journalReferenceIdForFirestore, journalReferenceIdFromFirestore, legacyJournalFirestoreId, legacyJournalId (+2 more)

### Community 151 - "Widgets Confirm"
Cohesion: 0.18
Nodes (10): cancelLabel, confirmed, confirmLabel, deleteAllLabel, DeleteContainerChoice, moveLabel, result, showConfirmDialog (+2 more)

### Community 152 - "Weather Fakes"
Cohesion: 0.22
Nodes (10): todoSortDebugLoggerProvider, build, DevTodoSortDebugSection, _showLogDialog, _applyOptimisticActiveOrder, _clearTaskDueDate, _reorderActiveTasks, _setTaskDueDate (+2 more)

### Community 153 - "Features Settings"
Cohesion: 0.11
Nodes (18): _addColor, build, createState, dispose, _hexController, _hexError, _hexFocusNode, palette (+10 more)

### Community 154 - "warmup_tracker.dart"
Cohesion: 0.22
Nodes (9): @immutable, CacheItemStatus, CacheStatusSnapshot, ConfettiEffect, PaperTextureParams, PetalFieldParams, MonthZoomPrewarmCheck, MonthZoomPrewarmStatus (+1 more)

### Community 155 - "Data Remote"
Cohesion: 0.11
Nodes (17): Color, IconData, currentWeatherProvider, accentColor, build, child, dense, ellipsize (+9 more)

### Community 156 - "Delta Scramble"
Cohesion: 0.20
Nodes (8): local, main, newRemote, newRemote2, oldRemote, oldRemote2, main, lib/core/sync/text_delta_injector.dart

### Community 157 - "Dev Sync"
Cohesion: 0.12
Nodes (16): 1. Shader asset, 2. Loading, 3. Tunable parameters, 4. Widget → painter → GPU, 5. App-level composition, 6. UI that interacts with the background, 7. GPU warmup (avoid first-frame jank), 8. Failure modes (+8 more)

### Community 158 - "Windows Runner"
Cohesion: 0.15
Nodes (10): _In_, _In_opt_, optional, wWinMain(), string, vector, wchar_t, CreateAndAttachConsole() (+2 more)

### Community 159 - "Domain Services"
Cohesion: 0.20
Nodes (9): after, before, between, _digits, first, FractionalIndex, _keyAfter, _keyBefore (+1 more)

### Community 160 - "Shell Features"
Cohesion: 0.20
Nodes (9): analyticsList, devList, journalEntryList, journalEntryListAll, journalPreview, searchResults, settingsList, ShellPageStorageKeys (+1 more)

### Community 161 - "Windows Ephemeral"
Cohesion: 0.11
Nodes (22): EventChannel, codec_, messenger_, name_, string, T, unique_ptr, string (+14 more)

### Community 162 - "Sync Text"
Cohesion: 0.22
Nodes (8): _, adjustedSelection, _commonPrefixLength, _commonSuffixLength, _fallbackMerge, injectRemoteDelta, _insertAt, TextDeltaInjector

### Community 163 - "Sync Pending"
Cohesion: 0.07
Nodes (41): assetsProvider, assetValuationsProvider, budgetsProvider, financeCategoriesProvider, tagColorsProvider, transactionsProvider, CashFlowGranularity, abs (+33 more)

### Community 164 - "App Providers"
Cohesion: 0.05
Nodes (40): financeRepositoryProvider, _save, _delete, _save, _BudgetModal, _BudgetModalState, build, _canSave (+32 more)

### Community 165 - "trackerRepositoryProvider"
Cohesion: 0.15
Nodes (12): main, ops, pos, s, allOps, main, merger, opsA (+4 more)

### Community 166 - "Theme Spacing"
Cohesion: 0.11
Nodes (17): LeafDesign, _angles, color, _count, design, _LeafDesignRow, _LeafPreview, _LeafPreviewPainter (+9 more)

### Community 167 - "quick_popups.dart"
Cohesion: 0.14
Nodes (12): AccessibilityMode, GpuPreference, UIThreadPolicy, DartProject, aot_library_path_, assets_path_, dart_entrypoint_, dart_entrypoint_arguments_ (+4 more)

### Community 168 - "Windows Ephemeral"
Cohesion: 0.06
Nodes (60): ConsumerState, ConsumerStatefulWidget, allTodoTasksProvider, remoteSyncServiceProvider, todoListsProvider, todoRepositoryProvider, todoTasksProvider, ColorPickerField (+52 more)

### Community 169 - "@immutable"
Cohesion: 0.12
Nodes (16): best, colorFor, cursor, highlightedStyle, index, keywords, _keywordSpans, lower (+8 more)

### Community 170 - "ByteBufferStreamReader"
Cohesion: 0.07
Nodes (24): BinaryMessenger, Send, SetMessageHandler, GetRegistrar(), FlutterDesktopPluginRegistrarRef, set, T, unique_ptr (+16 more)

### Community 171 - "Widgets Enter"
Cohesion: 0.05
Nodes (37): buildDayGroups, color, colorValue, cumulative, day, _DayHeader, _EmptyLedger, _FinanceView (+29 more)

### Community 172 - "Gpusurfacetexture Windows"
Cohesion: 0.12
Nodes (17): class, geometricShaderProvider, createState, DevOutOfSyncPurgeSection, _DevOutOfSyncPurgeSectionState, _lastResults, _purgeAll, _purging (+9 more)

### Community 173 - "@immutable"
Cohesion: 0.22
Nodes (8): a, b, betweens, indices, main, prev, sortedBetweens, sortedIndices

### Community 175 - "Shell Features"
Cohesion: 0.20
Nodes (9): 1. Database Schema (SQLite / CRDT Ready), 2. State Management & Logic (Riverpod), 3. UI / Widget Architecture, A. The 1-Year Rolling Maximum (Independent Integers), A. The Journal Page Entry Flow, B. Analytics Page: Default View (The 4 Dashboards), B. Enum Dynamic Color Generation, C. Analytics Page: Calendar View (+1 more)

### Community 176 - "Messenger Windows"
Cohesion: 0.17
Nodes (12): vector, ReplyManager::SendResponseData(), EngineMethodResult, codec_, reply_manager_, BinaryReply, string, T (+4 more)

### Community 177 - "dart:math"
Cohesion: 0.08
Nodes (21): StatisticTracker, build, HeatmapCalendar, month, tracker, values, package:voyager/core/theme/voyager_theme.dart, package:voyager/domain/services/analytics_service.dart (+13 more)

### Community 178 - "Constants Hotkey"
Cohesion: 0.29
Nodes (6): defaultCalendarNavigateLeftKey, defaultCalendarNavigateRightKey, defaultJournalHotkey, defaultTodoHotkey, legacyJournalHotkey, legacyTodoHotkey

### Community 179 - "Sync Firestore"
Cohesion: 0.15
Nodes (11): FirestoreCollections, journalEntries, journals, syncOperations, todoLists, todoTasks, AppFonts, applyTo (+3 more)

### Community 180 - "@visibleForTesting"
Cohesion: 0.13
Nodes (16): goalAllocationsProvider, savingsGoalsProvider, SavingsGoal, build, allocatedCents, build, _EmptyGoals, FinanceGoalsView (+8 more)

### Community 181 - "MethodCall"
Cohesion: 0.09
Nodes (22): DocumentReference, FirebaseFirestore, FirebaseFunctions?, _call, _firestore, FirestoreWeatherReader, _functions, getCurrentWeather (+14 more)

### Community 182 - "Anagram List1"
Cohesion: 0.33
Nodes (5): list1, list2, main, s1, s2

### Community 183 - "dart:math"
Cohesion: 0.38
Nodes (10): HWND, LPARAM, LRESULT, UINT, WPARAM, EnableFullDpiSupportIfAvailable(), GetThisFromHandle, MessageHandler (+2 more)

### Community 184 - "Calendar Features"
Cohesion: 0.33
Nodes (6): InheritedWidget, _ContextualPopoverAccentScope, CalendarEventTapScope, CalendarMorphYearDotsOpacity, _MorphProgress, _WeekMorphProgress

### Community 185 - "debouncer.dart"
Cohesion: 0.10
Nodes (20): _amountController, build, _canSave, _colorValue, createState, _datePopoverOpen, _delete, dispose (+12 more)

### Community 186 - "Widgets Rounded"
Cohesion: 0.40
Nodes (4): dart:ui, Error, main, main

### Community 187 - "Chart Weather"
Cohesion: 0.25
Nodes (7): firestoreId, legacyTodoListFirestoreId, legacyTodoListId, localId, todoListDocumentIdForFirestore, todoListDocumentIdFromFirestore, return

### Community 188 - "Android Gradlew"
Cohesion: 0.60
Nodes (3): gradlew script, die(), warn()

### Community 189 - "Deinterleave Evens"
Cohesion: 0.40
Nodes (4): evens, main, odds, s

### Community 190 - "WeatherApiClient"
Cohesion: 0.14
Nodes (21): allJournalEntriesProvider, analyticsServiceProvider, periodicPromptServiceProvider, trackersProvider, trackerValuesProvider, AnalyticsPage, build, _calendarViewMonthlyBaseYearProvider (+13 more)

### Community 191 - "Dev Flags"
Cohesion: 0.33
Nodes (5): DevFlags, showTimeSelectorHitboxes, slowHeatmapPopoverAnimation, verboseSync, static bool

### Community 192 - "Utils Ids"
Cohesion: 0.25
Nodes (7): day, month, newId, trackerValueId, utcNow, _uuid, package:uuid/uuid.dart

### Community 193 - "Utils Journal"
Cohesion: 0.40
Nodes (4): colorForTag, extractTags, journalTagPattern, matches

### Community 194 - "Google Desktop"
Cohesion: 0.06
Nodes (36): DateTimeRange, _AllocateModal, _AllocateModalState, _amountController, _canSave, createState, _date, _datePopoverOpen (+28 more)

### Community 195 - "Constants Todo"
Cohesion: 0.50
Nodes (3): normalizeUnstarredSortOrder, starredSortOrderMax, unstarredSortOrderBase

### Community 196 - "PLAN.md"
Cohesion: 0.22
Nodes (8): AGENTS, Features, Frontend UI, Future Additions, High Level Overview, Low Level Design, Miscellaneous, Theme

### Community 198 - "Calendar Customclipper"
Cohesion: 0.67
Nodes (3): CustomClipper, _WeekTimedViewportClipper, Path

### Community 199 - "High-Level Design: Voyager Weekly Calendar View"
Cohesion: 0.22
Nodes (8): High-Level Design: Voyager Weekly Calendar View, I. System Overview, II. Visual Layout & Z-Index Stack, III. The Overlap Engine (Column Splitting), IV. Data Entity Specifications, The Zero-Duration Task Layout Logic, Todo Task Logic, V. User Interaction Flow

### Community 200 - "IMPORT_EXPORT.md"
Cohesion: 0.25
Nodes (7): 1. The Database Layer (Drift), 2. The Export Pipeline (`DataExportService`), 3. The Import Pipeline (`DataImportService`), 4. The Trickle Sync Engine (`OutboxSyncWorker`), 5. Testing Strategy, Integration Tests (Slow, End-to-End), Unit Tests (Fast, Isolated)

### Community 201 - "Constants App"
Cohesion: 0.50
Nodes (3): accentBorderAlpha, softDeleteRetentionDays, syncDebounceSeconds

### Community 202 - "todoSortDebugLoggerProvider"
Cohesion: 0.11
Nodes (17): body, build, floatingActionButton, PlatformAdaptiveScaffold, build, child, color, GeometricProgressRing (+9 more)

### Community 203 - "Color"
Cohesion: 0.13
Nodes (15): BoxConstraints? constraints,
  EdgeInsetsGeometry, VoyagerMenuItemPosition, accent, build, checkColor, createState, menuPadding, menuStyle (+7 more)

### Community 208 - "SingleChildLayoutDelegate"
Cohesion: 0.25
Nodes (7): key, keyToken, modifiers, parseHotKey, parts, _physicalKey, package:hotkey_manager/hotkey_manager.dart

### Community 212 - "ADR 001: Local-First Data Model"
Cohesion: 0.33
Nodes (5): ADR 001: Local-First Data Model, Consequences, Context, Decision, Status

### Community 213 - "ADR 002: Sync Protocol Boundaries"
Cohesion: 0.33
Nodes (5): ADR 002: Sync Protocol Boundaries, Consequences, Context, Decision, Status

### Community 214 - "ADR 003: Module Structure"
Cohesion: 0.33
Nodes (5): ADR 003: Module Structure, Consequences, Context, Decision, Status

### Community 215 - "List"
Cohesion: 0.40
Nodes (6): _AddListMenuItem, _AddListMenuItemState, _RoundedDropdownMenuItem, _RoundedDropdownMenuItemState, Object?, PopupMenuEntry

### Community 216 - "_VoyagerAppState"
Cohesion: 0.12
Nodes (15): axisReservedSize, chars, compactNumberLabel, intervals, magnitude, magnitudeSuffixes, mantissa, mantissas (+7 more)

### Community 217 - "Voyager"
Cohesion: 0.33
Nodes (5): Architecture, Features, Setup, Testing, Voyager

### Community 218 - "_AddListMenuItem"
Cohesion: 0.06
Nodes (34): Delay, _authRepo, _db, _firestore, initialize, _instance, _isDraining, isInitialized (+26 more)

### Community 219 - "weatherForecastProvider"
Cohesion: 0.13
Nodes (14): 1. Core Purpose & Overview, 2. Database Models & Schema Integration, 3. Detailed Component Breakdown & Edge Cases, 4. Popover & Morphing Editor, 5. Summary of Key Technical Edge Cases, A. Hover Tooltip Overlay, A. The Grid View & Sparkline Stack, B. Morph Route Transition (+6 more)

### Community 220 - "TIME_SELECTOR.md"
Cohesion: 0.40
Nodes (4): 1. The Container (The Anchored Popover), 2. The Internal Layout (Start Time Popup), 3. The Internal Layout (End Time / Duration Popup), The Execution Summary

### Community 221 - "MessageHandler"
Cohesion: 0.14
Nodes (13): HWND, LPARAM, LRESULT, UINT, WPARAM, FlutterWindow, flutter_controller_, FlutterWindow::FlutterWindow() (+5 more)

### Community 222 - "weather_chart_curve_test.dart"
Cohesion: 0.05
Nodes (40): isDue, longestJournalStreak, missedPeriods, _nextPeriod, PeriodicPromptService, periodStartFor, _amountController, build (+32 more)

### Community 227 - "MorphDayEventStack"
Cohesion: 0.12
Nodes (16): CalendarEvent, calendarId, colorValue, copyWith, end, EventRecurrence, EventSource, externalId (+8 more)

### Community 228 - "package:voyager/domain/services/weather_forecast_chart.dart"
Cohesion: 0.40
Nodes (4): decoration, restingColor, selectedColor, VoyagerListItemSurface

### Community 230 - "_HeatmapDayCell"
Cohesion: 0.15
Nodes (13): TrackerValue, Calendar, Asset, AssetValuation, Budget, FinanceCategory, FinancialTransaction, GoalAllocation (+5 more)

### Community 231 - "_MorphAnimationLayer"
Cohesion: 0.22
Nodes (10): HotKey?, createHotkeyService, dispose, HotkeyService, _journalKey, NoOpHotkeyService, register, _todoKey (+2 more)

### Community 233 - "MethodResultFunctions"
Cohesion: 0.14
Nodes (15): ResultHandlerError, ResultHandlerNotImplemented, ResultHandlerSuccess, string, T, MethodResultFunctions, on_error_, on_not_implemented_ (+7 more)

### Community 234 - "package:voyager/core/sync/text_delta_injector.dart"
Cohesion: 0.15
Nodes (11): CloudFunctionWeatherClient, DevOpenWeatherClient, refreshForecast, refreshWeather, WeatherApiClient, package:voyager/domain/models/weather_models.dart, package:voyager/domain/services/weather_forecast_merge.dart, FakeWeatherApiClient (+3 more)

### Community 235 - "addedAt"
Cohesion: 0.40
Nodes (4): package:voyager/core/constants/google_auth_config.dart, package:voyager/data/remote/desktop_google_oauth.dart, StateError, main

### Community 236 - "sync_conflict.dart"
Cohesion: 0.05
Nodes (45): id, OutOfSyncJournalEntryPurge, OutOfSyncJournalEntryTarget, purgeAll, purgeOne, searchHint, targets, title (+37 more)

### Community 237 - "FlutterViewController::HandleTopLevelWindowProc"
Cohesion: 0.17
Nodes (10): FlutterViewId, HWND, LPARAM, LRESULT, optional, UINT, WPARAM, FlutterViewController::FlutterViewController() (+2 more)

### Community 238 - "alertTimeHour"
Cohesion: 0.17
Nodes (11): Duration, Duration get, cancel, DebouncedCallback, debounceDelay, Debouncer, delay, dispose (+3 more)

### Community 239 - "todoSortDebugLoggerProvider"
Cohesion: 0.50
Nodes (3): package:voyager/domain/services/weather_forecast_chart.dart, main, _period

### Community 240 - "MethodResult"
Cohesion: 0.67
Nodes (3): BorderedRoundedDropdown, RoundedDropdown, _RoundedDropdownState

### Community 241 - "voyager_spacing.dart"
Cohesion: 0.22
Nodes (8): compactListVerticalDensity, lg, md, sm, VoyagerSpacing, xl, xs, xxs

### Community 242 - "cadence"
Cohesion: 0.20
Nodes (9): AppThemeMode, BillingPeriod, CalendarViewMode, HeatmapMode, StartupPageMode, TrackerCadence, TrackerStyle, TrackerType (+1 more)

### Community 243 - "geometric_texture_settings.dart"
Cohesion: 0.08
Nodes (27): AnalyticsService, booleanTrueCount, completedTasks, countWords, heatmapIntensity, integerSeries, interpolateConsecutive, rollingMax (+19 more)

### Community 244 - "GpuSurfaceTexture"
Cohesion: 0.29
Nodes (6): nextQuote, QuoteBank, _quotes, _random, _used, List

### Community 245 - "calendarNavigateRightKey"
Cohesion: 0.40
Nodes (5): GeometricTextureParamsNotifier, GeometricWaveParamsNotifier, GeometricTextureParams, GeometricWaveParams, StateNotifier

### Community 246 - "collection"
Cohesion: 0.50
Nodes (3): 1. The Sparklines (Consecutive Trackers), 2. The Heatmap Grid, 3. Calendar View

### Community 248 - "dart:math"
Cohesion: 0.15
Nodes (11): configureDesktopWindow, desktopWindowChromeActive, _desktopWindowConfigured, exception, installWindowsKeyboardWorkaround, _isKnownWindowsKeyboardDesync, message, previous (+3 more)

### Community 249 - "_AddListMenuItem"
Cohesion: 0.22
Nodes (9): build, child, createState, didUpdateWidget, _entry, initState, LocalOverlayWrapper, _LocalOverlayWrapperState (+1 more)

### Community 251 - "_MonthGridBox"
Cohesion: 0.20
Nodes (10): settingsRepositoryProvider, _save, _persistTagColors, _ensureDefaultJournal, _persistEntryListWidth, _persistLastViewedJournal, _save, didUpdateWidget (+2 more)

### Community 252 - "package:voyager/core/sync/text_delta_injector.dart"
Cohesion: 0.13
Nodes (14): package:voyager/domain/todo/todo_task_sorting.dart, required String id,
  bool, byId, dueDate, dueDateSetAt, id, main, now (+6 more)

### Community 264 - "geometric_texture_settings.dart"
Cohesion: 0.33
Nodes (5): appSettingsWithGeometricTextureParams, appSettingsWithGeometricWaveParams, copyWith, geometricTextureParamsFromSettings, geometricWaveParamsFromSettings

### Community 362 - "dart:ui"
Cohesion: 0.05
Nodes (45): dart:convert, CrdtDocumentResolver, _merger, _mergeSyncOperations, resolvePayload, list, loadQuotesFromAssets, raw (+37 more)

### Community 438 - "GLASS_BUTTON.md"
Cohesion: 0.20
Nodes (9): 1. Basic Icon + Text Button (Toolbar / App Header), 2. Custom Size & Recolor (Primary Action), 3. Icon-Only Glass Button, API Reference, File Locations, GlassButton, Key Features, Parameters (+1 more)

### Community 439 - "weather_chart_curve_test.dart"
Cohesion: 0.40
Nodes (4): package:voyager/features/shell/weather_chart_curve.dart, curveFor, main, spots

## Knowledge Gaps
- **5720 isolated node(s):** `s`, `ops`, `pos`, `main`, `s1` (+5715 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **193 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `any` connect `Domain Services` to `Palette Color`?**
  _High betweenness centrality (0.092) - this node is a cross-community bridge._
- **Why does `BasicMessageChannel` connect `Windows Ephemeral` to `ByteBufferStreamReader`, `Palette Color`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
- **Why does `EncodableValue` connect `Domain Services` to `Palette Color`?**
  _High betweenness centrality (0.010) - this node is a cross-community bridge._
- **What connects `s`, `ops`, `pos` to the rest of the system?**
  _5720 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Database Data` be split into smaller, more focused modules?**
  _Cohesion score 0.00516795865633075 - nodes in this community are weakly interconnected._
- **Should `Calendar Features` be split into smaller, more focused modules?**
  _Cohesion score 0.007751937984496124 - nodes in this community are weakly interconnected._
- **Should `Calendar Day` be split into smaller, more focused modules?**
  _Cohesion score 0.009009009009009009 - nodes in this community are weakly interconnected._