# Graph Report - Voyager  (2026-08-26)

## Corpus Check
- 754 files · ~994,111 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 19045 nodes · 30441 edges · 542 communities (473 shown, 69 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 296 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ca45fbab`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Database Data
- Calendar Features
- Calendar Day
- Journal Features
- Features Shell
- Calendar Features
- Sync Remote
- Todo Features
- Repositories Domain
- Calendar Grid
- App Dev
- Todo Features
- Models Domain
- Calendar Markers
- Dev Remote
- Repositories Data
- Sync Weather
- Todo Task
- App Providers
- Analytics Features
- Merger Services
- Search Features
- Todo Features
- Text Widgets
- Windows Runner
- Widgets Color
- Widgets Rounded
- Widgets Contextual
- Windows Ephemeral
- Time Widgets
- Calendar Features
- Widgets Datetime
- Domain Services
- Windows Cpp
- Shell Features
- Import Export
- Todo List
- Sync Firestore
- Windows Ephemeral
- Sync Engine
- Calendar Event
- Sync Conflict
- Calendar Features
- Features Dev
- Models Domain
- Todo Features
- Sync Conflict
- Time Widgets
- Domain Services
- Dev Journal
- Windows Ephemeral
- Widgets Text
- Weather Features
- Widgets Field
- Weather Forecast
- Windows Ephemeral
- Windows Ephemeral
- Widgets Notched
- Data Remote
- Crdt Frac
- Weather Features
- Journal Features
- Sync Activity
- Models Domain
- Platform Keyboard
- Database Users
- Features Shell
- Windows Ephemeral
- Firestore Data
- Windows Ephemeral
- Widgets Geometric
- Text Sync
- Calendar Features
- Domain Services
- Widgets Keep
- Domain Services
- Main App
- Widgets Date
- Features Shell
- Calendar Features
- Sync Journal
- Domain Services
- Shell Shortcuts
- Models Domain
- Cloud Data
- Journal Features
- Widgets Desktop
- Domain Calendar
- Windows Ephemeral
- Client Data
- Dev Cache
- Features Shell
- Functions Forecast
- Features Page
- Dev Features
- Theme Menu
- Registrar Windows
- Functions Src
- Dev Settings
- Dev Todo
- Binding Utils
- Widgets Resizable
- Auth Data
- Character Domain
- Features Calendar
- Features Calendar
- Widgets Clamp
- Hotkey Features
- Palette Color
- Dev Sync
- Domain Repositories
- Calendar Features
- Windows Ephemeral
- Widgets Desktop
- Widgets Dropdown
- App Providers
- Windows Ephemeral
- Sync Debouncer
- Widgets Menu
- Widgets Search
- Models Domain
- Settings Palette
- Windows Ephemeral
- Platform Widgets
- Shell Features
- Windows Ephemeral
- Database Data
- Menu Widgets
- Sync Outbox
- Functions Scripts
- Dev Warmup
- Todo Task
- Gradient Widgets
- Models Analytics
- Models Domain
- Windows Ephemeral
- Platform Domain
- Domain Services
- Functions Tsconfig
- Auth Firebase
- Color Widgets
- FlutterView
- Windows Ephemeral
- Windows Ephemeral
- Widgets Selector
- Constants Google
- Features Analytics
- Features Settings
- Shell App
- Widgets Features
- Constants Journal
- Widgets Confirm
- Weather Fakes
- Features Settings
- warmup_tracker.dart
- Data Remote
- Delta Scramble
- Dev Sync
- Windows Runner
- Domain Services
- Shell Features
- Windows Ephemeral
- Sync Text
- Sync Pending
- App Providers
- trackerRepositoryProvider
- Theme Spacing
- quick_popups.dart
- Windows Ephemeral
- @immutable
- ByteBufferStreamReader
- Widgets Enter
- Gpusurfacetexture Windows
- @immutable
- Constants Todo
- Shell Features
- Messenger Windows
- dart:math
- Constants Hotkey
- Sync Firestore
- @visibleForTesting
- MethodCall
- Anagram List1
- dart:math
- Calendar Features
- debouncer.dart
- Widgets Rounded
- Chart Weather
- Android Gradlew
- Deinterleave Evens
- WeatherApiClient
- Dev Flags
- Utils Ids
- Utils Journal
- Google Desktop
- Constants Todo
- PLAN.md
- Mainactivity Android
- Calendar Customclipper
- High-Level Design: Voyager Weekly Calendar View
- IMPORT_EXPORT.md
- Constants App
- todoSortDebugLoggerProvider
- Color
- Constants Default
- desktop_google_oauth_test.dart
- Domain Services
- Calendar Features
- SingleChildLayoutDelegate
- ADR 001: Local-First Data Model
- ADR 002: Sync Protocol Boundaries
- ADR 003: Module Structure
- List
- _VoyagerAppState
- Voyager
- _AddListMenuItem
- weatherForecastProvider
- TIME_SELECTOR.md
- MessageHandler
- weather_chart_curve_test.dart
- CalendarRepository
- graphify.md
- graphify.md
- FEEDBACK.md
- MorphDayEventStack
- package:voyager/domain/services/weather_forecast_chart.dart
- _CalendarMorphWarmupState
- _HeatmapDayCell
- _MorphAnimationLayer
- MethodResultFunctions
- package:voyager/core/sync/text_delta_injector.dart
- addedAt
- sync_conflict.dart
- FlutterViewController::HandleTopLevelWindowProc
- alertTimeHour
- todoSortDebugLoggerProvider
- MethodResult
- voyager_spacing.dart
- cadence
- geometric_texture_settings.dart
- GpuSurfaceTexture
- calendarNavigateRightKey
- collection
- _HeatmapWeekBlock
- dart:math
- _AddListMenuItem
- completed
- _MonthGridBox
- package:voyager/core/sync/text_delta_injector.dart
- _TrackerDialog
- VoyagerColors
- _YearGridBox
- @visibleForTesting
- dart:async
- package:voyager/core/theme/voyager_theme.dart
- _MorphAnimationLayer
- _QuickDelayedDragStartListener
- _CalendarWeekTimelineState
- fake_weather_api_client.dart
- accentColor
- addedAt
- alertOnPeriodicPrompts
- alertTimeHour
- allocatedAt
- amountCents
- anchorDueDate
- asOf
- assetId
- birthYear
- body
- boolValue
- cadence
- calendarId
- calendarNavigateLeftKey
- calendarNavigateRightKey
- collection
- collectionName
- colorPaletteJson
- colorValue
- completed
- createdAt
- customQuote
- customStartupPage
- defaultBool
- _CalendarInteractiveEventTapState
- _AnalyticsSectionState
- NotificationPopoverWarmup
- _PinnedNoteRowState
- _revealEvent
- _revealTask
- _StableViewScrollPosition
- accentColor
- addedAt
- alertOnPeriodicPrompts
- workout_overlay.dart
- allocatedAt
- amountCents
- anchorDueDate
- build
- InternalFirebaseAuthSettings
- birthDate
- birthYear
- body
- journal_write_coordinator.dart
- cadence
- calendarId
- calendarNavigateLeftKey
- InternalAuthCredentialInput
- collection
- collectionName
- study_keyboard_shortcuts.dart
- colorValue
- study_rich_text.dart
- completedAt
- _LifeTrackerPageState
- MethodChannel
- app_router.dart
- spell_check_overlay_alignment_test.dart
- BinaryMessengerImpl
- -getFIRAuthCredentialFromArgumentsappcompletion
- AuthPigeonFirebaseApp
- shell_page_transition.dart
- .CallTopLevelWindowProcDelegates
- None Rect
- FLTIdTokenChannelStreamHandler
- None Size
- None String
- FLTAuthStateChannelStreamHandler
- stat_number_format_test.dart
- rubber_band.dart
- FlutterError
- AuthPigeonFirebaseApp
- _HeatmapDayCell
- leetcode_tag_matrix.dart
- plugin_registrar.cc
- workout_test.dart
- InternalUserInfo
- AuthPigeonFirebaseApp
- mood_gradient_slider.dart
- shell_weather_widget_test.dart
- MethodResultFunctions
- -checkActionCodeAppcodecompletion
- String get
- -verifyPhoneNumberApprequestcompletion
- firebase_auth_errors.dart
- vim_enabled_scope.dart
- _MonthWeekMorphLayer
- MultiFactorUserHostApi
- leetcode_constants.dart
- Timer?
- SoftDeletable
- selector_pill.dart
- StreamHandlerFunctions
- wWinMain
- build
- spell_check_suggestions_test.dart
- calendar_days_test.dart
- life_tracker_models.dart
- tag_suggestions.dart
- TEST
- int?
- Intent
- subset_mono_font.py
- build
- _HeatmapSquare
- analytics_service.dart
- InternalUserInfo
- CalendarRepository
- _YearGridBox
- PetalField
- _StableViewScrollPosition
- allDreamEntriesProvider
- allJournalEntriesProvider
- bucketListItemsProvider
- blossom_stat_popup.dart
- FLTAuthStateChannelStreamHandler
- voyager_list_item_surface.dart
- VoyagerColors
- LifeTreeCanvas
- firebase_auth_plugin.h
- package:voyager/core/utils/journal_tags.dart
- accentColor
- addedAt
- alertOnPeriodicPrompts
- alertTimeHour
- algorithm
- allocatedAt
- amountCents
- anchorDueDate
- asOf
- assetId
- backText
- birthDate
- birthYear
- body
- boolValue
- cadence
- calendarId
- calendarNavigateLeftKey
- calendarNavigateRightKey
- cardId
- code
- codeLanguage
- collection
- addedAt
- alertOnPeriodicPrompts
- alertTimeHour
- algorithm
- allocatedAt
- amountCents
- anchorDueDate
- asOf
- assetId
- backText
- birthDate
- birthYear
- body
- boolValue
- cadence
- calendarId
- calendarNavigateLeftKey
- calendarNavigateRightKey
- package:voyager/features/analytics/stat_number_format.dart
- <MultiFactorTotpSecretHostApi>
- study_grade_during_flip_test.dart
- VimMode
- Map
- GpuSurfaceTexture
- T
- GLASS_BUTTON.md
- PluginRegistrarManager
- study_keyboard_shortcuts.dart
- @visibleForTesting
- _HeatmapSquare
- <FirebaseAuthUserHostApi>
- _HeatmapWeekBlock
- PigeonParser
- PigeonParser
- _MonthGridBox
- cloud_function_weather_client.dart
- FLTFirebaseAuthPlugin
- flutter_windows.h
- PigeonParser
- String?
- character_sequence_crdt_merger.dart
- _revealEvent
- _revealTask
- firebase_options.dart
- _StableViewScrollPosition
- sparkline_touch.dart
- key_binding_dialog.dart
- newline_normalization.dart
- <MultiFactorTotpHostApi>
- life_tracker_stat_popup_width_test.dart
- InternalAuthCredential
- FlutterError
- 0.18.0
- Migration Guide
- http_callable_client.dart
- EncodableMap
- workout_constants.dart
- connectivity_status_test.dart
- Firebase Auth for Flutter
- InternalPhoneMultiFactorAssertion
- journal_constants.dart
- PackageDescription
- _TrackModalState
- fractional_index.dart
- trackersProvider
- Constants.java
- -registerAuthStateListenerAppcompletion
- StudyRepository
- DriftWorkoutRepository
- NotificationPopoverWarmup
- FirebaseApp
- _auth
- _delegate
- _delegate
- _auth
- _delegate
- toString
- _auth
- _delegate
- _multiFactor
- toString
- _PinnedNoteRowState
- voyager_spacing.dart
- periodic_prompt_service.dart
- MultiFactorUserHostApi
- build
- window_size_class.dart
- ChangeNotifier
- MultiFactorTotpHostApi
- BucketListRepository
- CalendarRepository
- DreamRepository
- FinanceRepository
- TEST
- JobRepository
- NotificationRepository
- DriftStudyRepository
- PixelBufferTexture
- package:voyager/core/constants/app_constants.dart
- package:voyager/features/life_tracker/life_tree_geometry.dart
- RenderBox
- _SingleChildViewportElement
- package:voyager/core/vim/vim_text_ops.dart
- FinanceRepository
- DriftTodoRepository
- customWordsProvider
- _Device
- TrackerRepository
- todoListStatsProvider
- SetUp
- -randomNonce
- FixedExtentScrollPhysics
- VoyagerColors
- DateTimePickerDialog
- TimeRangePickerDialog
- _QuickDelayedDragStartListener
- _CalendarWeekTimelineState
- WeatherForecastChart
- package:voyager/domain/models/analytics_models.dart
- Set
- interpolate_consecutive_test.dart
- neetcode150.dart
- LifeTreeCanvas
- _HeatmapSquare

## God Nodes (most connected - your core abstractions)
1. `FLTFirebaseAuthPlugin` - 87 edges
2. `FLTFirebaseAuthPlugin` - 87 edges
3. `Builder` - 86 edges
4. `remoteSyncServiceProvider` - 85 edges
5. `Result` - 73 edges
6. `_State` - 71 edges
7. `CanIgnoreReturnValue` - 70 edges
8. `settingsProvider` - 66 edges
9. `FirebaseAuthPlugin` - 63 edges
10. `AuthPigeonFirebaseApp` - 56 edges

## Surprising Connections (you probably didn't know these)
- `main` --references--> `settingsProvider`  [EXTRACTED]
  test/snippet_quick_add_test.dart → lib/app/providers.dart
- `_NoopRemoteSync` --implements--> `RemoteSyncService`  [EXTRACTED]
  test/glass_transition_backdrop_test.dart → lib/core/sync/remote_sync_service.dart
- `_FixedSettings` --inherits--> `SettingsNotifier`  [EXTRACTED]
  test/leetcode_activity_perf_test.dart → lib/app/providers.dart
- `_FixedSettings` --inherits--> `SettingsNotifier`  [EXTRACTED]
  test/leetcode_activity_view_test.dart → lib/app/providers.dart
- `_FixedSettings` --inherits--> `SettingsNotifier`  [EXTRACTED]
  test/leetcode_session_hides_test.dart → lib/app/providers.dart

## Import Cycles
- None detected.

## Communities (542 total, 69 thin omitted)

### Community 0 - "Database Data"
Cohesion: 0.00
Nodes (904): BoolColumn get, _, _accentColorMeta, actualTableName, _addedAtMeta, _alertOnPeriodicPromptsMeta, _alertTimeHourMeta, _algorithmMeta (+896 more)

### Community 1 - "Calendar Features"
Cohesion: 0.01
Nodes (265): AnimationStatusListener?, _, CalendarViewMode, _abortMorphAnimation, accentColor, adjacentColor, _advanceWarmup, anchor (+257 more)

### Community 2 - "Calendar Day"
Cohesion: 0.01
Nodes (237): accentColor, adjacentBorderT, adjacentTextT, alpha, _animatingEventId, available, barHeight, barStride (+229 more)

### Community 3 - "Journal Features"
Cohesion: 0.01
Nodes (198): JournalDebugLogger, journalAllEntryIdsProvider, journalEntryCacheInvalidatorProvider, journalEntryCountsProvider, journalListEntriesProvider, journalRepositoryProvider, journalsProvider, journalWriteCoordinatorProvider (+190 more)

### Community 4 - "Features Shell"
Cohesion: 0.02
Nodes (123): allowOverflow, allowVerticalOverflow, axisMaxY, axisMinY, belowPath, bottom, bottomAxisHeight, bucket (+115 more)

### Community 5 - "Calendar Features"
Cohesion: 0.02
Nodes (98): accentColor, allDayAreaTop, _AllDayShelfColumn, allDayShelfHeight, borderedClipRects, borderedDayColumnRect, borderedRects, borderRadius (+90 more)

### Community 6 - "Sync Remote"
Cohesion: 0.01
Nodes (209): CharacterOpRegistry, CharacterOpRegistry get, CharacterSequenceCrdtMerger, CrdtDocumentResolver, _activeDebouncers, _activelyEditedDocuments, addPendingTextMergeListener, applyPendingDreamEntryTextMerge (+201 more)

### Community 7 - "Todo Features"
Cohesion: 0.02
Nodes (96): _applyPendingNotesMerge, _beginEditingSession, build, _buildHeader, _clearDueDate, _close, color, _container (+88 more)

### Community 8 - "Repositories Domain"
Cohesion: 0.01
Nodes (223): addCustomWord, appendOperation, appendOperationGroup, appendOperationsBatch, authStateChanges, claimCalendarLock, claimWeatherFetchLock, countCardsReviewedToday (+215 more)

### Community 9 - "Calendar Grid"
Cohesion: 0.02
Nodes (83): calendar_day_entries.dart, calendar_day_grid.dart, calendar_todo_markers.dart, calendar_week_timeline.dart, CalendarEntryMenuBuilder, accentColor, allDay, base (+75 more)

### Community 10 - "App Dev"
Cohesion: 0.02
Nodes (100): DreamEntry?, accentColor, _appliedSavedWidth, _bodyEditorKey, _bodyFocusNode, bodyPreview, _buildEditorArea, _buildEntryList (+92 more)

### Community 11 - "Todo Features"
Cohesion: 0.01
Nodes (149): AsyncValue, _activeInList, _activeSectionHeight, _addingTask, animateIn, _animController, _appliedSavedShowAllTasks, _applyListMoveOptimistic (+141 more)

### Community 12 - "Models Domain"
Cohesion: 0.01
Nodes (150): GeometricWaveShape, accentColor, alertOnPeriodicPrompts, alertTimeHour, birthDate, birthYear, calendarNavigateLeftKey, calendarNavigateRightKey (+142 more)

### Community 13 - "Calendar Markers"
Cohesion: 0.03
Nodes (69): build, buildCalendarTodoMarkers, CalendarDayTodoBar, CalendarDayTodoIcons, CalendarDayTodoTile, calendarMaxTodoIconsPerDay, calendarMorphTodoIconProgress, calendarPackWeekAllDayShelf (+61 more)

### Community 14 - "Dev Remote"
Cohesion: 0.03
Nodes (63): _addInstantDiff, allMatched, _charMerger, compareAllJournalEntries, compareAllTodoLists, comparedAt, _compareJournalEntry, compareTodoList (+55 more)

### Community 15 - "Repositories Data"
Cohesion: 0.01
Nodes (241): addCustomWord, _applicationCompanion, _categoryCompanion, _companyCompanion, countCardsReviewedToday, countCardsReviewedTotal, countDueCards, countDueCardsInDeck (+233 more)

### Community 16 - "Sync Weather"
Cohesion: 0.02
Nodes (91): BackupFormatException, class MockDocumentReference extends, DocumentReference, FirebaseFirestore, package:voyager/data/remote/firestore_sync_repository.dart, package:voyager/features/settings/services/data_import_service.dart, ../test/fakes/fake_weather_api_client.dart, addCustomWord (+83 more)

### Community 17 - "Todo Task"
Cohesion: 0.03
Nodes (68): activeTasks, activeTopLevelTasks, applyDueDateChange, applyMoveToBottomOfCategory, applyNewUndatedTask, applyNormalizeUnstarredIfNeeded, applyReorder, _applyStar (+60 more)

### Community 18 - "App Providers"
Cohesion: 0.01
Nodes (176): AuthNotifier, CacheStatusSnapshot, ConnectivityStatusController, DataImportService, DevSettingsController, DreamWriteCoordinator, FpsMonitorController, GeometricTextureParams (+168 more)

### Community 19 - "Analytics Features"
Cohesion: 0.01
Nodes (231): AnalyticsService, await, accent, _addOption, all, analytics, anchor, anchorDateRect (+223 more)

### Community 20 - "Merger Services"
Cohesion: 0.03
Nodes (75): _anyNumberedLine, _applyEnterContinuation, applyListEditing, buffer, build, bullet, _bulletPattern, child (+67 more)

### Community 21 - "Search Features"
Cohesion: 0.12
Nodes (17): HotKey?, key, keyToken, modifiers, parseHotKey, parts, _physicalKey, createHotkeyService (+9 more)

### Community 22 - "Todo Features"
Cohesion: 0.02
Nodes (92): accent, child, color, _completingNow, _controller, copyWith, createState, _dateLabel (+84 more)

### Community 23 - "Text Widgets"
Cohesion: 0.03
Nodes (59): accentColor, _applyHighlightText, _baselineFor, _bringCursorScheduled, build, _buildField, _capHeight, contentPadding (+51 more)

### Community 24 - "Windows Runner"
Cohesion: 0.05
Nodes (37): Archive, archive, backupFormatVersion, backupManifestFileName, buildArchiveContents, _collections, DataExportService, exportDataToZip (+29 more)

### Community 25 - "Widgets Color"
Cohesion: 0.04
Nodes (50): autofocus, boxH, boxW, cappedMaxHeight, cappedMaxWidth, cell, clipPartialNextRow, ColorPaletteGrid (+42 more)

### Community 26 - "Widgets Rounded"
Cohesion: 0.04
Nodes (48): AddListDropdownValue, addListLabel, _addListSentinel, BorderedRoundedDropdown, build, closedTrailing, createState, didChangeDependencies (+40 more)

### Community 27 - "Widgets Contextual"
Cohesion: 0.05
Nodes (38): accentColor, barrierColor, barrierDismissible, barrierLabel, _borderWidth, build, builder, buildPage (+30 more)

### Community 28 - "Windows Ephemeral"
Cohesion: 0.02
Nodes (89): _advance, _advancePetal, approachLogDt, baseOpacity, build, burstAt, center, _clock (+81 more)

### Community 29 - "Time Widgets"
Cohesion: 0.05
Nodes (40): _amPmController, applyTo, build, _buildWheel, createBallisticSimulation, createState, _currentAmPmIndex, _currentDate (+32 more)

### Community 30 - "Calendar Features"
Cohesion: 0.06
Nodes (34): CalendarDayEntry, assigned, _buildClusters, CalendarOverlapSlot, clusterColumns, clusters, column, columnEnds (+26 more)

### Community 31 - "Widgets Datetime"
Cohesion: 0.05
Nodes (38): ColorScheme, build, _ClockDial, _ClockTimeMode, _ClockTimePicker, _ClockTimePickerState, colorScheme, createState (+30 more)

### Community 32 - "Domain Services"
Cohesion: 0.05
Nodes (42): ForecastPeriod?, 0, bucketHours, buckets, buildDayForecastChartSeries, chartBucketHour, chartBucketRangeCenteredOn, currentTimeChartHour (+34 more)

### Community 33 - "Windows Cpp"
Cohesion: 0.14
Nodes (15): startLeetCodeTrackFlow, LeetCodeTrackDraft, _chain, clear, _deleteQuietly, draft, _draftFileName, _file (+7 more)

### Community 34 - "Shell Features"
Cohesion: 0.02
Nodes (147): ConsumerState, ConsumerStatefulWidget, studyAllCardsProvider, studyCardsProvider, studyDeckByIdProvider, studyDecksProvider, studyDeckStatsProvider, studyFoldersProvider (+139 more)

### Community 35 - "Import Export"
Cohesion: 0.04
Nodes (44): FocusOnKeyEventCallback?, InputCounterWidgetBuilder?, accentColor, autofocus, borderRadius, build, buildCounter, _buildField (+36 more)

### Community 36 - "Todo List"
Cohesion: 0.01
Nodes (321): 0.0.1, 0.0.2, 0.0.2+1, 0.0.3, 0.0.3+1, 0.0.4, 0.10.0, 0.10.0+1 (+313 more)

### Community 37 - "Sync Firestore"
Cohesion: 0.02
Nodes (129): assetToFirestore, assetValuationToFirestore, body, bucketListItemToFirestore, budgetToFirestore, calendarEventToFirestore, calendarToFirestore, copyWith (+121 more)

### Community 38 - "Windows Ephemeral"
Cohesion: 0.01
Nodes (170): Float32List, ImageShader?, a, accentColor, _animation, apply, _atlas, _atlasColors (+162 more)

### Community 39 - "Sync Engine"
Cohesion: 0.03
Nodes (58): BackgroundSyncOrchestrator, backoffMultiplier, _bucketListRepository, _calendarRepository, cancelScheduledDocumentSync, _crdtResolver, _debouncer, _debouncerFor (+50 more)

### Community 40 - "Calendar Event"
Cohesion: 0.08
Nodes (25): applySettings, devForceConflictUi, disableCache, loadFromSettings, _persist, setDevForceConflictUi, setDisableCache, setShowCacheStatus (+17 more)

### Community 41 - "Sync Conflict"
Cohesion: 0.07
Nodes (28): accentColor, build, _canPop, createState, DateSelectorPopover, _DateSelectorPopoverState, didUpdateWidget, dispose (+20 more)

### Community 42 - "Calendar Features"
Cohesion: 0.03
Nodes (79): build, color, ColorCornerFlag, colorValue, _CornerFlagPainter, JournalBookmarkFlag, JournalTitleCornerFlag, onSelected (+71 more)

### Community 43 - "Features Dev"
Cohesion: 0.02
Nodes (97): alpha, _angleOf, attempts, base, baseAngle, _baseLobeCount, best, bestDistance (+89 more)

### Community 44 - "Models Domain"
Cohesion: 0.03
Nodes (57): 0, apostrophe, at, backspace, bare, base, caret, char (+49 more)

### Community 45 - "Todo Features"
Cohesion: 0.07
Nodes (145): ActionCodeInfoOperationBox, -initWithValue, AuthPigeonFirebaseApp, +fromList, -hash, -isEqual, +makeWithAppNametenantIdcustomAuthDomain, +nullableFromList (+137 more)

### Community 46 - "Sync Conflict"
Cohesion: 0.05
Nodes (42): build, burst, burstFromContext, color, colors, _ConfettiBurst, _ConfettiBurstState, ConfettiOverlay (+34 more)

### Community 47 - "Time Widgets"
Cohesion: 0.05
Nodes (45): _activeIsStart, _applyDuration, _applyEndDt, _applyStartDt, _applyTimeDt, build, _canPop, createState (+37 more)

### Community 48 - "Domain Services"
Cohesion: 0.06
Nodes (35): archiveLat, archiveLon, bucketHour, bucketMap, d, day, forecastArchiveLocationMatches, forecastBucketKey (+27 more)

### Community 49 - "Dev Journal"
Cohesion: 0.02
Nodes (135): ByteData, Image?, currentWeatherProvider, notificationBadgeStateProvider, _Await, id, JobColumn, jobRequiredColumns (+127 more)

### Community 50 - "Windows Ephemeral"
Cohesion: 0.06
Nodes (30): package:voyager/features/study/study_cram_page.dart, _cardOffset, cards, container, _deckId, main, now, _pumpCramPage (+22 more)

### Community 51 - "Widgets Text"
Cohesion: 0.06
Nodes (34): _bandAt, build, color, controller, createState, didUpdateWidget, dispose, focusNode (+26 more)

### Community 52 - "Weather Features"
Cohesion: 0.06
Nodes (48): EncodedType, flutter::StandardCodecSerializer, PigeonInternalCodecSerializer, ReadValueOfType, WriteValue, ByteBufferStreamReader, bytes_, location_ (+40 more)

### Community 53 - "Widgets Field"
Cohesion: 0.04
Nodes (47): jobStatusEventsProvider, accentColor, application, applicationId, build, _commit, companies, _companyController (+39 more)

### Community 54 - "Weather Forecast"
Cohesion: 0.06
Nodes (33): DayForecastChartSeries, _animation, _beginTransition, build, _controller, createState, _currentTimeHour, didUpdateWidget (+25 more)

### Community 55 - "Windows Ephemeral"
Cohesion: 0.04
Nodes (53): JournalEntry, _accentColor, _activeQuery, _baseline, body, _bodyController, _bodyFocusNode, _changeEntryDateAndTime (+45 more)

### Community 56 - "Windows Ephemeral"
Cohesion: 0.04
Nodes (47): Axis get, AxisDirection, AxisDirection get, class _RenderSingleChildViewport extends, class _SingleChildViewportElement extends, LayerHandle, applyPaintTransform, attach (+39 more)

### Community 57 - "Widgets Notched"
Cohesion: 0.06
Nodes (31): accentColor, alignLabelToTop, borderRadius, borderWidth, build, child, color, contentPadding (+23 more)

### Community 58 - "Data Remote"
Cohesion: 0.03
Nodes (63): Icon, IconButton, firestoreId, legacyTodoListFirestoreId, legacyTodoListId, localId, todoListDocumentIdForFirestore, todoListDocumentIdFromFirestore (+55 more)

### Community 59 - "Crdt Frac"
Cohesion: 0.07
Nodes (145): ActionCodeInfoOperationBox, -initWithValue, AuthPigeonFirebaseApp, +fromList, -hash, -isEqual, +makeWithAppNametenantIdcustomAuthDomain, +nullableFromList (+137 more)

### Community 60 - "Weather Features"
Cohesion: 0.07
Nodes (29): DateFormat, weatherForecastLastDayProvider, DailyForecastSummary, createState, _DailyCard, day, dayFormat, forecast (+21 more)

### Community 61 - "Journal Features"
Cohesion: 0.05
Nodes (39): package:voyager/features/journal/journal_settings_dialog.dart, package:voyager/features/todo/todo_settings_dialog.dart, SwitchListTile, closeSettings, container, db, _firstId, firstWhere (+31 more)

### Community 62 - "Sync Activity"
Cohesion: 0.04
Nodes (64): assetsProvider, assetValuationsProvider, financeCategoriesProvider, transactionsProvider, abs, accent, _AnalyticsCard, anchor (+56 more)

### Community 63 - "Models Domain"
Cohesion: 0.06
Nodes (35): body, byCreated, byDate, colorValue, compareJournalEntriesNewestFirst, copyWith, customQuote, entryDate (+27 more)

### Community 64 - "Platform Keyboard"
Cohesion: 0.08
Nodes (27): class, build, _confirmPermanentRemoteDelete, conflict, _ConflictDocumentIds, createState, _DiffColumn, dispose (+19 more)

### Community 65 - "Database Users"
Cohesion: 0.04
Nodes (87): AssetsTableCompanion, AssetsTableData, AssetValuationsTableCompanion, AssetValuationsTableData, BucketListItemsTableCompanion, BucketListItemsTableData, BudgetsTableCompanion, BudgetsTableData (+79 more)

### Community 66 - "Features Shell"
Cohesion: 0.13
Nodes (24): allDreamEntriesProvider, dreamRepositoryProvider, dreamWriteCoordinatorProvider, journalEntryCacheInvalidatorProvider, journalListEntriesProvider, journalRepositoryProvider, journalsProvider, journalWriteCoordinatorProvider (+16 more)

### Community 67 - "Windows Ephemeral"
Cohesion: 0.04
Nodes (56): dart:typed_data, _Pixels, RenderRepaintBoundary, _alpha, argbAt, boundary, bytes, controller (+48 more)

### Community 68 - "Firestore Data"
Cohesion: 0.05
Nodes (41): customWordsProvider, dictionaryProvider, _addToDictionary, _add, build, _buildResults, _bundledSeen, _BundledWordRow (+33 more)

### Community 69 - "Windows Ephemeral"
Cohesion: 0.04
Nodes (56): baseR, buildLeafSprite, canvas, center, cx, cy, endRecording, gradient (+48 more)

### Community 70 - "Widgets Geometric"
Cohesion: 0.03
Nodes (61): accentColor, animates, _animating, baseColor, build, _clock, copyWith, createState (+53 more)

### Community 71 - "Text Sync"
Cohesion: 0.07
Nodes (25): addListener, advance, applyToLocalText, bufferWhileEditing, clearDocument, collection, documentId, documentKey (+17 more)

### Community 72 - "Calendar Features"
Cohesion: 0.04
Nodes (41): dart:ui, Error, exception, installWindowsKeyboardWorkaround, _isKnownWindowsKeyboardDesync, message, previous, resyncWindowsKeyboardState (+33 more)

### Community 73 - "Domain Services"
Cohesion: 0.08
Nodes (29): LeetCodeProblem?, LeetCodeSolution, paperShaderProvider, _BackSolution, _backTitleKey, build, _CardBack, _CardFront (+21 more)

### Community 74 - "Widgets Keep"
Cohesion: 0.07
Nodes (27): accentColor, build, _canPop, createState, _currentDateTime, DateTimeSelectorPopover, _DateTimeSelectorPopoverState, didChangeDependencies (+19 more)

### Community 75 - "Domain Services"
Cohesion: 0.06
Nodes (31): package:voyager/features/study/study_deck_workbench_page.dart, _cards, cardsByDeck, container, _controlBar, countCardsReviewedToday, countCardsReviewedTotal, countDueCards (+23 more)

### Community 76 - "Main App"
Cohesion: 0.04
Nodes (51): Finder get, DreamRepository, JournalRepository, _bodyField, container, _CountingDreamRepository, db, _delegate (+43 more)

### Community 77 - "Widgets Date"
Cohesion: 0.09
Nodes (29): AutomaticKeepAliveClientMixin, ChildIndexGetter?, EdgeInsetsGeometry, IndexedWidgetBuilder, _PalettePickDialog, _PalettePickDialogState, build, cacheExtent (+21 more)

### Community 78 - "Features Shell"
Cohesion: 0.03
Nodes (84): studyRepositoryProvider, 2, copyWith, _dateOnly, days, due, dueAt, dueLeetCodeProblems (+76 more)

### Community 79 - "Calendar Features"
Cohesion: 0.05
Nodes (38): BoxConstraints?, BoxConstraints? constraints,
  EdgeInsetsGeometry, Clip, Color? accentColor,
  bool, barrierColor, barrierDismissible, barrierLabel, build (+30 more)

### Community 80 - "Sync Journal"
Cohesion: 0.02
Nodes (122): 0, big, body, _bracketObject, c, caret, caseSensitive, character (+114 more)

### Community 81 - "Domain Services"
Cohesion: 0.06
Nodes (39): calendarRepositoryProvider, calendarsProvider, allCalendars, assigner, changeCalendarListColor, choice, color, context (+31 more)

### Community 82 - "Shell Shortcuts"
Cohesion: 0.07
Nodes (30): Intent, _SubmitIntent, _CodeOutdentIntent, _CodeTabIntent, _blockingShortcuts, _BlockShellTabIntent, BlockShellTabShortcuts, build (+22 more)

### Community 83 - "Models Domain"
Cohesion: 0.04
Nodes (54): boolValue, buildDreamLoggedTracker, buildJournalEntriesTracker, buildStreakTracker, buildWordCountTracker, buildWorkedOutTracker, cadence, cap (+46 more)

### Community 84 - "Cloud Data"
Cohesion: 0.08
Nodes (23): A. Journal — create and propagate, Automated tests (CI / local), B. Journal — concurrent edit (CRDT stress), C. Journal — metadata, Check import/export works, especially for todo lists, and deleting journals keeps journal count consistent, CRDT / journal body checks, D. Todo — create and propagate, E. Todo — reorder and star (+15 more)

### Community 85 - "Journal Features"
Cohesion: 0.05
Nodes (40): _accent, accentColor, anchor, build, _buildCustomPage, _buildPresetPage, _commit, createState (+32 more)

### Community 86 - "Widgets Desktop"
Cohesion: 0.05
Nodes (37): buildDailyForecastSummaries, byDay, conditionCode, dailySummaries, date, day, days, description (+29 more)

### Community 87 - "Domain Calendar"
Cohesion: 0.06
Nodes (34): a, _areConsecutiveCalendarDays, b, calendarEventBarEndsOnDay, calendarEventBarsBridge, calendarEventBarStartsOnDay, calendarEventOccursOnDay, calendarEventOccursOnDayNormalized (+26 more)

### Community 88 - "Windows Ephemeral"
Cohesion: 0.06
Nodes (34): bill, buildNotificationFeed, copyWith, createdAt, days, daysUntil, deletedAt, dismissalKey (+26 more)

### Community 89 - "Client Data"
Cohesion: 0.03
Nodes (68): anchor, _anim, borderRadius, build, _buildRootMenu, _buildSubmenu, capturedThemes, child (+60 more)

### Community 90 - "Dev Cache"
Cohesion: 0.15
Nodes (12): package:voyager/features/life_tracker/blossom_stat_popup.dart, package:voyager/features/life_tracker/life_tree_popover.dart, SettingsNotifier, build, _FixedSettings, getSize, main, _popupWidth (+4 more)

### Community 91 - "Features Shell"
Cohesion: 0.10
Nodes (20): belowCurvePath, curvedLinePath, hourAtPixelX, matchesScale, maxX, maxY, minX, minY (+12 more)

### Community 92 - "Functions Forecast"
Cohesion: 0.13
Nodes (15): chartBucketHour(), firestore_1, forecastBucketKey(), formatDateKey(), localPartsFromUtc(), localTodayKey(), mergeForecastArchive(), parseBucketDateKey() (+7 more)

### Community 93 - "Features Page"
Cohesion: 0.06
Nodes (35): 10. Open polish (non-blocking), 11.1 The runtime lives inside `VimTextScope`, not a sibling scope, 11.2 Undo is handled by the snippet layer, not by the field's undo stack, 11.3 "Was that typed?" is answered by the key event, not the diff alone, 11. Implementation notes (where this deviates), 1. Goals, 2.1 Snippet record, 2.2 Global settings (alongside the list) (+27 more)

### Community 94 - "Dev Features"
Cohesion: 0.04
Nodes (45): Exception, classifySyncFailure, describeSyncFailure, _permanentFirestoreCodes, SyncFailureKind, toString, SyncPayloadTooLargeException, BackupFormatException (+37 more)

### Community 95 - "Theme Menu"
Cohesion: 0.07
Nodes (25): body, build, floatingActionButton, PlatformAdaptiveScaffold, build, child, EnterToSubmitScope, onSubmit (+17 more)

### Community 96 - "Registrar Windows"
Cohesion: 0.03
Nodes (68): TrackerValue, _addMonths, allocatedAt, amountCents, anchorDueDate, annualCents, annualCentsFor, asOf (+60 more)

### Community 97 - "Functions Src"
Cohesion: 0.16
Nodes (17): ArchiveDoc, archiveLocationMatches(), chartBucketHour(), clearForecastArchive(), forecastBucketKey(), ForecastPeriod, formatDateKey(), localPartsFromUtc() (+9 more)

### Community 98 - "Dev Settings"
Cohesion: 0.06
Nodes (43): authNotifierProvider, backgroundSyncOrchestratorProvider, bool?, connectivityStatusProvider, databaseProvider, HotkeyService, lazyLoadProvider, _bootstrap (+35 more)

### Community 99 - "Dev Todo"
Cohesion: 0.05
Nodes (39): Brightness, accentLineAmount, accentLineTarget, appBar, blurScale, brightness, _build, card (+31 more)

### Community 100 - "Binding Utils"
Cohesion: 0.08
Nodes (22): DateTime? dueAt,
  double, package:voyager/core/widgets/search_highlight_text.dart, package:voyager/features/study/study_card_tile.dart, _highlighted, hits, main, _pumpText, _renderedText (+14 more)

### Community 101 - "Widgets Resizable"
Cohesion: 0.09
Nodes (23): build, clampListWidth, createState, defaultListWidth, dividerWidth, dragClampListWidth, _dragging, _dragStartGlobalX (+15 more)

### Community 102 - "Auth Data"
Cohesion: 0.08
Nodes (29): build, _canvasController, createState, dispose, geometry, getClip, _globalCenterFor, _grassColor (+21 more)

### Community 103 - "Character Domain"
Cohesion: 0.03
Nodes (167): string_view, AuthPigeonFirebaseApp, app_name_, AuthPigeonFirebaseApp::AuthPigeonFirebaseApp(), custom_auth_domain, Hash, set_app_name, set_custom_auth_domain (+159 more)

### Community 104 - "Features Calendar"
Cohesion: 0.07
Nodes (30): @immutable, CacheItemStatus get, CacheItemStatus, ConfettiEffect, SnippetMarks, VimChromePlacement, VimSearchPrompt, VimFieldBinding (+22 more)

### Community 105 - "Features Calendar"
Cohesion: 0.02
Nodes (115): _alwaysFocused, _applyCaseOverRange, _applyMotion, _applyOperatorRange, _applyValue, _blockVisualLineHorizontal, _cachedMatches, _cachedMatchesFor (+107 more)

### Community 106 - "Widgets Clamp"
Cohesion: 0.06
Nodes (32): calendarId, colorValue, copyWith, end, EventSource, exceptionDates, externalId, isFullDay (+24 more)

### Community 107 - "Hotkey Features"
Cohesion: 0.05
Nodes (37): appendOperation, appendOperationGroup, appendOperationsBatch, _calendarLock, claimCalendarLock, claimWeatherFetchLock, _collectionWatchers, _currentWeather (+29 more)

### Community 108 - "Palette Color"
Cohesion: 0.09
Nodes (22): ContextMenuRegionState, _back, build, createState, _face, _footer, _front, keywords (+14 more)

### Community 109 - "Dev Sync"
Cohesion: 0.04
Nodes (52): tagColorsProvider, tagPoolProvider, accentColor, _accept, build, _caretGap, child, _close (+44 more)

### Community 110 - "Domain Repositories"
Cohesion: 0.02
Nodes (113): AdditionalUserInfo? get, AuthCredential? get, additionalUserInfo, app, applyActionCode, authStateChanges, checkActionCode, clear (+105 more)

### Community 111 - "Calendar Features"
Cohesion: 0.04
Nodes (47): DateTimeRange, goalAllocationsProvider, SavingsGoal, BucketListItem, completed, completedAt, copyWith, createdAt (+39 more)

### Community 112 - "Windows Ephemeral"
Cohesion: 0.06
Nodes (4): get, NonNull, Builder, CanIgnoreReturnValue

### Community 113 - "Widgets Desktop"
Cohesion: 0.09
Nodes (24): FormField, FormFieldState, InputDecoration?, accentColor, build, createState, decoration, decoratorKey (+16 more)

### Community 114 - "Widgets Dropdown"
Cohesion: 0.04
Nodes (52): _absorbEdit, _advanceTabstop, _alwaysFocused, _applying, _applyValue, _caret, contains, cutsLiteral (+44 more)

### Community 115 - "App Providers"
Cohesion: 0.09
Nodes (23): FragmentProgram?, FragmentShader?, baseColor, build, createState, defaults, didUpdateWidget, dispose (+15 more)

### Community 116 - "Windows Ephemeral"
Cohesion: 0.05
Nodes (36): accent, after, before, _canRedo, _canUndo, cardIds, _clearHistory, createState (+28 more)

### Community 117 - "Sync Debouncer"
Cohesion: 0.02
Nodes (102): CodeController, VimTextOverlay, _VimTextOverlayState, _SparklineTouchScope, _SparklineTouchScopeState, _StatTile, _StatTileState, CalendarInteractiveEventTap (+94 more)

### Community 118 - "Widgets Menu"
Cohesion: 0.24
Nodes (12): -getFIRAuthCredentialFromArgumentsappcompletion, -handleInternalErrorwithError, -handleMultiFactorErrorcompletionwithError, -linkWithCredentialAppinputcompletion, -reauthenticateWithCredentialAppinputcompletion, -sendSignInLinkToEmailAppemailactionCodeSettingscompletion, -signInWithCredentialAppinputcompletion, -signInWithCustomTokenApptokencompletion (+4 more)

### Community 119 - "Widgets Search"
Cohesion: 0.03
Nodes (11): InternalFirebaseAuthSettings, InternalActionCodeSettings, InternalAdditionalUserInfo, InternalFirebaseAuthSettings, InternalIdTokenResult, InternalUserInfo, Nullable, AdditionalUserInfo (+3 more)

### Community 120 - "Models Domain"
Cohesion: 0.09
Nodes (22): DateTime get, colorValue, completed, copyWith, dueDate, dueDateSetAt, effectiveRecurrenceAnchor, fromJson (+14 more)

### Community 121 - "Settings Palette"
Cohesion: 0.08
Nodes (25): autoExpand, build, card, createState, dispose, error, focusReplacement, initialTrigger (+17 more)

### Community 122 - "Windows Ephemeral"
Cohesion: 0.03
Nodes (64): anchorRect, anchorRectFor, box, _ChartPlaceholder, _close, _controller, createState, _cuesController (+56 more)

### Community 123 - "Platform Widgets"
Cohesion: 0.07
Nodes (36): geometricDebugRowFadeProvider, geometricShaderProvider, geometricTextureParamsProvider, geometricWaveParamsProvider, _AppBackground, build, buildOverscrollIndicator, buildScrollbar (+28 more)

### Community 124 - "Shell Features"
Cohesion: 0.07
Nodes (31): _persistSplitWidth, _save, _amountController, build, container, createState, _date, _datePopoverOpen (+23 more)

### Community 125 - "Windows Ephemeral"
Cohesion: 0.09
Nodes (21): Card, ContextMenuRegion, LeetCodeDifficulty, package:voyager/core/theme/srs_mastery_color.dart, StudyFlipCard, algorithm, card, difficulty (+13 more)

### Community 126 - "Database Data"
Cohesion: 0.04
Nodes (45): @DataClassName, @TableIndex, AssetsTable, AssetValuationsTable, BucketListItemsTable, BudgetsTable, CalendarEventsTable, CalendarsTable (+37 more)

### Community 127 - "Menu Widgets"
Cohesion: 0.08
Nodes (28): ActivityAware, ActivityPluginBinding, FlutterFirebasePlugin, FlutterPlugin, FlutterPluginBinding, Task, FlutterFirebaseAuthPlugin, Activity (+20 more)

### Community 128 - "Sync Outbox"
Cohesion: 0.07
Nodes (35): build, customQuotesProvider, quotePoolProvider, saveSettings, settingsRepositoryProvider, CustomQuote, _persistEntryListWidth, _persistLastViewedJournal (+27 more)

### Community 129 - "Functions Scripts"
Cohesion: 0.13
Nodes (14): dependencies, firebase-admin, firebase-functions, devDependencies, typescript, engines, node, main (+6 more)

### Community 130 - "Dev Warmup"
Cohesion: 0.03
Nodes (64): Alignment, BorderRadius, alignment, autofocus, blurSigma, borderColor, borderOpacity, borderRadius (+56 more)

### Community 131 - "Todo Task"
Cohesion: 0.09
Nodes (8): AuthPigeonFirebaseApp, FirebaseAuthHostApi, FirebaseAuthUserHostApi, InternalUserCredential, InternalUserDetails, MultiFactorUserHostApi, Result, VoidResult

### Community 132 - "Gradient Widgets"
Cohesion: 0.03
Nodes (59): controller, count, createState, _CurrentExerciseTitle, didUpdateWidget, _discard, dispose, exercise (+51 more)

### Community 133 - "Models Analytics"
Cohesion: 0.04
Nodes (46): _applyDecision, bucket0, bucket1, bucket2, _BucketProgressBar, build, _canRedo, _canUndo (+38 more)

### Community 134 - "Models Domain"
Cohesion: 0.14
Nodes (63): AuthError, FederatedOAuthProvider, FutureBase, AuthPigeonFirebaseApp, AuthResult, FlutterError, function, InternalActionCodeSettings (+55 more)

### Community 135 - "Windows Ephemeral"
Cohesion: 0.11
Nodes (19): Animation, _animDuration, _barVisible, build, child, _controller, createState, DesktopWindowFrame (+11 more)

### Community 136 - "Platform Domain"
Cohesion: 0.03
Nodes (20): ByteBuffer, Retention, Target, FlutterError, GeneratedAndroidFirebaseAuth, InternalAuthCredential, InternalAuthCredentialInput, InternalMultiFactorSession (+12 more)

### Community 137 - "Domain Services"
Cohesion: 0.03
Nodes (54): Color?, EdgeInsets, Key? dateKey,
  double, srsMasteryColor, decoration, focusBorderColor, focusShape, hoverColor (+46 more)

### Community 138 - "Functions Tsconfig"
Cohesion: 0.15
Nodes (12): compileOnSave, compilerOptions, esModuleInterop, module, noImplicitReturns, noUnusedLocals, outDir, skipLibCheck (+4 more)

### Community 139 - "Auth Firebase"
Cohesion: 0.07
Nodes (31): trackerRepositoryProvider, trackerValuesProvider, build, _buildEditor, _clampInt, commit, _commitPendingForDateChange, createState (+23 more)

### Community 140 - "Color Widgets"
Cohesion: 0.04
Nodes (47): DateTime? now,
  bool, amountCents, assetCents, best, BreakdownSlice, byAsset, cash, cashCents (+39 more)

### Community 141 - "FlutterView"
Cohesion: 0.10
Nodes (19): dropdownMenuTheme, _itemHighlightRadius, itemPadding, menuBorderRadius, menuColor, menuColorOf, menuStyle, menuTheme (+11 more)

### Community 142 - "Windows Ephemeral"
Cohesion: 0.09
Nodes (7): TotpSecret, FlutterFirebaseTotpMultiFactor, InternalTotpSecret, Override, FlutterFirebaseTotpSecret, Override, InternalTotpSecret

### Community 143 - "Windows Ephemeral"
Cohesion: 0.03
Nodes (79): accentColor, _advanceWidth, boxes, build, _caretForegroundFor, caretOffset, child, computeLuminance (+71 more)

### Community 144 - "Widgets Selector"
Cohesion: 0.09
Nodes (13): FirebaseException, ForceResendingToken, Locale?, PhoneMultiFactorInfo, InternalVerifyPhoneNumberRequest, Activity, AuthPigeonFirebaseApp, InternalVerifyPhoneNumberRequest (+5 more)

### Community 145 - "Constants Google"
Cohesion: 0.06
Nodes (51): ActionCodeInfoOperationBox, -initWithValue, AuthPigeonFirebaseApp, -init, +makeWithAppNametenantIdcustomAuthDomain, <GenerateInterfaces>, -pigeonInterfaceInfoerror, NSInteger (+43 more)

### Community 146 - "Features Analytics"
Cohesion: 0.14
Nodes (13): Files touched by this migration, How it works, Intentional exception: the inline todo-item rename field, Known limitations / non-goals, `LabeledTextField` (`lib/core/widgets/labeled_text_field.dart`), `TagHighlightedTextField` (`lib/core/widgets/tag_highlighted_text_field.dart`), Text box widget: `NotchedFieldBorder`, The notch math (+5 more)

### Community 147 - "Features Settings"
Cohesion: 0.05
Nodes (66): ActionCodeInfoOperationBox, -initWithValue, AuthPigeonFirebaseApp, -init, +makeWithAppNametenantIdcustomAuthDomain, <GenerateInterfaces>, -pigeonInterfaceInfoerror, NSArray (+58 more)

### Community 148 - "Shell App"
Cohesion: 0.02
Nodes (108): @DriftDatabase, DriftBucketListRepository, DriftCalendarRepository, DriftFinanceRepository, DriftNotificationRepository, DriftSettingsRepository, DriftTrackerRepository, fakes/fake_weather_api_client.dart (+100 more)

### Community 149 - "Widgets Features"
Cohesion: 0.06
Nodes (31): bytes, character, charOps, charOpsKey, chunkCount, chunkCountKey, chunkIndex, chunkIndexKey (+23 more)

### Community 150 - "Constants Journal"
Cohesion: 0.04
Nodes (48): build, CompactBackBar, label, onBack, WeightUnit, Exercise, _accept, active (+40 more)

### Community 151 - "Widgets Confirm"
Cohesion: 0.08
Nodes (31): Auth, auth::AuthStateListener, auth::IdTokenListener, mutex, StreamHandler<flutter::EncodableValue>, AuthStateStreamHandler, auth_, listener_ (+23 more)

### Community 152 - "Weather Fakes"
Cohesion: 0.04
Nodes (49): ClipRectLayer, LayerLink, anchorOffset, applyPaintTransform, asIs, attach, build, _checkScheduled (+41 more)

### Community 153 - "Features Settings"
Cohesion: 0.07
Nodes (40): FLTFirebaseAuthPlugin, -applicationdidReceiveRemoteNotificationfetchCompletionHandler, -applicationdidRegisterForRemoteNotificationsWithDeviceToken, -applicationopenURLoptions, -authorizationControllerdidCompleteWithAuthorization, -cleanupWithCompletion, -detachFromEngineForRegistrar, -didReinitializeFirebaseCore (+32 more)

### Community 154 - "warmup_tracker.dart"
Cohesion: 0.05
Nodes (87): FLTFirebaseAuthPlugin, -applicationdidReceiveRemoteNotificationfetchCompletionHandler, -applicationdidRegisterForRemoteNotificationsWithDeviceToken, -applicationopenURLoptions, -applyActionCodeAppcodecompletion, -authorizationControllerdidCompleteWithError, -checkActionCodeAppcodecompletion, -cleanupWithCompletion (+79 more)

### Community 155 - "Data Remote"
Cohesion: 0.11
Nodes (18): BaseSliderTrackShape, accent, build, fillColor, getPreferredRect, getPreferredSize, gradient, GradientSliderTrackShape (+10 more)

### Community 156 - "Delta Scramble"
Cohesion: 0.20
Nodes (8): local, main, newRemote, newRemote2, oldRemote, oldRemote2, main, lib/core/sync/text_delta_injector.dart

### Community 157 - "Dev Sync"
Cohesion: 0.12
Nodes (16): 1. Shader asset, 2. Loading, 3. Tunable parameters, 4. Widget → painter → GPU, 5. App-level composition, 6. UI that interacts with the background, 7. GPU warmup (avoid first-frame jank), 8. Failure modes (+8 more)

### Community 158 - "Windows Runner"
Cohesion: 0.08
Nodes (23): 10. Self-echo suppression discards genuine concurrent remote changes, 11. Self-echo marks are written after the upload, so suppression rarely fires, 12. Live sync refetches documents the snapshot listener already delivered, 13. Operation-log entries are written for collections that have no collaborative text, 14. Weather persistence writes back a stale whole-settings snapshot, 15. Live-sync drops queued document IDs when a pull throws, 16. `permanentlyDeleteFromRemote` deletes operations under the wrong document ID, 17. `purgeExpiredDeleted` loads every row of every table on every launch (+15 more)

### Community 159 - "Domain Services"
Cohesion: 0.08
Nodes (22): FLTAuthStateChannelStreamHandler, -initWithAuth, -onCancelWithArguments, -onListenWithArgumentseventSink, FLTIdTokenChannelStreamHandler, -initWithAuth, -onCancelWithArguments, -onListenWithArgumentseventSink (+14 more)

### Community 160 - "Shell Features"
Cohesion: 0.07
Nodes (27): dart:collection, _current, currentFps, dispose, _fps, _history, historyLength, isRunning (+19 more)

### Community 161 - "Windows Ephemeral"
Cohesion: 0.06
Nodes (34): AppThemeMode, _controller, createState, dispose, divisions, _focusNode, initialUsername, label (+26 more)

### Community 162 - "Sync Text"
Cohesion: 0.22
Nodes (8): _, adjustedSelection, _commonPrefixLength, _commonSuffixLength, _fallbackMerge, injectRemoteDelta, _insertAt, TextDeltaInjector

### Community 163 - "Sync Pending"
Cohesion: 0.02
Nodes (79): build, QuickJournalPopup, QuickTodoPopup, getOrderedDestinations, icon, index, label, order (+71 more)

### Community 164 - "App Providers"
Cohesion: 0.03
Nodes (67): accentColor, _active, _badge, _beginSuspension, blockCaret, build, builder, _canCreateSnippets (+59 more)

### Community 165 - "trackerRepositoryProvider"
Cohesion: 0.08
Nodes (25): Color? highlightColor,
  List, best, collapsed, colorFor, cursor, end, highlightedStyle, index (+17 more)

### Community 166 - "Theme Spacing"
Cohesion: 0.10
Nodes (24): ActionCodeSettings, GetTokenResult, MultiFactorInfo, FlutterFirebaseAuthPluginException, FlutterError, FlutterFirebaseAuthUser, Activity, AuthPigeonFirebaseApp (+16 more)

### Community 167 - "quick_popups.dart"
Cohesion: 0.07
Nodes (26): confirmed, context, copyLeetCodeCode, dismiss, draft, draftFuture, _firstCodeOf, gradeAndSaveLeetCodeProblem (+18 more)

### Community 168 - "Windows Ephemeral"
Cohesion: 0.06
Nodes (29): _addSnippet, anchor, _applySuggestion, build, createState, editableTextState, getConstraintsForChild, getPositionForChild (+21 more)

### Community 169 - "@immutable"
Cohesion: 0.07
Nodes (28): _auth, AuthNotifier, dispose, _isAuthenticated, _subscription, _auth, _authErrorMessage, authStateChanges (+20 more)

### Community 170 - "ByteBufferStreamReader"
Cohesion: 0.04
Nodes (44): accentColor, alignLabelToTop, autofocus, borderRadius, build, _buildField, contentPadding, controller (+36 more)

### Community 171 - "Widgets Enter"
Cohesion: 0.03
Nodes (64): financeRepositoryProvider, _save, _delete, _save, _delete, _delete, _save, color (+56 more)

### Community 172 - "Gpusurfacetexture Windows"
Cohesion: 0.11
Nodes (31): -applyActionCodeAppcodecompletion, -authorizationControllerdidCompleteWithError, -confirmPasswordResetAppcodenewPasswordcompletion, +convertToFlutterError, -createUserWithEmailAndPasswordAppemailpasswordcompletion, -deleteAppcompletion, -fetchSignInMethodsForEmailAppemailcompletion, -getFIRAuthFromAppNameFromPigeon (+23 more)

### Community 173 - "@immutable"
Cohesion: 0.06
Nodes (35): 10. Explicitly Out of Scope (v1), 11. Acceptance Criteria (summary), 12. Decision Log (from product review), 1. Purpose, 2. Shell & Navigation, 3.1 Always-visible header (compact), 3.2 Main surface — flat table, 3.3 Editor panel (todo-page pattern) (+27 more)

### Community 174 - "Constants Todo"
Cohesion: 0.05
Nodes (36): anchor, _aspect, at, body, _Bone, _bones, dotPaint, fromLTWH (+28 more)

### Community 175 - "Shell Features"
Cohesion: 0.20
Nodes (9): 1. Database Schema (SQLite / CRDT Ready), 2. State Management & Logic (Riverpod), 3. UI / Widget Architecture, A. The 1-Year Rolling Maximum (Independent Integers), A. The Journal Page Entry Flow, B. Analytics Page: Default View (The 4 Dashboards), B. Enum Dynamic Color Generation, C. Analytics Page: Calendar View (+1 more)

### Community 176 - "Messenger Windows"
Cohesion: 0.09
Nodes (21): build, card, _face, keywords, matches, multiSelectEnabled, now, onDelete (+13 more)

### Community 177 - "dart:math"
Cohesion: 0.06
Nodes (30): affectedEntryIds, allJournals, assigner, choice, color, context, created, defaultColor (+22 more)

### Community 178 - "Constants Hotkey"
Cohesion: 0.18
Nodes (10): defaultCalendarNavigateLeftKey, defaultCalendarNavigateRightKey, defaultJournalHotkey, defaultStudyEasyKey, defaultStudyFailKey, defaultStudyGoodKey, defaultStudyHardKey, defaultTodoHotkey (+2 more)

### Community 179 - "Sync Firestore"
Cohesion: 0.03
Nodes (69): dart:convert, CrdtDocumentResolver, _merger, _mergeSyncOperations, resolvePayload, _dreamRepository, DreamWriteCoordinator, _findTask (+61 more)

### Community 180 - "@visibleForTesting"
Cohesion: 0.05
Nodes (38): AnalyticsService, booleanTrueCount, completedTasks, countWords, heatmapIntensity, integerSeries, interpolateConsecutive, rollingMax (+30 more)

### Community 181 - "MethodCall"
Cohesion: 0.05
Nodes (43): LeetCodeRepository, LeetCodeCramPage, package:voyager/features/leetcode/leetcode_cram_page.dart, package:voyager/features/leetcode/leetcode_session_page.dart, _FakeLeetCodeRepository, _RecordingLeetCodeRepository, _RecordingLeetCodeRepository, _button (+35 more)

### Community 182 - "Anagram List1"
Cohesion: 0.33
Nodes (5): list1, list2, main, s1, s2

### Community 183 - "dart:math"
Cohesion: 0.05
Nodes (40): BillingPeriod, _amountController, build, _canSave, _colorValue, container, createState, _datePopoverOpen (+32 more)

### Community 184 - "Calendar Features"
Cohesion: 0.10
Nodes (19): class ConnectivityStatusController extends, _consecutiveFailures, didChangeAppLifecycleState, dispose, _disposed, _failuresBeforeOffline, _isOnline, offlineInterval (+11 more)

### Community 185 - "debouncer.dart"
Cohesion: 0.04
Nodes (45): _applyDecision, bucket0, bucket1, bucket2, _BucketProgressBar, _canRedo, _canUndo, _cardKey (+37 more)

### Community 186 - "Widgets Rounded"
Cohesion: 0.11
Nodes (19): build, _buildLayer, children, createState, didChangeDependencies, didUpdateWidget, dispose, duration (+11 more)

### Community 187 - "Chart Weather"
Cohesion: 0.08
Nodes (24): FixedExtentScrollController, applyTo, build, _controller, createBallisticSimulation, createState, didUpdateWidget, _displayIndex (+16 more)

### Community 188 - "Android Gradlew"
Cohesion: 0.60
Nodes (3): gradlew script, die(), warn()

### Community 189 - "Deinterleave Evens"
Cohesion: 0.40
Nodes (4): evens, main, odds, s

### Community 190 - "WeatherApiClient"
Cohesion: 0.12
Nodes (16): JobApplication, application, build, _cell, color, _colorColumnWidth, _columnFlex, columns (+8 more)

### Community 191 - "Dev Flags"
Cohesion: 0.17
Nodes (11): DevFlags, disableCache, disablePetalField, forceOffline, showLifeTrackerRestore, showLifeTreeSegmentDebug, showTimeSelectorHitboxes, slowHeatmapPopoverAnimation (+3 more)

### Community 192 - "Utils Ids"
Cohesion: 0.25
Nodes (7): day, month, newId, trackerValueId, utcNow, _uuid, package:uuid/uuid.dart

### Community 193 - "Utils Journal"
Cohesion: 0.40
Nodes (4): colorForTag, extractTags, journalTagPattern, matches

### Community 194 - "Google Desktop"
Cohesion: 0.06
Nodes (30): backtickStrings, blockComments, char, charLiterals, closer, _closesAsCharLiteral, _cStyle, false (+22 more)

### Community 196 - "PLAN.md"
Cohesion: 0.22
Nodes (8): AGENTS, Features, Frontend UI, Future Additions, High Level Overview, Low Level Design, Miscellaneous, Theme

### Community 198 - "Calendar Customclipper"
Cohesion: 0.04
Nodes (47): assets, assetValuations, bucketListItems, budgets, calendarEvents, calendars, crdtBacked, customQuotes (+39 more)

### Community 199 - "High-Level Design: Voyager Weekly Calendar View"
Cohesion: 0.22
Nodes (8): High-Level Design: Voyager Weekly Calendar View, I. System Overview, II. Visual Layout & Z-Index Stack, III. The Overlap Engine (Column Splitting), IV. Data Entity Specifications, The Zero-Duration Task Layout Logic, Todo Task Logic, V. User Interaction Flow

### Community 200 - "IMPORT_EXPORT.md"
Cohesion: 0.25
Nodes (7): 1. The Database Layer (Drift), 2. The Export Pipeline (`DataExportService`), 3. The Import Pipeline (`DataImportService`), 4. The Trickle Sync Engine (`OutboxSyncWorker`), 5. Testing Strategy, Integration Tests (Slow, End-to-End), Unit Tests (Fast, Isolated)

### Community 201 - "Constants App"
Cohesion: 0.50
Nodes (3): accentBorderAlpha, softDeleteRetentionDays, syncDebounceSeconds

### Community 202 - "todoSortDebugLoggerProvider"
Cohesion: 0.05
Nodes (39): AuthRepository, Delay, _authRepo, clearFor, _clearPending, data, _db, drainableCollections (+31 more)

### Community 203 - "Color"
Cohesion: 0.04
Nodes (52): accent, axis, _bottomFade, branchIndex, child, _ClockText, _ClockTextState, color (+44 more)

### Community 208 - "SingleChildLayoutDelegate"
Cohesion: 0.08
Nodes (25): <FirebaseAuthHostApi>, -applyActionCodeAppcodecompletion, -checkActionCodeAppcodecompletion, -confirmPasswordResetAppcodenewPasswordcompletion, -createUserWithEmailAndPasswordAppemailpasswordcompletion, -fetchSignInMethodsForEmailAppemailcompletion, -initializeRecaptchaConfigAppcompletion, -registerAuthStateListenerAppcompletion (+17 more)

### Community 212 - "ADR 001: Local-First Data Model"
Cohesion: 0.33
Nodes (5): ADR 001: Local-First Data Model, Consequences, Context, Decision, Status

### Community 213 - "ADR 002: Sync Protocol Boundaries"
Cohesion: 0.20
Nodes (9): ADR 002: Sync Protocol Boundaries, Amendment: Jobs hard delete (2026-08-22), Consequences, Consequences, Context, Context, Decision, Decision (+1 more)

### Community 214 - "ADR 003: Module Structure"
Cohesion: 0.33
Nodes (5): ADR 003: Module Structure, Consequences, Context, Decision, Status

### Community 215 - "List"
Cohesion: 0.08
Nodes (25): <FirebaseAuthHostApi>, -applyActionCodeAppcodecompletion, -checkActionCodeAppcodecompletion, -confirmPasswordResetAppcodenewPasswordcompletion, -createUserWithEmailAndPasswordAppemailpasswordcompletion, -fetchSignInMethodsForEmailAppemailcompletion, -initializeRecaptchaConfigAppcompletion, -registerAuthStateListenerAppcompletion (+17 more)

### Community 216 - "_VoyagerAppState"
Cohesion: 0.07
Nodes (29): CapturedThemes, Color? get, _ContextualPopoverRoute, VoyagerPopupMenuRoute, accentColor, anchor, barrierColor, barrierDismissible (+21 more)

### Community 217 - "Voyager"
Cohesion: 0.33
Nodes (5): Architecture, Features, Setup, Testing, Voyager

### Community 218 - "_AddListMenuItem"
Cohesion: 0.09
Nodes (24): weatherForecastProvider, _advance, build, createState, dispose, _done, _frame, initState (+16 more)

### Community 219 - "weatherForecastProvider"
Cohesion: 0.13
Nodes (14): 1. Core Purpose & Overview, 2. Database Models & Schema Integration, 3. Detailed Component Breakdown & Edge Cases, 4. Popover & Morphing Editor, 5. Summary of Key Technical Edge Cases, A. Hover Tooltip Overlay, A. The Grid View & Sparkline Stack, B. Morph Route Transition (+6 more)

### Community 220 - "TIME_SELECTOR.md"
Cohesion: 0.40
Nodes (4): 1. The Container (The Anchored Popover), 2. The Internal Layout (Start Time Popup), 3. The Internal Layout (End Time / Duration Popup), The Execution Summary

### Community 221 - "MessageHandler"
Cohesion: 0.04
Nodes (52): formatTime12Hour, formatTimeOfDay12Hour, jm, local, now, _closingAfterSave, _completed, createState (+44 more)

### Community 222 - "weather_chart_curve_test.dart"
Cohesion: 0.09
Nodes (20): dart:math, indices, main, r, sorted, axisReservedSize, chars, compactNumberLabel (+12 more)

### Community 223 - "CalendarRepository"
Cohesion: 0.06
Nodes (34): appendOperation, appendOperationGroup, appendOperationsBatch, claimCalendarLock, claimWeatherFetchLock, _collection, deleteDocument, deleteOperations (+26 more)

### Community 226 - "FEEDBACK.md"
Cohesion: 0.25
Nodes (7): Calendar, FEEDBACK, Finance, Global Inbox, LeetCode Tracker, Life Tracker, Settings

### Community 227 - "MorphDayEventStack"
Cohesion: 0.02
Nodes (94): LeetCodeApiQuestion?, LeetCodeCodeController, LeetCodeTrackDraft?, LeetCodeTrackDraft? draft,
  LeetCodeTrackDraftOutcome, LeetCodeTrackDraftStore, accentColor, _addExample, _addSolution (+86 more)

### Community 228 - "package:voyager/domain/services/weather_forecast_chart.dart"
Cohesion: 0.20
Nodes (11): MultiFactorAssertion, MultiFactorResolver, FlutterFirebaseMultiFactor, AuthPigeonFirebaseApp, InternalMultiFactorInfo, InternalMultiFactorSession, InternalPhoneMultiFactorAssertion, InternalUserCredential (+3 more)

### Community 229 - "_CalendarMorphWarmupState"
Cohesion: 0.06
Nodes (30): _boundaryBackward, _boundaryChar, _boundaryForwardPad, checkIncremental, checkTextSync, _customWords, _dictionary, _fetchLastSpans (+22 more)

### Community 230 - "_HeatmapDayCell"
Cohesion: 0.03
Nodes (65): CurvedAnimation, accentColor, allowed, attach, _blockCaret, build, capsLockBadgeVisible, CapsLockCaretIndicator (+57 more)

### Community 231 - "_MorphAnimationLayer"
Cohesion: 0.07
Nodes (29): _attach, back, build, controller, createState, _detach, didUpdateWidget, dispose (+21 more)

### Community 233 - "MethodResultFunctions"
Cohesion: 0.03
Nodes (68): int?, build, colorValue, tag, TagChip, buffer, build, cached (+60 more)

### Community 234 - "package:voyager/core/sync/text_delta_injector.dart"
Cohesion: 0.06
Nodes (36): jobSelectedApplicationProvider, _categoryById, _closePanel, _companyByKey, _CompanyColors, _count, _createApplication, createState (+28 more)

### Community 235 - "addedAt"
Cohesion: 0.06
Nodes (40): budgetsProvider, Budget, _BudgetModal, _BudgetModalState, build, _canSave, container, createState (+32 more)

### Community 236 - "sync_conflict.dart"
Cohesion: 0.06
Nodes (32): _, CalendarEvent, allDay, allDayEvent, available, calendarDayEntriesForDay, CalendarDayEntryKind, calendarVisibleEntryCount (+24 more)

### Community 237 - "FlutterViewController::HandleTopLevelWindowProc"
Cohesion: 0.06
Nodes (33): Architecture overview, Clipboard rules, Data model, Display width, Download / prefetch, Explicitly deferred, Goals, Import / export (+25 more)

### Community 238 - "alertTimeHour"
Cohesion: 0.07
Nodes (26): age, assumedRestingHeartRateBpm, assumedSleepHoursPerNight, days, daysLived, earthOrbitalSpeedKmh, footnote, _integerFormat (+18 more)

### Community 239 - "todoSortDebugLoggerProvider"
Cohesion: 0.10
Nodes (20): IconData? get, Iterable, buildCatalogMenu, configurableManageMenuEntries, defaultCatalogMenuChild, defaultConfigurableManageMenuEntries, defaultEntityManageMenuEntries, entityManageMenuEntries (+12 more)

### Community 240 - "MethodResult"
Cohesion: 0.07
Nodes (28): Buttons, Cards / Containers, Checkbox, Colors, Components, Design System: Voyager, Do:, Do's and Don'ts (+20 more)

### Community 241 - "voyager_spacing.dart"
Cohesion: 0.05
Nodes (46): bucketListItemsProvider, bucketListRepositoryProvider, accentColor, _adding, _addItem, BucketListPopup, _BucketListPopupState, _BucketListRow (+38 more)

### Community 242 - "cadence"
Cohesion: 0.07
Nodes (29): FocusNode get, accentColor, build, color, controller, createState, didUpdateWidget, dispose (+21 more)

### Community 243 - "geometric_texture_settings.dart"
Cohesion: 0.06
Nodes (44): leetcodeProblemsProvider, leetcodeQuestionCountsProvider, _ActivityDetailCard, _ActivityDetailCardState, anchorRect, build, _close, _closing (+36 more)

### Community 244 - "GpuSurfaceTexture"
Cohesion: 0.08
Nodes (24): accentColor, blossoms, build, createState, _dashed, _dashOff, _dashOn, haloColor (+16 more)

### Community 245 - "calendarNavigateRightKey"
Cohesion: 0.01
Nodes (141): _FakeLeetCodeApi, JournalWriteCoordinator, LeetCodeTrackDraft? draft,
  String?, FirestorePullService, RemoteSyncService, close, _endpoint, fetchBySlug (+133 more)

### Community 246 - "collection"
Cohesion: 0.50
Nodes (3): 1. The Sparklines (Consecutive Trackers), 2. The Heatmap Grid, 3. Calendar View

### Community 247 - "_HeatmapWeekBlock"
Cohesion: 0.07
Nodes (27): RemoteSyncService, _addSolution, _algorithmField, _algorithmHint, _algorithmText, _bruteForce, button, db (+19 more)

### Community 248 - "dart:math"
Cohesion: 0.10
Nodes (21): devLeafGalleryPanelOpenProvider, petalFieldParamsProvider, LeafDesign, _angles, build, color, _count, design (+13 more)

### Community 249 - "_AddListMenuItem"
Cohesion: 0.08
Nodes (23): double?, build, dark, fill, icon, size, stroke, weatherDuotoneIconData (+15 more)

### Community 251 - "_MonthGridBox"
Cohesion: 0.11
Nodes (17): _collapseBlankLines, cut, _decodeHtmlEntities, _dropFromFirstExample, _dropTrailingSections, _exampleHeading, examples, headings (+9 more)

### Community 252 - "package:voyager/core/sync/text_delta_injector.dart"
Cohesion: 0.04
Nodes (46): _allDayFocusNode, build, _buildPayload, _calendarFlagColor, _calendarId, calendars, _closingAfterSave, _colorValue (+38 more)

### Community 253 - "_TrackerDialog"
Cohesion: 0.18
Nodes (11): NSObject, PigeonParser, +getManualList, +getPigeonAuthCredentialtoken, +getPigeonDetails, +getPigeonTotpSecret, +getPigeonUserCredentialFromAuthResultauthorizationCode, +getPigeonUserCredentialFromFIRUser (+3 more)

### Community 254 - "VoyagerColors"
Cohesion: 0.04
Nodes (44): LeetCodeSolution? get, algorithm, byCreated, bySolved, code, codeLanguage, compareLeetCodeProblemsNewestFirst, copyWith (+36 more)

### Community 257 - "_YearGridBox"
Cohesion: 0.06
Nodes (34): autoExpand, copyWith, false, fromJson, hashCode, hasMatch, i, id (+26 more)

### Community 258 - "@visibleForTesting"
Cohesion: 0.13
Nodes (14): DWORD, queue, HWND, LPARAM, LRESULT, UINT, WPARAM, RegisterWithRegistrar (+6 more)

### Community 259 - "dart:async"
Cohesion: 0.08
Nodes (23): build, _canImport, _controller, createState, deckId, dispose, _import, importedCount (+15 more)

### Community 260 - "package:voyager/core/theme/voyager_theme.dart"
Cohesion: 0.07
Nodes (32): Calendar, applicationId, applicationUrl, categoryId, changedAt, colorValue, company, copyWith (+24 more)

### Community 261 - "_MorphAnimationLayer"
Cohesion: 0.05
Nodes (41): int? get, addExerciseToDay, createExercise, deleteExercise, deletePlanEntry, movePlanEntry, _ref, saveExercise (+33 more)

### Community 262 - "_QuickDelayedDragStartListener"
Cohesion: 0.05
Nodes (29): MessageHandler, MethodCallHandler, GenerateInterfaces, MultiFactoResolverHostApi, MultiFactorTotpHostApi, MultiFactorTotpSecretHostApi, BasicMessageChannel, codec_ (+21 more)

### Community 263 - "_CalendarWeekTimelineState"
Cohesion: 0.07
Nodes (28): cacheGeneration, clear, degreeGridArgb, _entries, from, generation, gradientStartHour, hashCode (+20 more)

### Community 264 - "fake_weather_api_client.dart"
Cohesion: 0.13
Nodes (16): InternalTotpSecret, InternalTotpSecret, code_interval_seconds, code_length, enrollment_completion_deadline, FromEncodableList, Hash, hashing_algorithm (+8 more)

### Community 265 - "accentColor"
Cohesion: 0.06
Nodes (35): accent, color, createState, didChangeDependencies, dispose, _formatRemaining, _LiveDot, _LiveDotState (+27 more)

### Community 266 - "addedAt"
Cohesion: 0.06
Nodes (31): _activeEditRange, build, _cachedSpans, controller, createState, descenderClearance, didChangeDependencies, didUpdateWidget (+23 more)

### Community 267 - "alertOnPeriodicPrompts"
Cohesion: 0.05
Nodes (42): _, authRepositoryProvider, geometricShaderProvider, todoListStatsProvider, build, createState, dispose, _emailController (+34 more)

### Community 268 - "alertTimeHour"
Cohesion: 0.09
Nodes (21): backText, cardId, colorValue, copyWith, deckId, dueAt, ease, fromJson (+13 more)

### Community 269 - "allocatedAt"
Cohesion: 0.03
Nodes (97): Axis, ConsumerWidget, activeWorkoutPlanProvider, cacheStatusSnapshotProvider, devSettingsProvider, exercisesProvider, fpsMonitorProvider, workoutPlanEntriesProvider (+89 more)

### Community 270 - "amountCents"
Cohesion: 0.06
Nodes (30): clearLog, _enqueue, log, _logFileName, logFilePath, _maxLogBytes, readLog, _writeChain (+22 more)

### Community 271 - "anchorDueDate"
Cohesion: 0.09
Nodes (31): allJournalEntriesProvider, analyticsServiceProvider, AnalyticsPage, build, _calendarViewMonthlyBaseYearProvider, _calendarViewYearlyBaseYearProvider, _calendarViewYearProvider, _ConsecutiveCalendarChart (+23 more)

### Community 272 - "asOf"
Cohesion: 0.09
Nodes (21): bundled, candidates, candidatesComplete, complete, contains, custom, customMatches, dictionarySearchLimit (+13 more)

### Community 273 - "assetId"
Cohesion: 0.06
Nodes (30): applySettings, clearTimer, collection, direction, _displayDuration, dispose, event, eventFor (+22 more)

### Community 274 - "birthYear"
Cohesion: 0.04
Nodes (45): authRepositoryProvider, build, createState, DevRemotePurgeSection, _DevRemotePurgeSectionState, dispose, _entryIdController, _purgeEntry (+37 more)

### Community 275 - "body"
Cohesion: 0.05
Nodes (43): AppLifecycleListener?, configureDesktopWindow, desktopWindowChromeActive, _desktopWindowConfigured, _away, _controller, dispose, _focusedSingleLineEditable (+35 more)

### Community 276 - "boolValue"
Cohesion: 0.12
Nodes (15): AuthStateListener, FirebaseAuth, FirebasePluginPlatform, FirebaseService, IdTokenListener, AuthStateChannelStreamHandler, Override, IdTokenChannelStreamHandler (+7 more)

### Community 277 - "cadence"
Cohesion: 0.07
Nodes (33): vector, ReplyManager::SendResponseData(), EngineMethodResult, codec_, reply_manager_, BinaryReply, string, T (+25 more)

### Community 278 - "calendarId"
Cohesion: 0.17
Nodes (10): crossfade, reduced, VoyagerMotion, _buffered, _maxBuffered, notify, notifyOne, onWrite (+2 more)

### Community 279 - "calendarNavigateLeftKey"
Cohesion: 0.11
Nodes (21): InternalActionCodeInfoData, InternalActionCodeInfo, unique_ptr, InternalActionCodeInfo, data_, FromEncodableList, Hash, operation_ (+13 more)

### Community 280 - "calendarNavigateRightKey"
Cohesion: 0.20
Nodes (9): candidates, edits1, generateSuggestions, _letterChars, list, lower, _maxDistance2Explored, maxResults (+1 more)

### Community 281 - "collection"
Cohesion: 0.06
Nodes (36): CustomPainter, _ConfettiPainter, _VimOverlayPainter, _ClockDialPainter, GeometricTexturePainter, _GlassBorderPainter, _PetalPainter, _SelectionHighlightPainter (+28 more)

### Community 282 - "collectionName"
Cohesion: 0.08
Nodes (20): EncodableValueVariant, any, decodeColorPaletteJson, encodeColorPaletteJson, formatColorHex, jsonEncode, normalizeColorValue, normalized (+12 more)

### Community 283 - "colorPaletteJson"
Cohesion: 0.12
Nodes (18): WindowProcDelegate, GetRegistrar(), FlutterDesktopPluginRegistrarRef, T, FlutterViewId, HWND, LPARAM, LRESULT (+10 more)

### Community 284 - "colorValue"
Cohesion: 0.10
Nodes (20): attempted, attemptedFraction, attemptedPercent, CacheItemState, cacheStateColor, cacheStateLabel, cacheStatusFromAsync, cacheStatusFromWarmup (+12 more)

### Community 285 - "completed"
Cohesion: 0.10
Nodes (20): alt, binding, control, _displayToken, formatKeyBinding, join, KeyBinding, keyBindingFromKeyEvent (+12 more)

### Community 286 - "createdAt"
Cohesion: 0.14
Nodes (13): at, buffer, composing, formatEditUpdate, i, mapped, normalizeNewlines, NormalizeNewlinesFormatter (+5 more)

### Community 287 - "customQuote"
Cohesion: 0.33
Nodes (5): Blossoms, Bucket List Popup, Fallen Leaves, Swing And Bubble, Unified Canvas

### Community 288 - "customStartupPage"
Cohesion: 0.04
Nodes (48): bringCursorIntoView, buildVoyagerSpellCheckConfiguration, canAddSnippet, containerOf, copyWith, editableState, editableTextStateOf, forceSpellCheckDisplay (+40 more)

### Community 289 - "defaultBool"
Cohesion: 0.02
Nodes (104): CustomPaint, dart:io, DateTime? deletedAt,
  int, Directory, File, MoodGradientSlider, package:drift/native.dart, package:flutter_test/flutter_test.dart (+96 more)

### Community 290 - "_CalendarInteractiveEventTapState"
Cohesion: 0.38
Nodes (7): StreamHandlerCancel, StreamHandlerListen, T, unique_ptr, StreamHandlerFunctions, on_cancel_, on_listen_

### Community 291 - "_AnalyticsSectionState"
Cohesion: 0.15
Nodes (13): _accentColor, build, _controller, createState, DemoPage, _DemoPageState, dispose, _grassColor (+5 more)

### Community 292 - "NotificationPopoverWarmup"
Cohesion: 0.12
Nodes (15): body, byCreated, byDate, compareDreamEntriesNewestFirst, copyWith, DreamEntry, entryDate, fromJson (+7 more)

### Community 294 - "_revealEvent"
Cohesion: 0.07
Nodes (27): main, ops, pos, s, allOps, main, merger, opsA (+19 more)

### Community 295 - "_revealTask"
Cohesion: 0.06
Nodes (32): JobCategory, JobCompany, JobsActions, JobsActions get, _actions, category, _CategoryTile, companies (+24 more)

### Community 296 - "_StableViewScrollPosition"
Cohesion: 0.12
Nodes (15): FirebaseAuthUserHostApi, Delete, GetIdToken, LinkWithCredential, LinkWithProvider, ReauthenticateWithCredential, ReauthenticateWithProvider, Reload (+7 more)

### Community 297 - "accentColor"
Cohesion: 0.07
Nodes (26): CapsLockIndicatorScope, enabled, of, updateShouldNotify, VimSession, package:voyager/core/caps_lock/caps_lock_caret_indicator.dart, package:voyager/core/caps_lock/caps_lock_indicator_scope.dart, package:voyager/core/caps_lock/caps_lock_state.dart (+18 more)

### Community 298 - "addedAt"
Cohesion: 0.15
Nodes (12): 1. How It Should Be Done (In-Memory Stack), 2. How the UI Should Be Adjusted, 3. The Edge Cases You Must Account For, Additional Notes, Cramming Mode, Deck Workbench, Global Study Hub, Multi-Select and Batch Actions (+4 more)

### Community 299 - "alertOnPeriodicPrompts"
Cohesion: 0.27
Nodes (9): ResultHandlerError, ResultHandlerNotImplemented, ResultHandlerSuccess, string, T, MethodResultFunctions, on_error_, on_not_implemented_ (+1 more)

### Community 300 - "workout_overlay.dart"
Cohesion: 0.06
Nodes (32): after, before, _canRedo, _canUndo, _cardKey, _clearHistory, createState, _deleteAndAdvance (+24 more)

### Community 301 - "allocatedAt"
Cohesion: 0.17
Nodes (11): Accessibility & Inclusion, Brand Commitments, Capabilities and Constraints, Evidence on Hand, Operating Context, Platform, Positioning, Product (+3 more)

### Community 302 - "amountCents"
Cohesion: 0.14
Nodes (13): cancelLabel, confirmed, confirmLabel, deleteAllLabel, DeleteContainerChoice, moveLabel, result, showConfirmDialog (+5 more)

### Community 303 - "anchorDueDate"
Cohesion: 0.50
Nodes (4): -checkActionCodeAppcodecompletion, +operationFromRequestType, -parseActionCode, -resolveActionCodeOperationForAppcodefallbackInfocompletion

### Community 304 - "build"
Cohesion: 0.14
Nodes (12): AccessibilityMode, GpuPreference, UIThreadPolicy, DartProject, aot_library_path_, assets_path_, dart_entrypoint_, dart_entrypoint_arguments_ (+4 more)

### Community 305 - "InternalFirebaseAuthSettings"
Cohesion: 0.07
Nodes (26): FLTPhoneNumberVerificationStreamHandler, -initWithAuthrequest, -initWithAuthrequestsessionfactorInfo, -onCancelWithArguments, -onListenWithArgumentseventSink, FLTPhoneNumberVerificationStreamHandler, FlutterStreamHandler, NSObject (+18 more)

### Community 306 - "birthDate"
Cohesion: 0.09
Nodes (20): nanoseconds, FlutterDesktopEngineRef, FlutterDesktopPluginRegistrarRef, function, HWND, LPARAM, LRESULT, optional (+12 more)

### Community 307 - "birthYear"
Cohesion: 0.10
Nodes (36): Point, OnCreate, HWND, LPARAM, LRESULT, RECT, Size, UINT (+28 more)

### Community 308 - "body"
Cohesion: 0.02
Nodes (93): AppSettings?, AsyncNotifier, SettingsNotifier, AppSettings, _apiKeyController, build, createState, didUpdateWidget (+85 more)

### Community 309 - "journal_write_coordinator.dart"
Cohesion: 0.12
Nodes (16): _cardPool, cards, colors, deck, deckId, deckName, _deckTopics, now (+8 more)

### Community 310 - "cadence"
Cohesion: 0.22
Nodes (8): Dashboard, Detailed View, LeetCode Tracker, Page Architecture & Layout, Review Deck, Review Sessions, The Flashcard, Tracking Flow

### Community 311 - "calendarId"
Cohesion: 0.10
Nodes (12): ActionCodeResult, ActionCodeInfoOperation, EMAIL_SIGN_IN, PASSWORD_RESET, RECOVER_EMAIL, REVERT_SECOND_FACTOR_ADDITION, UNKNOWN, VERIFY_AND_CHANGE_EMAIL (+4 more)

### Community 312 - "calendarNavigateLeftKey"
Cohesion: 0.05
Nodes (39): variant, FlutterDesktopPluginRegistrarRef, FirebaseAuthPluginCApiRegisterWithRegistrar(), ErrorOr, v_, FlutterError, code_, details_ (+31 more)

### Community 313 - "InternalAuthCredentialInput"
Cohesion: 0.08
Nodes (23): DevSettingsController, FpsMonitorController, JournalDebugLogger, SyncCompareLogger, begin, complete, fail, stateFor (+15 more)

### Community 314 - "collection"
Cohesion: 0.09
Nodes (23): LeetCodeSolution, anchorRect, build, _close, _closing, _ComplexityTile, _controller, createState (+15 more)

### Community 315 - "collectionName"
Cohesion: 0.13
Nodes (19): remoteSyncCompareServiceProvider, syncCompareLoggerProvider, build, _compareAllTodoLists, _compareJournalEntries, _compareTodoList, _comparing, createState (+11 more)

### Community 316 - "study_keyboard_shortcuts.dart"
Cohesion: 0.07
Nodes (27): A — Titles match → draft opened, Add / remove rows, B — Titles do not match → fresh GraphQL form opened, draft still on disk, Baseline refreshes, C — GraphQL failed (or no username / no submission) and draft opened, Close / discard / replace, Decision log, Draft toasts (+19 more)

### Community 317 - "colorValue"
Cohesion: 0.07
Nodes (26): a, algorithm, b, code, codeLanguage, description, difficulty, examples (+18 more)

### Community 318 - "study_rich_text.dart"
Cohesion: 0.06
Nodes (34): a, addDays, addDaysKeepingTime, _addMonthsClamped, _addYearsClamped, after, anchorLocal, _clampDay (+26 more)

### Community 319 - "completedAt"
Cohesion: 0.29
Nodes (6): Active Workout, Detailed Exercise View, Global Voyager Integrations, Miscellaneous, Planner View, Workout Tracker

### Community 320 - "_LifeTrackerPageState"
Cohesion: 0.08
Nodes (23): build, _canSave, _colorValue, container, createState, _datePopoverOpen, dispose, existing (+15 more)

### Community 321 - "MethodChannel"
Cohesion: 0.07
Nodes (28): DateTime? recurrenceEndDate,
  List, DateTime? recurrenceEndDate,
  String, package:voyager/domain/services/calendar_recurrence.dart, package:voyager/domain/services/recurrence_engine.dart, required DateTime end,
  RecurrenceRule, d, end, exceptionDates (+20 more)

### Community 322 - "app_router.dart"
Cohesion: 0.07
Nodes (25): package:flutter_code_editor/flutter_code_editor.dart, package:voyager/core/widgets/field_scroll_padding.dart, package:voyager/core/widgets/labeled_text_field.dart, package:voyager/core/widgets/tag_highlighted_text_field.dart, support/search_page_harness.dart, main, _pumpField, pumpWidget (+17 more)

### Community 323 - "spell_check_overlay_alignment_test.dart"
Cohesion: 0.09
Nodes (20): StatisticTracker, build, HeatmapCalendar, month, tracker, values, package:voyager/domain/services/analytics_service.dart, analytics (+12 more)

### Community 324 - "BinaryMessengerImpl"
Cohesion: 0.10
Nodes (18): GeneratedPluginRegistrant, Keep, FlutterEngine, engine_, GetRegistrarForPlugin, next_frame_callback_, owns_engine_, ProcessExternalWindowMessage (+10 more)

### Community 325 - "-getFIRAuthCredentialFromArgumentsappcompletion"
Cohesion: 0.11
Nodes (18): 1-D Dynamic Programming, 2-D Dynamic Programming, Advanced Graphs, Arrays & Hashing, Backtracking, Binary Search, Bit Manipulation, Graphs (+10 more)

### Community 326 - "AuthPigeonFirebaseApp"
Cohesion: 0.08
Nodes (24): 2, applyStudyGrade, _clampEase, copyWith, _dateOnly, days, due, ease (+16 more)

### Community 327 - "shell_page_transition.dart"
Cohesion: 0.07
Nodes (28): package:voyager/features/leetcode/leetcode_flashcard.dart, package:voyager/features/leetcode/leetcode_inline_code.dart, package:voyager/features/leetcode/leetcode_mini_flashcard.dart, String? description,
  String, algorithm, _allText, codeLanguage, explanation (+20 more)

### Community 330 - "FLTIdTokenChannelStreamHandler"
Cohesion: 0.20
Nodes (11): todoSortDebugLoggerProvider, build, DevTodoSortDebugSection, _showLogDialog, _applyOptimisticActiveOrder, _clearTaskDueDate, _moveTaskToBottom, _reorderActiveTasks (+3 more)

### Community 333 - "FLTAuthStateChannelStreamHandler"
Cohesion: 0.05
Nodes (40): studyFolderByIdProvider, display, divisions, _GeometricTextureSlider, label, max, min, onChanged (+32 more)

### Community 337 - "stat_number_format_test.dart"
Cohesion: 0.07
Nodes (26): JobRepository get, addCategory, addSeason, addStage, createApplication, deleteApplication, deleteCategory, deleteCompany (+18 more)

### Community 338 - "rubber_band.dart"
Cohesion: 0.50
Nodes (3): magnitude, projectMomentum, rubberBand

### Community 339 - "FlutterError"
Cohesion: 0.15
Nodes (15): NSArray, NSDictionary, InternalAdditionalUserInfo, -init, +makeWithIsNewUserproviderIdusernameauthorizationCodeprofile, InternalIdTokenResult, +makeWithTokenexpirationTimestampauthTimestampissuedAtTimestampsignInProviderclaimssignInSecondFactor, InternalSignInProvider (+7 more)

### Community 340 - "AuthPigeonFirebaseApp"
Cohesion: 0.12
Nodes (15): 10. Decisions locked in, 1. Goals, 2. What exists today, 3. Product model, 4. Why the full bundled list is in scope, 5.1 Settings entry, 5.2 Dialog, 5.3 Misspelling popup (+7 more)

### Community 341 - "_HeatmapDayCell"
Cohesion: 0.50
Nodes (3): resolveNewItemTarget, shortDestinationName, trimmed

### Community 342 - "leetcode_tag_matrix.dart"
Cohesion: 0.08
Nodes (24): accentColor, _boxKey, build, _celebrate, celebrateOnComplete, CheckMarkPainter, color, _controller (+16 more)

### Community 343 - "plugin_registrar.cc"
Cohesion: 0.18
Nodes (10): DateTime? dueAt,
  int, package:voyager/domain/services/leetcode_srs_engine.dart, dueAt, ease, id, interval, main, _now (+2 more)

### Community 344 - "workout_test.dart"
Cohesion: 0.09
Nodes (23): accentColor, autofocus, build, companies, controller, createState, dispose, _fieldKey (+15 more)

### Community 345 - "InternalUserInfo"
Cohesion: 0.17
Nodes (11): charOpEncodeIsolateThreshold, charOps, CharOpsEncodeRequest, chunks, compute, encodeCharOpPayloads, encodeCharOpPayloadsSync, groupId (+3 more)

### Community 346 - "AuthPigeonFirebaseApp"
Cohesion: 0.02
Nodes (80): anchor, _BreakdownRow, build, child, count, counts, date, difficulty (+72 more)

### Community 347 - "mood_gradient_slider.dart"
Cohesion: 0.05
Nodes (36): IconData?, accentColor, build, child, dense, ellipsize, fillWhenActive, icon (+28 more)

### Community 348 - "shell_weather_widget_test.dart"
Cohesion: 0.18
Nodes (10): colorForLeetCodeDifficulty, key, kLeetCodeEasyColor, kLeetCodeHardColor, kLeetCodeMediumColor, labelForLeetCodeDifficulty, labelForLeetCodeLanguage, leetCodeCodeLanguages (+2 more)

### Community 349 - "MethodResultFunctions"
Cohesion: 0.12
Nodes (15): 1. Desired product behavior, 2.1 Ctrl/Cmd+Z, 2.2 Why the field undo stack alone is not enough, 2. What works today, 3.1 Vim Normal `u`, 3.2 Why `u` was left out, 3. What does not work today, 4.1 Behavior (+7 more)

### Community 350 - "-checkActionCodeAppcodecompletion"
Cohesion: 0.11
Nodes (13): FlutterDesktopViewControllerRef, FlutterDesktopViewRef, FlutterViewController, controller_, ForceRedraw, HandleTopLevelWindowProc, view_id, shared_ptr (+5 more)

### Community 351 - "String get"
Cohesion: 0.15
Nodes (12): analyticsList, devList, financeInsights, financeLedgerNarrow, financeLedgerWide, journalEntryList, journalEntryListAll, journalPreview (+4 more)

### Community 352 - "-verifyPhoneNumberApprequestcompletion"
Cohesion: 0.12
Nodes (16): description, difficulty, easy, examples, hard, LeetCodeApiQuestion, LeetCodeQuestionCounts, medium (+8 more)

### Community 353 - "firebase_auth_errors.dart"
Cohesion: 0.11
Nodes (17): Acceptance criteria, Architecture (implementation sketch), Context menu, Data & validation, Decisions log (from product), Edge-case summary, Field scope, Goal (+9 more)

### Community 354 - "vim_enabled_scope.dart"
Cohesion: 0.11
Nodes (16): App, Credential, Future, AdditionalUserInfo, EncodableMap, InternalAdditionalUserInfo, map, Variant (+8 more)

### Community 355 - "_MonthWeekMorphLayer"
Cohesion: 0.17
Nodes (10): FlutterViewId, HWND, LPARAM, LRESULT, optional, UINT, WPARAM, FlutterViewController::FlutterViewController() (+2 more)

### Community 356 - "MultiFactorUserHostApi"
Cohesion: 0.11
Nodes (18): AnimationController, build, createState, didChangeDependencies, didUpdateWidget, dispose, ease, enabled (+10 more)

### Community 357 - "leetcode_constants.dart"
Cohesion: 0.12
Nodes (16): devGeometricWavePanelOpenProvider, geometricWaveParamsProvider, GeometricWaveShape, build, DevGeometricWaveSection, display, divisions, label (+8 more)

### Community 358 - "Timer?"
Cohesion: 0.09
Nodes (21): byKey, counts, days, every, filterJobApplications, filterJobCompanies, haystack, jobCompanyKey (+13 more)

### Community 359 - "SoftDeletable"
Cohesion: 0.04
Nodes (45): buildExerciseHistory, byDay, colorValue, completed, completedAt, copyWith, cycleAnchor, cycleLength (+37 more)

### Community 360 - "selector_pill.dart"
Cohesion: 0.12
Nodes (18): _createTracker, _delete, _deleteTracker, _editTracker, _handleTap, _HeatmapBucket, _HeatmapBucketState, _heatmapDraggingProvider (+10 more)

### Community 361 - "StreamHandlerFunctions"
Cohesion: 0.12
Nodes (15): ActiveTagToken, counts, end, filterTagSuggestions, _hashCodeUnit, hashIndex, _isTagChar, _isWordChar (+7 more)

### Community 362 - "wWinMain"
Cohesion: 0.40
Nodes (4): Additional Notes, Icon UI:, Popover Layout, UX Of Clearing

### Community 363 - "build"
Cohesion: 0.08
Nodes (22): CalendarEvent, CalendarDayEntry, build, CalendarDayEntryBar, date, entry, fontSize, height (+14 more)

### Community 364 - "spell_check_suggestions_test.dart"
Cohesion: 0.08
Nodes (23): _deviceId, fetchForecastIfNeeded, isCacheStale, isForecastCacheStale, _lockDuration, mergeForecastLocally, _persistCache, _persistForecast (+15 more)

### Community 365 - "calendar_days_test.dart"
Cohesion: 0.03
Nodes (61): AnimatedContainer, package:drift/drift.dart, package:flutter/gestures.dart, package:voyager/core/constants/todo_constants.dart, package:voyager/core/theme/voyager_list_item_surface.dart, package:voyager/features/shell/reveal_request.dart, support/todo_page_harness.dart, main (+53 more)

### Community 366 - "life_tracker_models.dart"
Cohesion: 0.12
Nodes (15): _auto, empty, from, hasAuto, hashCode, hasManual, isEmpty, _manual (+7 more)

### Community 367 - "tag_suggestions.dart"
Cohesion: 0.12
Nodes (16): InheritedWidget, SnippetEnabledScope, enabled, _isFreeTextKeyboard, obscureText, of, readOnly, true (+8 more)

### Community 368 - "TEST"
Cohesion: 0.09
Nodes (34): calendarRepositoryProvider, deviceIdProvider, devSettingsProvider, _StatisticDetailPopup, build, _confirmDeleteAllCalendarEvents, devDisablePetalFieldProvider, devForceOfflineProvider (+26 more)

### Community 369 - "int?"
Cohesion: 0.22
Nodes (8): 1. Mode Switching (The State Anchors), 2. Insert Mode Triggers (Drops control to Flutter), 3. Immediate Motions (Standard Jumps), 4. Transient Motions (The "Find" state), 5. Memory State Triggers (Fast Repetition), 6. Verbs / Operators (The "Pending" state), 7. Instant Actions (No motions required), 8. Search State

### Community 370 - "Intent"
Cohesion: 0.19
Nodes (19): colorPaletteProvider, jobApplicationsProvider, jobCategoriesProvider, jobCompaniesProvider, jobSearchQueryProvider, jobSeasonsProvider, jobStagesProvider, jobStatusFilterProvider (+11 more)

### Community 372 - "build"
Cohesion: 0.12
Nodes (17): build, CalendarKeyboardShortcuts, _CalendarKeyboardShortcutsState, calendarNavShortcutsEnabled, child, createState, dispose, _handleKeyEvent (+9 more)

### Community 373 - "_HeatmapSquare"
Cohesion: 0.67
Nodes (4): CustomClipper, _WeekTimedViewportClipper, _TrunkAndBranchesClipper, Path?

### Community 374 - "analytics_service.dart"
Cohesion: 0.05
Nodes (36): Curve, double get, animateTo, controller, defaultSpring, dispose, jumpTo, _spring (+28 more)

### Community 375 - "InternalUserInfo"
Cohesion: 0.05
Nodes (37): Duration?, Duration get, cancel, DebouncedCallback, debounceDelay, Debouncer, delay, dispose (+29 more)

### Community 376 - "CalendarRepository"
Cohesion: 0.12
Nodes (16): build, _CreateNameColorDialog, _CreateNameColorDialogState, createState, dispose, initialColor, initState, _nameController (+8 more)

### Community 377 - "_YearGridBox"
Cohesion: 0.06
Nodes (34): _append, applySettings, _appStartMarker, bodyDraftEntryIds, bodyFocused, bodyText, clearLog, enabled (+26 more)

### Community 378 - "PetalField"
Cohesion: 0.15
Nodes (14): InternalAuthCredentialInput, InternalAuthCredentialInput, access_token, FromEncodableList, Hash, operator==, provider_id_, set_access_token (+6 more)

### Community 379 - "_StableViewScrollPosition"
Cohesion: 0.02
Nodes (121): EditableTextState, FocusNode, loadDictionaryFromAssets, raw, VoyagerSpellCheckService, package:flutter/rendering.dart, package:flutter/services.dart, package:voyager/core/snippets/snippet_enabled_scope.dart (+113 more)

### Community 380 - "allDreamEntriesProvider"
Cohesion: 0.07
Nodes (27): allOps, CharacterOpRegistry, CharacterOpSession, clear, clientId, ensureSession, _insertOp, key (+19 more)

### Community 381 - "allJournalEntriesProvider"
Cohesion: 0.09
Nodes (21): dart:async, _callbacks, flushAll, instance, PendingFlushRegistry, register, unregister, _activeUntil (+13 more)

### Community 382 - "bucketListItemsProvider"
Cohesion: 0.08
Nodes (24): CloudFunctionWeatherClient, _apiKey, DevOpenWeatherClient, _http, _periodFromForecastItem, refreshForecast, refreshWeather, WeatherForecast (+16 more)

### Community 383 - "blossom_stat_popup.dart"
Cohesion: 0.13
Nodes (14): back, cards, front, inBack, lineNumber, lines, normalized, parseStudyBulkImportText (+6 more)

### Community 384 - "FLTAuthStateChannelStreamHandler"
Cohesion: 0.18
Nodes (14): User, EncodableList, InternalUserCredential, InternalUserDetails, InternalUserInfo, vector, ParseProviderData, ParseUserDetails (+6 more)

### Community 385 - "voyager_list_item_surface.dart"
Cohesion: 0.12
Nodes (14): int? current,
  Set, assign, nextColor, palette, PaletteAssigner, paletteFromItems, pickColorFromPalette, pickPaletteColorWithRef (+6 more)

### Community 386 - "VoyagerColors"
Cohesion: 0.13
Nodes (13): <MultiFactorTotpHostApi>, -generateSecretSessionIdcompletion, -getAssertionForEnrollmentSecretKeyoneTimePasswordcompletion, -getAssertionForSignInEnrollmentIdoneTimePasswordcompletion, <MultiFactorTotpSecretHostApi>, -generateQrCodeUrlSecretKeyaccountNameissuercompletion, -openInOtpAppSecretKeyqrCodeUrlcompletion, <MultiFactorUserHostApi> (+5 more)

### Community 387 - "LifeTreeCanvas"
Cohesion: 0.13
Nodes (14): Subscription, BillRadarPanel, build, _dueColor, _dueLabel, _EmptyRadar, showAnnual, subscription (+6 more)

### Community 388 - "firebase_auth_plugin.h"
Cohesion: 0.08
Nodes (24): dart:ffi, addListener, _cachedGetKeyState, _canProbe, capsLockIndicatorSupportsPlatform, debugProbe, getKeyState, _GetKeyStateDart (+16 more)

### Community 389 - "package:voyager/core/utils/journal_tags.dart"
Cohesion: 0.10
Nodes (20): copyWith, deleteRecurringEvent, editedStart, editRecurringEvent, isFirstOccurrence, local, masterStart, newStartDate (+12 more)

### Community 390 - "accentColor"
Cohesion: 0.12
Nodes (15): CharOpsPayload, applyMergedPayload, applyMergedText, _completeCharOpPayloads, _latestSnapshot, _legacySnapshot, mergeOperations, _mergeParsed (+7 more)

### Community 391 - "addedAt"
Cohesion: 0.13
Nodes (15): _addColor, build, createState, dispose, _hexController, _hexError, _hexFocusNode, palette (+7 more)

### Community 392 - "alertOnPeriodicPrompts"
Cohesion: 0.05
Nodes (40): Completer, http.BaseClient, LeetCodeRepository, DriftLeetCodeRepository, DriftSettingsRepository, SettingsRepository, StreamedResponse, db (+32 more)

### Community 393 - "alertTimeHour"
Cohesion: 0.33
Nodes (5): isCustomWordToken, normalizeCustomWord, _wholeWordToken, wordTokenPattern, RegExp

### Community 394 - "algorithm"
Cohesion: 0.08
Nodes (26): assetValuationsProvider, _asOf, _AssetModal, _AssetModalState, build, _canSave, _colorValue, container (+18 more)

### Community 396 - "amountCents"
Cohesion: 0.40
Nodes (5): _CapsLockCaretPainter, VimAnchoredChrome, ClampToTargetBounds, _SingleChildViewport, SingleChildRenderObjectWidget

### Community 397 - "anchorDueDate"
Cohesion: 0.13
Nodes (14): copyWith, EventRecurrence, frequency, hashCode, interval, isCustom, none, operator (+6 more)

### Community 398 - "asOf"
Cohesion: 0.04
Nodes (45): DecoratedBox, GlobalKey, GlobalKey get, _RenderCapsLockCaret, RenderVimAnchoredChrome, attach, createRenderObject, detach (+37 more)

### Community 399 - "assetId"
Cohesion: 0.05
Nodes (40): _, GoRouter, Key? get, auth, child, _preloadedShellPaths, routerProvider, settingsRepo (+32 more)

### Community 400 - "backText"
Cohesion: 0.05
Nodes (35): EditableText, context, focus, isTextInputFocused, subtreeIsVisible, package:voyager/core/widgets/snippet_editor.dart, button, db (+27 more)

### Community 401 - "birthDate"
Cohesion: 0.19
Nodes (13): hiddenNotificationFeedProvider, notificationRepositoryProvider, visibleNotificationFeedProvider, _addNote, _clearAll, _deleteNote, _FeedSection, _HiddenSection (+5 more)

### Community 402 - "birthYear"
Cohesion: 0.09
Nodes (22): 10. Open implementation notes (non-blocking), 1. Goals, 2.1 Setting, 2.2 Where it may appear, 2. Product model, 3. Visibility rules (normative), 4.1 Horizontal flip at the right edge, 4. Visual design (+14 more)

### Community 403 - "body"
Cohesion: 0.16
Nodes (12): journalDebugLoggerProvider, build, DevJournalDebugSection, _showLogDialog, build, count, LeetCodeTagMatrix, problems (+4 more)

### Community 404 - "boolValue"
Cohesion: 0.09
Nodes (20): DateTime?, collection, detectedAt, documentId, fromStorage, id, localPayloadJson, localText (+12 more)

### Community 405 - "cadence"
Cohesion: 0.17
Nodes (11): googleOAuthClientId, _googleOAuthClientIdRaw, googleOAuthClientSecret, _googleOAuthClientSecretRaw, googleOAuthRedirectPort, id, isGoogleOAuthClientSecretConfigured, isGoogleOAuthConfigured (+3 more)

### Community 406 - "calendarId"
Cohesion: 0.14
Nodes (12): bool get, isAndroid, isLinux, isWindows, VoyagerPlatform, copyWithDeleted, createdAt, deletedAt (+4 more)

### Community 407 - "calendarNavigateLeftKey"
Cohesion: 0.13
Nodes (14): build, _captured, createState, current, dispose, _handleKeyEvent, initState, _KeyBindingDialog (+6 more)

### Community 408 - "calendarNavigateRightKey"
Cohesion: 0.15
Nodes (13): lifeTrackerStatsProvider, accentColor, BlossomStatPopup, build, _lineWidth, _padding, painter, stat (+5 more)

### Community 409 - "cardId"
Cohesion: 0.40
Nodes (6): _AddListMenuItem, _AddListMenuItemState, _RoundedDropdownMenuItem, _RoundedDropdownMenuItemState, Object?, PopupMenuEntry

### Community 410 - "code"
Cohesion: 0.06
Nodes (27): _set, function, TextureRegistrarImpl::UnregisterTexture(), unique_ptr, PluginRegistrar, AddPlugin, ClearPlugins, plugins_ (+19 more)

### Community 411 - "codeLanguage"
Cohesion: 0.15
Nodes (12): BackupCollection, backupContentEquals, BackupRecord, buildBackupCollections, content, data, fromJson, id (+4 more)

### Community 412 - "collection"
Cohesion: 0.14
Nodes (13): active, data, disabled, enabled, expandKey, hashCode, index, of (+5 more)

### Community 413 - "addedAt"
Cohesion: 0.10
Nodes (21): build, LeetCodePage, _LeetCodeViewMode, _leetCodeViewModeProvider, build, _CompletionRow, _CompletionRowState, count (+13 more)

### Community 414 - "alertOnPeriodicPrompts"
Cohesion: 0.03
Nodes (70): settingsProvider, build, canManage, createState, _error, initialTrigger, messenger, openSettings (+62 more)

### Community 416 - "algorithm"
Cohesion: 0.09
Nodes (22): byDay, countFor, days, difficulty, easy, hard, kLeetCodeActivityWindowDays, leetCodeBusiestDayInYear (+14 more)

### Community 417 - "allocatedAt"
Cohesion: 0.14
Nodes (12): _authorizationEndpoint, DesktopGoogleOAuth, redirectUri, _tokenEndpoint, package:oauth2/oauth2.dart, package:url_launcher/url_launcher.dart, package:voyager/core/constants/google_auth_config.dart, package:voyager/data/remote/desktop_google_oauth.dart (+4 more)

### Community 418 - "amountCents"
Cohesion: 0.12
Nodes (16): double? contentWidth,
  bool, build, contentWidth, _controller, createState, dispose, enterToSubmit, initial (+8 more)

### Community 419 - "anchorDueDate"
Cohesion: 0.15
Nodes (12): contains, embedded, firebaseAuthErrorMessage, invalidCredentialHints, _isGenericInternalMessage, lower, _messageFromEmbeddedDetails, normalizedCode (+4 more)

### Community 420 - "asOf"
Cohesion: 0.15
Nodes (13): build, _controller, createState, didUpdateWidget, dispose, _error, initState, _lastUpdatedLabel (+5 more)

### Community 421 - "assetId"
Cohesion: 0.14
Nodes (13): package:voyager/domain/todo/todo_task_sorting.dart, required String id,
  bool, byId, dueDate, dueDateSetAt, id, main, now (+5 more)

### Community 422 - "backText"
Cohesion: 0.04
Nodes (48): BackdropFilter, build, _canSave, _CategoryModal, _colorValue, container, createState, dispose (+40 more)

### Community 423 - "birthDate"
Cohesion: 0.12
Nodes (15): package:voyager/core/constants/job_constants.dart, package:voyager/domain/jobs/job_queries.dart, package:voyager/domain/models/job_models.dart, String? id,
  String, app, company, main, now (+7 more)

### Community 424 - "birthYear"
Cohesion: 0.50
Nodes (3): package:voyager/core/spellcheck/dictionary_search.dart, _bundled, main

### Community 425 - "body"
Cohesion: 0.50
Nodes (4): FLTAuthStateChannelStreamHandler, -initWithAuth, -onCancelWithArguments, -onListenWithArgumentseventSink

### Community 427 - "cadence"
Cohesion: 0.50
Nodes (3): jobDefaultStage, jobSeedCompanies, jobSeedStages

### Community 428 - "calendarId"
Cohesion: 0.50
Nodes (4): FLTIdTokenChannelStreamHandler, -initWithAuth, -onCancelWithArguments, -onListenWithArgumentseventSink

### Community 429 - "calendarNavigateLeftKey"
Cohesion: 0.13
Nodes (13): <MultiFactorTotpHostApi>, -generateSecretSessionIdcompletion, -getAssertionForEnrollmentSecretKeyoneTimePasswordcompletion, -getAssertionForSignInEnrollmentIdoneTimePasswordcompletion, <MultiFactorTotpSecretHostApi>, -generateQrCodeUrlSecretKeyaccountNameissuercompletion, -openInOtpAppSecretKeyqrCodeUrlcompletion, <MultiFactorUserHostApi> (+5 more)

### Community 430 - "calendarNavigateRightKey"
Cohesion: 0.50
Nodes (4): FLTAuthStateChannelStreamHandler, -initWithAuth, FlutterStreamHandler, NSObject

### Community 433 - "study_grade_during_flip_test.dart"
Cohesion: 0.09
Nodes (21): IgnorePointer, package:voyager/features/study/study_flip_card.dart, package:voyager/features/study/study_keyboard_shortcuts.dart, package:voyager/features/study/study_session_page.dart, main, _pumpCard, pumpWidget, cards (+13 more)

### Community 434 - "VimMode"
Cohesion: 0.17
Nodes (12): @visibleForTesting, renderEditableOf, sync, restoreIfSelectAll, handleCharacter, calendarNavDeltaForEvent, calendarNavShortcutsEnabledForState, applyLeetCodeCodeEdits (+4 more)

### Community 436 - "GpuSurfaceTexture"
Cohesion: 0.39
Nodes (5): FlutterDesktopGpuSurfaceDescriptor, FlutterDesktopGpuSurfaceType, ObtainDescriptorCallback, GpuSurfaceTexture, obtain_descriptor_callback_

### Community 438 - "GLASS_BUTTON.md"
Cohesion: 0.20
Nodes (9): 1. Basic Icon + Text Button (Toolbar / App Header), 2. Custom Size & Recolor (Primary Action), 3. Icon-Only Glass Button, API Reference, File Locations, GlassButton, Key Features, Parameters (+1 more)

### Community 439 - "PluginRegistrarManager"
Cohesion: 0.03
Nodes (95): K, FromEncodableList, operator==, AuthPigeonFirebaseApp, EncodableList, InternalActionCodeSettings, InternalAdditionalUserInfo, InternalAuthCredential (+87 more)

### Community 440 - "study_keyboard_shortcuts.dart"
Cohesion: 0.15
Nodes (14): InternalUserProfile, InternalUserProfile, display_name, display_name_changed, FromEncodableList, Hash, operator==, photo_url (+6 more)

### Community 441 - "@visibleForTesting"
Cohesion: 0.13
Nodes (15): <FirebaseAuthUserHostApi>, -deleteAppcompletion, -getIdTokenAppforceRefreshcompletion, -linkWithCredentialAppinputcompletion, -linkWithProviderAppsignInProvidercompletion, -reauthenticateWithCredentialAppinputcompletion, -reauthenticateWithProviderAppsignInProvidercompletion, -reloadAppcompletion (+7 more)

### Community 442 - "_HeatmapSquare"
Cohesion: 0.20
Nodes (10): _ContextMenuLayoutDelegate, _SubmenuLayoutDelegate, _PopoverLayoutDelegate, _TextFieldContextMenuLayoutDelegate, VoyagerPopupMenuRouteLayout, _SparklineBubbleLayout, _ChartBubbleLayout, _BubbleLayout (+2 more)

### Community 443 - "<FirebaseAuthUserHostApi>"
Cohesion: 0.13
Nodes (15): <FirebaseAuthUserHostApi>, -deleteAppcompletion, -getIdTokenAppforceRefreshcompletion, -linkWithCredentialAppinputcompletion, -linkWithProviderAppsignInProvidercompletion, -reauthenticateWithCredentialAppinputcompletion, -reauthenticateWithProviderAppsignInProvidercompletion, -reloadAppcompletion (+7 more)

### Community 444 - "_HeatmapWeekBlock"
Cohesion: 0.50
Nodes (4): FLTIdTokenChannelStreamHandler, -initWithAuth, FlutterStreamHandler, NSObject

### Community 445 - "PigeonParser"
Cohesion: 0.26
Nodes (13): FIRActionCodeSettings, PigeonParser, +getManualList, +getPigeonAdditionalUserInfoauthorizationCode, +getPigeonAuthCredentialtoken, +getPigeonDetails, +getPigeonTotpSecret, +getPigeonUserCredentialFromAuthResultauthorizationCode (+5 more)

### Community 446 - "PigeonParser"
Cohesion: 0.26
Nodes (13): FIRActionCodeSettings, PigeonParser, +getManualList, +getPigeonAdditionalUserInfoauthorizationCode, +getPigeonAuthCredentialtoken, +getPigeonDetails, +getPigeonTotpSecret, +getPigeonUserCredentialFromAuthResultauthorizationCode (+5 more)

### Community 447 - "_MonthGridBox"
Cohesion: 0.15
Nodes (12): int? integerCap,
  int, boolValue, defaultInt, enumValue, epoch, integerCap, intValue, main (+4 more)

### Community 449 - "FLTFirebaseAuthPlugin"
Cohesion: 0.09
Nodes (20): FLTPhoneNumberVerificationStreamHandler, -initWithAuthrequest, -initWithAuthrequestsessionfactorInfo, -onCancelWithArguments, -onListenWithArgumentseventSink, FLTPhoneNumberVerificationStreamHandler, FlutterStreamHandler, NSObject (+12 more)

### Community 450 - "flutter_windows.h"
Cohesion: 0.18
Nodes (9): _In_, _In_opt_, wWinMain(), string, vector, wchar_t, CreateAndAttachConsole(), GetCommandLineArguments() (+1 more)

### Community 451 - "PigeonParser"
Cohesion: 0.18
Nodes (11): NSObject, PigeonParser, +getManualList, +getPigeonAuthCredentialtoken, +getPigeonDetails, +getPigeonTotpSecret, +getPigeonUserCredentialFromAuthResultauthorizationCode, +getPigeonUserCredentialFromFIRUser (+3 more)

### Community 452 - "String?"
Cohesion: 0.04
Nodes (84): leetCodeTrackDraftStoreProvider, allTodoTasksProvider, calendarEventsProvider, calendarTodoMarkersProvider, colorPaletteProvider, leetCodeRepositoryProvider, remoteSyncServiceProvider, todoListsProvider (+76 more)

### Community 456 - "firebase_options.dart"
Cohesion: 0.33
Nodes (5): Analytics, Dream Editor & Collapsible Scratchpad UX, Dream Journal, Global Theme & Visual Identity, Split-Pane Dashboard

### Community 457 - "_StableViewScrollPosition"
Cohesion: 0.10
Nodes (16): calendar, debug, journal, manage, search, VoyagerIcons, kMinTouchTarget, SnippetSettingsLauncher (+8 more)

### Community 460 - "key_binding_dialog.dart"
Cohesion: 0.15
Nodes (12): AppThemeMode, BillingPeriod, CalendarViewMode, HeatmapMode, LeetCodeDifficulty, SnippetExpandKey, StartupPageMode, TrackerCadence (+4 more)

### Community 461 - "newline_normalization.dart"
Cohesion: 0.09
Nodes (24): FlutterDesktopMessage, TextureVariant, BinaryMessengerImpl::BinaryMessengerImpl(), BinaryMessengerImpl::Send(), BinaryMessengerImpl::SetMessageHandler(), BinaryMessageHandler, BinaryReply, FlutterDesktopMessengerRef (+16 more)

### Community 462 - "<MultiFactorTotpHostApi>"
Cohesion: 0.05
Nodes (36): Duration? dwell,
  Duration, IconData? icon,
  List, build, child, createState, didUpdateWidget, _entry, initState (+28 more)

### Community 463 - "life_tracker_stat_popup_width_test.dart"
Cohesion: 0.15
Nodes (10): refreshForecast, refreshWeather, package:voyager/domain/models/weather_models.dart, package:voyager/domain/services/weather_forecast_chart.dart, package:voyager/domain/services/weather_forecast_merge.dart, main, _period, main (+2 more)

### Community 466 - "0.18.0"
Cohesion: 0.33
Nodes (6): 0.18.0, `AuthProvider` & `AuthCredential`, `FirebaseAuth`, General, `User`, `UserCredential`

### Community 467 - "Migration Guide"
Cohesion: 0.33
Nodes (6): ActionCodeSettings, Email Sign-in Methods, Email Updates, FirebaseAuth Instance, Microsoft Authentication, Migration Guide

### Community 468 - "http_callable_client.dart"
Cohesion: 0.08
Nodes (24): Client, FirebaseFunctions?, _call, _firestore, FirestoreWeatherReader, _functions, getCurrentWeather, _httpCallable (+16 more)

### Community 470 - "workout_constants.dart"
Cohesion: 0.08
Nodes (22): FirebaseAuthMultiFactorException, Component, ComponentRegistrar, FirebaseAuthException, kCycleWorkoutPlanId, kMaxReps, kMaxSets, kMaxWeightKg (+14 more)

### Community 472 - "Firebase Auth for Flutter"
Cohesion: 0.40
Nodes (4): Firebase Auth for Flutter, Getting Started, Issues and feedback, Usage

### Community 474 - "journal_constants.dart"
Cohesion: 0.18
Nodes (10): firestoreId, firestoreJournalId, journalDocumentIdForFirestore, journalDocumentIdFromFirestore, journalReferenceIdForFirestore, journalReferenceIdFromFirestore, legacyJournalFirestoreId, legacyJournalId (+2 more)

### Community 477 - "fractional_index.dart"
Cohesion: 0.20
Nodes (9): after, before, between, _digits, first, FractionalIndex, _keyAfter, _keyBefore (+1 more)

### Community 478 - "trackersProvider"
Cohesion: 0.29
Nodes (7): pinnedNotesProvider, trackersProvider, _AnalyticsSection, _AnalyticsSectionState, build, _PinnedNotesSection, _PinnedNotesSectionState

### Community 481 - "StudyRepository"
Cohesion: 0.67
Nodes (3): StudyRepository, _FakeStudyRepository, _RecordingStudyRepository

### Community 483 - "NotificationPopoverWarmup"
Cohesion: 0.20
Nodes (9): package:fake_async/fake_async.dart, package:voyager/core/sync/connectivity_status.dart, binding, main, offline, online, start, timeout (+1 more)

### Community 498 - "_PinnedNoteRowState"
Cohesion: 0.22
Nodes (8): AppFonts, applyTo, family, leadingFor, monoFamily, style, trackingFor, _tuned

### Community 499 - "voyager_spacing.dart"
Cohesion: 0.22
Nodes (8): compactListVerticalDensity, lg, md, sm, VoyagerSpacing, xl, xs, xxs

### Community 500 - "periodic_prompt_service.dart"
Cohesion: 0.22
Nodes (8): isDue, kTrackerStorageWeekStartsMonday, longestJournalStreak, missedPeriods, _nextPeriod, PeriodicPromptService, periodStartFor, trackerPeriodStartFor

### Community 501 - "MultiFactorUserHostApi"
Cohesion: 0.22
Nodes (8): MultiFactorUserHostApi, MultiFactorUserHostApi, EnrollPhone, EnrollTotp, GetEnrolledFactors, GetSession, SetUp, Unenroll

### Community 502 - "build"
Cohesion: 0.25
Nodes (8): cachedCurrentWeatherProvider, connectivityStatusProvider, syncActivityProvider, build, _ConnectivityIndicator, _SyncActivityIndicator, _WeatherButton, _WeatherButtonState

### Community 503 - "window_size_class.dart"
Cohesion: 0.29
Nodes (6): BuildContext, isCompactWidth, kCompactWidthBreakpoint, WindowSizeClass, WindowSizeClassX, WindowSizeClass get

### Community 504 - "ChangeNotifier"
Cohesion: 0.29
Nodes (7): ChangeNotifier, _Repaint, CapsLockState, TodoSortDebugLogger, CalendarEventTapState, LifeTreeCanvasController, _TreeAnimation

### Community 505 - "MultiFactorTotpHostApi"
Cohesion: 0.29
Nodes (6): MultiFactorTotpHostApi, MultiFactorTotpHostApi, GenerateSecret, GetAssertionForEnrollment, GetAssertionForSignIn, SetUp

### Community 509 - "FinanceRepository"
Cohesion: 0.04
Nodes (49): AppDatabase, DateTime? stamp,
  int, InMemorySyncRepository, id, OutOfSyncJournalEntryPurge, OutOfSyncJournalEntryTarget, purgeAll, purgeOne (+41 more)

### Community 515 - "PixelBufferTexture"
Cohesion: 0.47
Nodes (4): CopyBufferCallback, FlutterDesktopPixelBuffer, PixelBufferTexture, copy_buffer_callback_

### Community 516 - "package:voyager/core/constants/app_constants.dart"
Cohesion: 0.33
Nodes (5): isExpired, purgeCutoff, purgeEligibleAfter, SoftDeletePolicy, package:voyager/core/constants/app_constants.dart

### Community 517 - "package:voyager/features/life_tracker/life_tree_geometry.dart"
Cohesion: 0.33
Nodes (5): generateLifeTreeGeometry, lifeTreeGeometryProvider, lifeTreeGroundedLeavesProvider, LifeTreeGeometry, package:voyager/features/life_tracker/life_tree_geometry.dart

### Community 518 - "RenderBox"
Cohesion: 0.50
Nodes (4): RenderBox, _RenderSingleChildViewport, RenderAbstractViewport, RenderObjectWithChildMixin

### Community 519 - "_SingleChildViewportElement"
Cohesion: 0.50
Nodes (4): SingleChildRenderObjectElement, _SingleChildViewportElement, NotifiableElementMixin, ViewportElementMixin

### Community 520 - "package:voyager/core/vim/vim_text_ops.dart"
Cohesion: 0.33
Nodes (5): MultiFactorTotpSecretHostApi, MultiFactorTotpSecretHostApi, GenerateQrCodeUrl, OpenInOtpApp, SetUp

### Community 527 - "SetUp"
Cohesion: 0.40
Nodes (4): GenerateInterfaces, GenerateInterfaces, PigeonInterface, SetUp

### Community 528 - "-randomNonce"
Cohesion: 0.50
Nodes (4): -authorizationControllerdidCompleteWithAuthorization, -randomNonce, -stringBySha256HashingString, NSString

### Community 529 - "FixedExtentScrollPhysics"
Cohesion: 0.67
Nodes (3): FixedExtentScrollPhysics, VoyagerWheelPhysics, _HighFrictionFixedExtentScrollPhysics

### Community 652 - "package:voyager/domain/models/analytics_models.dart"
Cohesion: 0.15
Nodes (13): HWND, LPARAM, LRESULT, UINT, WPARAM, FlutterWindow, flutter_controller_, FlutterWindow::FlutterWindow() (+5 more)

### Community 657 - "interpolate_consecutive_test.dart"
Cohesion: 0.40
Nodes (5): Action, _SuspendingRedoAction, _SuspendingUndoAction, RedoTextIntent, UndoTextIntent

### Community 668 - "neetcode150.dart"
Cohesion: 0.25
Nodes (7): countNeetCode150Matches, kLeetCodeProblemsetUrl, kNeetCode150Count, kNeetCodeRoadmapUrl, matched, neetCode150TitleLookup, neetCode150Titles

### Community 687 - "_HeatmapSquare"
Cohesion: 0.08
Nodes (25): FirebaseAuthHostApi, ApplyActionCode, CheckActionCode, ConfirmPasswordReset, CreateUserWithEmailAndPassword, FetchSignInMethodsForEmail, InitializeRecaptchaConfig, RegisterAuthStateListener (+17 more)

## Knowledge Gaps
- **13507 isolated node(s):** `Status`, `Goals`, `Non-goals (v1)`, `Architecture overview`, `Clipboard rules` (+13502 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **69 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `ActionCodeInfoOperation` connect `calendarId` to `Platform Domain`, `Windows Ephemeral`, `Constants Google`, `Features Settings`, `calendarNavigateLeftKey`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Why does `variant` connect `calendarNavigateLeftKey` to `Widgets Rounded`, `newline_normalization.dart`, `Models Domain`?**
  _High betweenness centrality (0.025) - this node is a cross-community bridge._
- **Why does `InternalActionCodeInfo` connect `Constants Google` to `calendarId`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **What connects `Status`, `Goals`, `Non-goals (v1)` to the rest of the system?**
  _13508 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Database Data` be split into smaller, more focused modules?**
  _Cohesion score 0.0022099447513812156 - nodes in this community are weakly interconnected._
- **Should `Calendar Features` be split into smaller, more focused modules?**
  _Cohesion score 0.007518796992481203 - nodes in this community are weakly interconnected._
- **Should `Calendar Day` be split into smaller, more focused modules?**
  _Cohesion score 0.008403361344537815 - nodes in this community are weakly interconnected._