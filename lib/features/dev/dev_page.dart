...
class DevPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return KeepAliveScrollView(
      ...
      children: [
        Text('Developer tools', style: Theme.of(context).textTheme.headlineSmall),
        const SizedBox(height: 8),
        Text('Local debugging utilities. These affect only this device.', style: Theme.of(context).textTheme.bodySmall),
        ...
        ListTile(
          ...
          onTap: () {
            ...
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Providers invalidated')));
          },
        ),
        ListTile(
          title: const Text('Purge soft-deleted items'),
          subtitle: const Text('Permanently remove items past the 30-day retention window'),
          ...
          onTap: () async {
            ...
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Soft-deleted items purged')));
            }
          },
        ),
        ...
        ListTile(
          ...
          subtitle: const Text('Deletes every journal container. Entries are not removed.'),
  ...
  Future<void> _confirmResetJournals(BuildContext context, WidgetRef ref) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Reset journals')),
        ],
      ),
    );
    ...
    ref.invalidate(journalEntriesProvider);
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('All journals deleted')));
    }
  }
}
