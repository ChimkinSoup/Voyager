      ...
      child: entriesAsync.when(
        data: (entries) => trackersAsync.when(
          data: (trackers) => rankingConfigsAsync.when(
            data: (rankingConfigs) {
            final words = entries.fold<int>(0, (sum, e) => sum + analytics.countWords(e.body));
            final streak = prompt.longestJournalStreak(entries);
            final calendarTrackers = trackers.where((t) => t.showOnCalendar).toList();
            return KeepAliveScrollView(
              storageKey: ShellPageStorageKeys.analyticsList,
              children: [
                Text('Total entries: ${analytics.totalJournalEntries(entries)}'),
                        ...
                        LineChartBarData(
                          spots: [
                            for (var i = 0; i < entries.length; i++)
                              FlSpot(i.toDouble(), analytics.countWords(entries[i].body).toDouble()),
                          ],
                          isCurved: true,
                ...
                Row(
                  children: [
                    Text('Statistic Trackers', style: Theme.of(context).textTheme.titleLarge),
                    const Spacer(),
                    FilledButton.icon(
                ...
                if (trackers.isEmpty)
                  const Card(
                    child: Padding(
                      ...
                      child: Text('No trackers yet. Create one to start logging custom stats.'),
                    ),
                  ),
                ...trackers.map(
                  (tracker) => _TrackerCard(
                    tracker: tracker,
                    weekStartsMonday: ref.watch(settingsProvider).value?.weekStartsOnMonday ?? true,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Column(
      ...
      children: [
        Row(
          children: [
            Text('Periodic Rankings', style: Theme.of(context).textTheme.titleLarge),
            const Spacer(),
            FilledButton.icon(
        ...
        if (configs.isEmpty)
          const Card(
            child: Padding(
              padding: EdgeInsets.all(16),
              child: Text('No rankings yet. Create one to get prompted on its cadence.'),
            ),
          ),
...
class _RankingCard extends ConsumerWidget {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: valuesAsync.when(
          data: (values) {
            final sorted = [...values]..sort((a, b) => a.periodStart.compareTo(b.periodStart));
            final lastCompleted = sorted.isEmpty ? null : sorted.last.periodStart;
            return Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Row(
                  children: [
                    Expanded(
                      child: Text(config.name, style: Theme.of(context).textTheme.titleMedium),
                    ),
                ...
                RankingPromptBanner(
                  ...
                  onSubmit: (value) => _saveRanking(ref, value, weekStartsMonday),
                ),
  ...
  Future<void> _saveRanking(WidgetRef ref, int value, bool weekStartsMonday) async {
...
class _RankingDialogState extends State<_RankingDialog> {
  ...
  var _colorStart = 0xFF4CAF50;
  var _colorEnd = 0xFFF44336;
          ...
          children: [
            ...
            DropdownButtonFormField<TrackerCadence>(
              ...
              items: TrackerCadence.values
                  .map((cadence) => DropdownMenuItem(value: cadence, child: Text(cadence.name)))
                  .toList(),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return valuesAsync.when(
      data: (values) => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('${tracker.name} calendar heatmap', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 8),
          HeatmapCalendar(
            tracker: tracker,
            values: values,
...
class _TrackerCardState extends ConsumerState<_TrackerCard> {
        ...
        child: valuesAsync.when(
          data: (values) {
            final current = values.cast<TrackerValue?>().firstWhere(
                  (value) => value != null && _samePeriod(value.periodStart, periodStart, widget.tracker.cadence),
                  orElse: () => null,
                );
            ...
            return Column(
              ...
              children: [
                Row(
                  children: [
                    CircleAvatar(radius: 6, backgroundColor: Color(widget.tracker.colorValue)),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(widget.tracker.name, style: Theme.of(context).textTheme.titleMedium),
                    ),
                    Switch(
  ...
  void _syncIntegerController(TrackerValue? value) {
    ...
    _intController.text = (value?.intValue ?? widget.tracker.defaultInt).toString();
  }

  Widget _valueEditor({required TrackerValue? current, required DateTime periodStart}) {
    switch (widget.tracker.type) {
      case TrackerType.integer:
        final cap = widget.tracker.integerCap;
        final parsed = int.tryParse(_intController.text) ?? widget.tracker.defaultInt;
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            if (cap != null)
              Slider(
                ...
                onChanged: (value) {
                  setState(() => _intController.text = value.round().toString());
                },
      ...
      case TrackerType.enumType:
        ...
        final currentValue = current?.enumValue ?? widget.tracker.defaultEnumOption;
  ...
  Future<void> _saveInteger(TrackerValue? current, DateTime periodStart) async {
    ...
    await _saveValue(current: current, periodStart: periodStart, intValue: value);
  }
  ...
  Future<void> _saveValue({
    ...
    final value = TrackerValue(
      id: current?.id ?? '${widget.tracker.id}_${periodStart.millisecondsSinceEpoch}',
...
class _TrackerHistoryChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final sorted = [...values]..sort((a, b) => a.periodStart.compareTo(b.periodStart));
    final spots = <FlSpot>[
      for (var i = 0; i < sorted.length; i++) FlSpot(i.toDouble(), _numericValue(sorted[i])),
    ];
    if (spots.isEmpty) return const SizedBox.shrink();

    final maxY = switch (tracker.type) {
      TrackerType.integer => (tracker.integerCap ?? spots.map((spot) => spot.y).fold<double>(1, (max, y) => y > max ? y : max)).toDouble(),
  ...
  double _numericValue(TrackerValue value) {
    return switch (tracker.type) {
      ...
      TrackerType.enumType => value.enumValue == null || value.enumValue!.isEmpty ? 0 : 1,
    };
  }
}

class _TrackerDialog extends StatefulWidget {
  const _TrackerDialog();

  @override
  State<_TrackerDialog> createState() => _TrackerDialogState();
}

class _TrackerDialogState extends State<_TrackerDialog> {
  ...
  var _colorValue = voyagerColorPalette.first;
              ...
              if (_type == TrackerType.enumType) ...[
                TextField(
                  ...
                  onChanged: (_) => setState(() {
                    if (!enumOptions.contains(_defaultEnumOption)) _defaultEnumOption = null;
                  }),
                ),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  initialValue: enumOptions.contains(_defaultEnumOption) ? _defaultEnumOption : null,
                  decoration: const InputDecoration(labelText: 'Default option'),
                  items: enumOptions.map((option) => DropdownMenuItem(value: option, child: Text(option))).toList(),
                  onChanged: (value) => setState(() => _defaultEnumOption = value),
                ),
              ],
            ],
          ),
  ...
  void _submit() {
    ...
    Navigator.pop(
      ...
      StatisticTracker(
        ...
        integerCap: _type == TrackerType.integer && _hasCap ? int.tryParse(_capController.text.trim()) : null,
...
DateTime _periodStart(
  ...
  return switch (cadence) {
    ...
    TrackerCadence.weekly => local.subtract(
        Duration(days: weekStartsMonday ? local.weekday - DateTime.monday : local.weekday % 7),
      ),
