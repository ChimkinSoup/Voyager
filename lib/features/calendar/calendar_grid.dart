              ...
              return Expanded(
                child: Row(
                  children: List.generate(7, (col) {
                    ...
                    final dayIndicators = indicators.where((i) => _sameDay(i.day, date)).take(3).toList();
                    return Expanded(
                      child: InkWell(
                        onTap: () => onDayTap(date),
                                  ...
                                  child: Column(
                                    ...
                                    children: dayEvents.take(2).map((e) {
                                      return Container(
                                        ...
                                        color: Color(e.colorValue).withValues(alpha: 0.45),
                                        child: Text(
                                          e.title,
                                          maxLines: 1,
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: List.generate(7, (i) {
        ...
        final dayEvents = events.where((e) => _sameDay(e.start, date)).toList();
        final dayIndicators = indicators.where((indicator) => _sameDay(indicator.day, date)).take(4).toList();
        return Expanded(
          child: InkWell(
            onTap: () => onDayTap(date),
                      ...
                      children: dayEvents
                          .map(
                            (e) => Padding(
                              ...
                              child: Row(
                                children: [
                                  CircleAvatar(radius: 3, backgroundColor: Color(e.colorValue)),
                                  const SizedBox(width: 4),
                                  Expanded(
                                    child: Text(e.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 10)),
                                  ),
                                ],
                              ),
                            ),
...
class _YearGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GridView.builder(
      ...
      itemBuilder: (_, i) {
        final month = i + 1;
        final count = events.where((e) => e.start.year == focused.year && e.start.month == month).length;
        final statCount = indicators.where((indicator) => indicator.day.year == focused.year && indicator.day.month == month).length;
        return InkWell(
          ...
          child: Card(
            ...
            child: Center(
              child: Column(
                ...
                children: [
                  Text(DateTime(focused.year, month).toString().split(' ')[0].split('-')[1]),
                  Text('$count events', style: Theme.of(context).textTheme.bodySmall),
                  if (statCount > 0) Text('$statCount stats', style: Theme.of(context).textTheme.bodySmall),
                ],
              ),
            ),
          ),
        ...
        for (final indicator in indicators)
          Tooltip(
            ...
            child: Container(
              ...
              height: 7,
              decoration: BoxDecoration(
                color: Color(indicator.colorValue).withValues(alpha: (0.35 + 0.65 * indicator.intensity).clamp(0.35, 1)),
