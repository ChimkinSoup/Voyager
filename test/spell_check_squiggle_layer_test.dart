...
void main() {
  ...
  testWidgets('hides the squiggle for a word while it is still being typed, '
    ...
    for (final ch in 'xqz'.split('')) {
      ...
      expect(
        ...
        isFalse,
        reason:
            'still typing "${controller.text}" — nothing should be flagged yet',
      );
    }
  ...
  testWidgets(
    'an edit earlier in the text does not disturb a later flagged word',
    (tester) async {
      final service = VoyagerSpellCheckService()
        ..updateDictionary({'cat', 'dog'});
  ...
  testWidgets(
    'a swap to unrelated text (reused controller) is fully rechecked',
    (tester) async {
      final service = VoyagerSpellCheckService()..updateDictionary({'hello'});
      final controller = TextEditingController(
  ...
  testWidgets(
    'pasting a misspelled word in Normal mode underlines it immediately',
    (tester) async {
      final service = VoyagerSpellCheckService()..updateDictionary({'hello'});
  ...
  testWidgets('leaving Insert rechecks a just-typed misspelling', (
    tester,
  ) async {
    final service = VoyagerSpellCheckService()..updateDictionary({'hello'});
    final controller = TextEditingController();
  ...
  testWidgets(
    'adding a custom word clears every occurrence immediately, without any edit',
    (tester) async {
      final service = VoyagerSpellCheckService()..updateDictionary({'hello'});
      final controller = TextEditingController(
        text: 'voyager hello Voyager voyager',
      );
      final focusNode = FocusNode();
      addTearDown(controller.dispose);
      addTearDown(focusNode.dispose);
  ...
  testWidgets(
    'picks up a dictionary change on the next real edit via the generation check',
    (tester) async {
      final service = VoyagerSpellCheckService()..updateDictionary({'hello'});
