...
// Voyager patch: firebase_auth's Windows plugin invokes IdTokenListener and
// AuthStateListener callbacks directly from whatever thread the Firebase C++
// Auth SDK fires them on, calling flutter::EventSink::Success() straight
// from there. That violates Flutter's platform-channel threading contract
// (channel messages must originate on the platform thread) and is a known,
// currently unfixed upstream bug — see
// https://github.com/firebase/flutterfire/issues/17800 and
// https://github.com/firebase/flutterfire/issues/11933 (the sibling report
// against cloud_firestore, whose Windows plugin already guards against this
// exact class of bug with the identical PlatformThreadDispatcher +
// EventSinkState pattern below — ported verbatim from
// cloud_firestore_plugin.cpp since it's the proven-correct fix, not a new
// one written for this file).
constexpr wchar_t kTaskRunnerWindowClassName[] =
    L"FirebaseAuthWindowsTaskRunnerWindow";
constexpr UINT kTaskRunnerWindowMessage = WM_APP + 0x4175;
...
class FlutterIdTokenListener : public firebase::auth::IdTokenListener {
 public:
  void SetEventSink(
    ...
    // Mutate the existing state (rather than replacing state_ with a new
    // object) so that an in-flight posted callback still holding the old
    // shared_ptr observes the cleared sink under the same lock, instead of
    // firing Success() against an event sink the stream has already
    // cancelled.
    if (!state_) {
      state_ = std::make_shared<EventSinkState>();
    }
    std::lock_guard<std::mutex> lock(state_->mutex);
    state_->events = std::move(event_sink);
  }
  ...
  void OnIdTokenChanged(Auth* auth) override {
    ...
    // OnIdTokenChanged fires from whatever thread the Firebase Auth SDK's
    // internal token-refresh machinery runs on, not necessarily the
    // platform thread — route through the dispatcher instead of calling
    // the event sink directly (see the PlatformThreadDispatcher comment
    // above).
    if (user.is_valid()) {
...
class FlutterAuthStateListener : public firebase::auth::AuthStateListener {
 public:
  void SetEventSink(
      std::unique_ptr<flutter::EventSink<flutter::EncodableValue>> event_sink) {
    // See FlutterIdTokenListener::SetEventSink — mutate the existing state
    // rather than replacing state_, so an in-flight posted callback sees
    // the cleared sink instead of firing against a cancelled stream.
    if (!state_) {
      state_ = std::make_shared<EventSinkState>();
    }
    std::lock_guard<std::mutex> lock(state_->mutex);
  ...
  void OnAuthStateChanged(Auth* auth) override {
    ...
    // OnAuthStateChanged, like OnIdTokenChanged above, fires from whatever
    // thread the Firebase Auth SDK runs it on — route through the
    // dispatcher rather than calling the event sink directly.
    if (user.is_valid()) {
      EncodableList userDetailsList = EncodableList();
      userDetailsList.push_back(userDetails.user_info().ToEncodableList());
      userDetailsList.push_back(userDetails.provider_data());
